...
const char* const LevelsFilter::s_nodeName = "LevelsFilter";
...
LevelsFilter::LevelsFilter(BMediaAddOn* pAddOn) :
	// * init base classes
...
{	
	...
	BDiscreteParameter *state = main->MakeDiscreteParameter(P_LEVEL, B_MEDIA_RAW_VIDEO, "Level", "Level");
	state->AddItem(B_HOST_TO_LENDIAN_INT32(1), "1");
	state->AddItem(B_HOST_TO_LENDIAN_INT32(3), "3");
	state->AddItem(B_HOST_TO_LENDIAN_INT32(5), "5");
	state->AddItem(B_HOST_TO_LENDIAN_INT32(15), "15");
	state->AddItem(B_HOST_TO_LENDIAN_INT32(17), "17");
	state->AddItem(B_HOST_TO_LENDIAN_INT32(51), "51");
	state->AddItem(B_HOST_TO_LENDIAN_INT32(85), "85");
	state->AddItem(B_HOST_TO_LENDIAN_INT32(255), "255");
	
	level = B_HOST_TO_LENDIAN_INT32(1);
	
	fLastLevelChange = system_time();
...
status_t LevelsFilter::HandleMessage(
	int32 code,
	const void* pData,
	size_t size) {
...
BMediaAddOn* LevelsFilter::AddOn(
	int32* poID) const {
...
void LevelsFilter::HandleEvent(const media_timed_event* pEvent, bigtime_t howLate, bool realTimeEvent)
{
	ASSERT(pEvent);
...
void LevelsFilter::NodeRegistered() {

	PRINT(("LevelsFilter::NodeRegistered()\n"));
...
bigtime_t LevelsFilter::OfflineTime()
{
...
status_t LevelsFilter::AcceptFormat(const media_destination& destination, media_format* format) 
{
...
void LevelsFilter::BufferReceived(BBuffer* pBuffer)
{
	ASSERT(pBuffer);
	...
	{
		PRINT(("LevelsFilter::BufferReceived():\n"
			"\tBad destination.\n"));
		pBuffer->Recycle();
		return;
	}
...
status_t LevelsFilter::Connected(
	const media_source& source,
	const media_destination& destination,
	const media_format& format,
	media_input* poInput) {
	
	PRINT(("LevelsFilter::Connected()\n""\tto source %ld\n", source.id));
...
void LevelsFilter::Disconnected(const media_source& source, const media_destination& destination)
{	
	PRINT(("LevelsFilter::Disconnected()\n"));
...
void LevelsFilter::DisposeInputCookie(int32 cookie)
{
...
status_t LevelsFilter::FormatChanged(
	const media_source& source,
	const media_destination& destination,
	int32 changeTag,
...
status_t LevelsFilter::GetLatencyFor(
	const media_destination& destination,
	bigtime_t* poLatency,
	media_node_id* poTimeSource) {
	
	PRINT(("LevelsFilter::GetLatencyFor()\n"));
...
status_t LevelsFilter::GetNextInput(int32* pioCookie, media_input* poInput)
{
	if(*pioCookie != 0)
		return B_BAD_INDEX;
...
void LevelsFilter::ProducerDataStatus(const media_destination& destination, int32 status, bigtime_t tpWhen)
{
	PRINT(("LevelsFilter::ProducerDataStatus()\n"));
...
status_t LevelsFilter::SeekTagRequested(
	const media_destination& destination,
	...
	uint32* poFlags)
{
	...
	PRINT(("LevelsFilter::SeekTagRequested()\n"
		"\tNot implemented.\n"));
	return B_ERROR;
}
...
void LevelsFilter::AdditionalBufferRequested(
	const media_source& source,
...
//	additionalbufferrequested = true;
	PRINT(("LevelsFilter::AdditionalBufferRequested\n"
		"\tOffline mode not implemented."));
}
		
void LevelsFilter::Connect(status_t status, const media_source& source, const media_destination& destination, const media_format& format,	char* pioName)
{
	
	PRINT(("LevelsFilter::Connect()\n"));
	status_t err;
...
void LevelsFilter::Disconnect(const media_source& source, const media_destination& destination)
{
	PRINT(("LevelsFilter::Disconnect()\n"));
...
status_t LevelsFilter::DisposeOutputCookie(int32 cookie)
{
	return B_OK;
}
		
void LevelsFilter::EnableOutput(
	const media_source& source,
	bool enabled,
	int32* _deprecated_)
{
	PRINT(("LevelsFilter::EnableOutput()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tbad source\n"));
		return;
...
status_t LevelsFilter::FormatChangeRequested(
	const media_source& source,
	const media_destination& destination,
	...
	int32* _deprecated_)
{
	...
	// deny
	PRINT(("LevelsFilter::FormatChangeRequested()\n"
		"\tNot supported.\n"));
		
	return B_MEDIA_BAD_FORMAT;
}
		
status_t LevelsFilter::FormatProposal(
	const media_source& source,
	media_format* pioFormat)
{

	PRINT(("LevelsFilter::FormatProposal()\n"));
...
status_t LevelsFilter::FormatSuggestionRequested(
	media_type type,
	int32 quality,
	media_format* poFormat)
{	
	PRINT(("LevelsFilter::FormatSuggestionRequested()\n"));
...
status_t LevelsFilter::GetLatency(bigtime_t* poLatency)
{	
	PRINT(("LevelsFilter::GetLatency()\n"));
...
status_t LevelsFilter::GetNextOutput(int32* pioCookie, media_output* poOutput)
{
	if (*pioCookie != 0)
		return B_BAD_INDEX;
...
void LevelsFilter::LatencyChanged(
	const media_source& source,
	const media_destination& destination,
	bigtime_t newLatency,
	uint32 flags)
{
	PRINT(("LevelsFilter::LatencyChanged()\n"));
...
void LevelsFilter::LateNoticeReceived(
	const media_source& source,
	bigtime_t howLate,
	bigtime_t tpWhen)
{
	PRINT(("LevelsFilter::LateNoticeReceived()\n"
		"\thowLate == %Ld\n"
		"\twhen    == %Ld\n", howLate, tpWhen));
...
status_t LevelsFilter::PrepareToConnect(
	const media_source& source,
	const media_destination& destination,
	media_format* pioFormat,
	...
	char* poName)
{
	...
	PRINT(("LevelsFilter::PrepareToConnect()\n"
		"\tproposed format: %s\n", formatStr));
...
status_t LevelsFilter::SetBufferGroup(
	const media_source& source,
	BBufferGroup* pGroup)
{
	PRINT(("LevelsFilter::SetBufferGroup()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tBad source.\n"));
		return B_MEDIA_BAD_SOURCE;
...
status_t LevelsFilter::SetPlayRate(int32 numerator, int32 denominator)
{
	// not supported
	return B_ERROR;
}
	
status_t LevelsFilter::VideoClippingChanged(
	const media_source& source,
	int16 numShorts,
	int16* pClipData,
...
status_t LevelsFilter::GetParameterValue(int32 id, bigtime_t* last_change, void* value, size_t* size)
{
	if (id != P_LEVEL)
		return B_BAD_VALUE;

	*last_change = fLastLevelChange;
	*size = sizeof(uint32);
	*((uint32 *)value) = level;
...
void LevelsFilter::SetParameterValue(int32 id, bigtime_t when, const void *value,	size_t size)
{
	if ((id != P_LEVEL) || !value || (size != sizeof(uint32)))
		return;

	if (*(int32 *)value == level)
		return;

	level = *(uint32 *)value;
	fLastLevelChange = when;
	BroadcastNewParameterValue(fLastLevelChange, P_LEVEL, &level, sizeof(level));
}
...
void LevelsFilter::handleStartEvent(
	const media_timed_event* pEvent) {
	PRINT(("LevelsFilter::handleStartEvent\n"));

	startFilter();	
}
		
void LevelsFilter::handleStopEvent(
	const media_timed_event* pEvent) {
	PRINT(("LevelsFilter::handleStopEvent\n"));
	
	stopFilter();
}
		
void LevelsFilter::ignoreEvent(const media_timed_event* pEvent)
{
	PRINT(("LevelsFilter::ignoreEvent\n"));	
}
...
//void LevelsFilter::specializeOutputFormat(media_format& ioFormat)
//{
//	char formatStr[256];
//	string_for_format(ioFormat, formatStr, 255);
//	PRINT(("LevelsFilter::specializeOutputFormat()\n"
//		"\tinput format: %s\n", formatStr));
//	
...
void LevelsFilter::initParameterValues() {
	m_fMixRatio = 0.5;
...
//void LevelsFilter::initParameterWeb() {
//	BParameterWeb* pWeb = new BParameterWeb();
//	BParameterGroup* pTopGroup = pWeb->MakeGroup("LevelsFilter Parameters");
//
...
void LevelsFilter::initFilter() {
	PRINT(("LevelsFilter::initFilter()\n"));
...
void LevelsFilter::startFilter() {
	PRINT(("LevelsFilter::startFilter()\n"));
}
void LevelsFilter::stopFilter() {
	PRINT(("LevelsFilter::stopFilter()\n"));
}

// figure processing latency by doing 'dry runs' of filterBuffer()
bigtime_t LevelsFilter::calcProcessingLatency() {
	PRINT(("LevelsFilter::calcProcessingLatency()\n"));
...
void LevelsFilter::filterBuffer(BBuffer* inBuffer)
{
	if (!inBuffer) 
		return;
