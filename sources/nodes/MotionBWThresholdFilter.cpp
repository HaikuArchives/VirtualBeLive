...
const char* const MotionBWThresholdFilter::s_nodeName = "MotionBWThresholdFilter";
...
MotionBWThresholdFilter::~MotionBWThresholdFilter() 
{
	// shut down
	Quit();
...
MotionBWThresholdFilter::MotionBWThresholdFilter(BMediaAddOn* pAddOn) :
	// * init base classes
...
{	
	...
	BContinuousParameter *pbwthreshold = main->MakeContinuousParameter(P_BWTHRESHOLD, B_MEDIA_RAW_VIDEO, "Threshold", "ThresholdBalance", "", 0.0, 255.0, 1.0);
	
	bwthreshold=32;
	fLastthresholdChange = system_time();
...
status_t MotionBWThresholdFilter::HandleMessage(
	int32 code,
	const void* pData,
	size_t size) {
...
BMediaAddOn* MotionBWThresholdFilter::AddOn(
	int32* poID) const {
...
void MotionBWThresholdFilter::SetRunMode(
	run_mode mode) {
...
void MotionBWThresholdFilter::HandleEvent(const media_timed_event* pEvent, bigtime_t howLate, bool realTimeEvent)
{
	ASSERT(pEvent);
...
void MotionBWThresholdFilter::NodeRegistered() {

	PRINT(("MotionBWThresholdFilter::NodeRegistered()\n"));
...
bigtime_t MotionBWThresholdFilter::OfflineTime()
{
...
status_t MotionBWThresholdFilter::AcceptFormat(const media_destination& destination, media_format* format) 
{
...
void MotionBWThresholdFilter::BufferReceived(BBuffer* pBuffer)
{
	ASSERT(pBuffer);
	...
	{
		PRINT(("MotionBWThresholdFilter::BufferReceived():\n"
			"\tBad destination.\n"));
		pBuffer->Recycle();
		return;
	}
...
status_t MotionBWThresholdFilter::Connected(
	const media_source& source,
	const media_destination& destination,
	const media_format& format,
	media_input* poInput) {
	
	PRINT(("MotionBWThresholdFilter::Connected()\n""\tto source %ld\n", source.id));
...
void MotionBWThresholdFilter::Disconnected(const media_source& source, const media_destination& destination)
{	
	PRINT(("MotionBWThresholdFilter::Disconnected()\n"));
...
void MotionBWThresholdFilter::DisposeInputCookie(int32 cookie)
{
...
status_t MotionBWThresholdFilter::FormatChanged(
	const media_source& source,
	const media_destination& destination,
	int32 changeTag,
...
status_t MotionBWThresholdFilter::GetLatencyFor(
	const media_destination& destination,
	bigtime_t* poLatency,
	media_node_id* poTimeSource) {
	
	PRINT(("MotionBWThresholdFilter::GetLatencyFor()\n"));
...
status_t MotionBWThresholdFilter::GetNextInput(int32* pioCookie, media_input* poInput)
{
	if(*pioCookie != 0)
		return B_BAD_INDEX;
...
void MotionBWThresholdFilter::ProducerDataStatus(const media_destination& destination, int32 status, bigtime_t tpWhen)
{
	PRINT(("MotionBWThresholdFilter::ProducerDataStatus()\n"));
...
status_t MotionBWThresholdFilter::SeekTagRequested(
	const media_destination& destination,
	...
	uint32* poFlags)
{
	...
	PRINT(("MotionBWThresholdFilter::SeekTagRequested()\n"
		"\tNot implemented.\n"));
	return B_ERROR;
}
...
void MotionBWThresholdFilter::AdditionalBufferRequested(
	const media_source& source,
...
//	additionalbufferrequested = true;
	PRINT(("MotionBWThresholdFilter::AdditionalBufferRequested\n"
		"\tOffline mode not implemented."));
}
		
void MotionBWThresholdFilter::Connect(status_t status, const media_source& source, const media_destination& destination, const media_format& format,	char* pioName)
{
	
	PRINT(("MotionBWThresholdFilter::Connect()\n"));
	status_t err;
...
void MotionBWThresholdFilter::Disconnect(const media_source& source, const media_destination& destination)
{
	PRINT(("MotionBWThresholdFilter::Disconnect()\n"));
...
status_t MotionBWThresholdFilter::DisposeOutputCookie(int32 cookie)
{
	return B_OK;
}
		
void MotionBWThresholdFilter::EnableOutput(
	const media_source& source,
	bool enabled,
	int32* _deprecated_)
{
	PRINT(("MotionBWThresholdFilter::EnableOutput()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tbad source\n"));
		return;
...
status_t MotionBWThresholdFilter::FormatChangeRequested(
	const media_source& source,
	const media_destination& destination,
	...
	int32* _deprecated_)
{
	...
	// deny
	PRINT(("MotionBWThresholdFilter::FormatChangeRequested()\n"
		"\tNot supported.\n"));
		
	return B_MEDIA_BAD_FORMAT;
}
		
status_t MotionBWThresholdFilter::FormatProposal(
	const media_source& source,
	media_format* pioFormat)
{

	PRINT(("MotionBWThresholdFilter::FormatProposal()\n"));
...
status_t MotionBWThresholdFilter::FormatSuggestionRequested(
	media_type type,
	int32 quality,
	media_format* poFormat)
{	
	PRINT(("MotionBWThresholdFilter::FormatSuggestionRequested()\n"));
...
status_t MotionBWThresholdFilter::GetLatency(bigtime_t* poLatency)
{	
	PRINT(("MotionBWThresholdFilter::GetLatency()\n"));
...
status_t MotionBWThresholdFilter::GetNextOutput(int32* pioCookie, media_output* poOutput)
{
	if (*pioCookie != 0)
		return B_BAD_INDEX;
...
void MotionBWThresholdFilter::LatencyChanged(
	const media_source& source,
	const media_destination& destination,
	bigtime_t newLatency,
	uint32 flags)
{
	PRINT(("MotionBWThresholdFilter::LatencyChanged()\n"));
...
void MotionBWThresholdFilter::LateNoticeReceived(
	const media_source& source,
	bigtime_t howLate,
	bigtime_t tpWhen)
{
	PRINT(("MotionBWThresholdFilter::LateNoticeReceived()\n"
		"\thowLate == %Ld\n"
		"\twhen    == %Ld\n", howLate, tpWhen));
...
status_t MotionBWThresholdFilter::PrepareToConnect(
	const media_source& source,
	const media_destination& destination,
	media_format* pioFormat,
	...
	char* poName)
{
	...
	PRINT(("MotionBWThresholdFilter::PrepareToConnect()\n"
		"\tproposed format: %s\n", formatStr));
...
status_t MotionBWThresholdFilter::SetBufferGroup(
	const media_source& source,
	BBufferGroup* pGroup)
{
	PRINT(("MotionBWThresholdFilter::SetBufferGroup()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tBad source.\n"));
		return B_MEDIA_BAD_SOURCE;
...
status_t MotionBWThresholdFilter::SetPlayRate(int32 numerator, int32 denominator)
{
	// not supported
	return B_ERROR;
}
	
status_t MotionBWThresholdFilter::VideoClippingChanged(
	const media_source& source,
	int16 numShorts,
	int16* pClipData,
...
status_t MotionBWThresholdFilter::GetParameterValue(int32 id, bigtime_t* last_change, void* value, size_t* size)
{
	switch (id)
	{
		default:
			...
			break;
		case P_BWTHRESHOLD:
			*((float *)value) = bwthreshold;
			break;
		
	}

	*last_change = fLastthresholdChange;
	*size = sizeof(float);

	return B_OK;
}
		
void MotionBWThresholdFilter::SetParameterValue(int32 id, bigtime_t when, const void *value,	size_t size)
{
	float	tmp;
	...
	switch (id)
	{
		case P_BWTHRESHOLD:
			tmp = *((float*)value);
			bwthreshold = (int32)tmp;
			BroadcastNewParameterValue(fLastthresholdChange, id, &bwthreshold, sizeof(int32));
			break;

		default:
			break;
	}
	fLastthresholdChange = when;	
}
...
void MotionBWThresholdFilter::handleStartEvent(
	const media_timed_event* pEvent) {
	PRINT(("MotionBWThresholdFilter::handleStartEvent\n"));

	startFilter();	
}
		
void MotionBWThresholdFilter::handleStopEvent(
	const media_timed_event* pEvent) {
	PRINT(("MotionBWThresholdFilter::handleStopEvent\n"));
	
	stopFilter();
}
		
void MotionBWThresholdFilter::ignoreEvent(const media_timed_event* pEvent)
{
	PRINT(("MotionBWThresholdFilter::ignoreEvent\n"));	
}
...
//void MotionBWThresholdFilter::specializeOutputFormat(media_format& ioFormat)
//{
//	char formatStr[256];
//	string_for_format(ioFormat, formatStr, 255);
//	PRINT(("MotionBWThresholdFilter::specializeOutputFormat()\n"
//		"\tinput format: %s\n", formatStr));
//	
...
void MotionBWThresholdFilter::initParameterValues() {
	m_fMixRatio = 0.5;
...
//void MotionBWThresholdFilter::initParameterWeb() {
//	BParameterWeb* pWeb = new BParameterWeb();
//	BParameterGroup* pTopGroup = pWeb->MakeGroup("MotionBWThresholdFilter Parameters");
//
...
void MotionBWThresholdFilter::initFilter() {
	PRINT(("MotionBWThresholdFilter::initFilter()\n"));
...
void MotionBWThresholdFilter::startFilter() {
	PRINT(("MotionBWThresholdFilter::startFilter()\n"));
}
void MotionBWThresholdFilter::stopFilter() {
	PRINT(("MotionBWThresholdFilter::stopFilter()\n"));
}

// figure processing latency by doing 'dry runs' of filterBuffer()
bigtime_t MotionBWThresholdFilter::calcProcessingLatency() {
	PRINT(("MotionBWThresholdFilter::calcProcessingLatency()\n"));
...
void MotionBWThresholdFilter::filterBuffer(BBuffer* inBuffer)
{
	if (!inBuffer) 
		return;
...
/* Sans BBitmap  */
	...
	uint32 *pi=tmp_in;
	uint32 *t=tmp_buffer;
	uint32 new_intensity,old_intensity;
	...
	while (po < inData + inHeader->size_used/4){
		new_in = *pi++;
		old_in = *t++;
		r = ((new_in&0x00ff0000)>>16);
		g = ((new_in&0x0000ff00)>>8);
		b = (new_in&0x000000ff);
		new_intensity = (r+g+b)/3;
		r = ((old_in&0x00ff0000)>>16);
		g = ((old_in&0x0000ff00)>>8);
		b = (old_in&0x000000ff);
		old_intensity = (r+g+b)/3;
		if (abs(new_intensity-old_intensity)>bwthreshold) *po++=0x00ffffff; 
		else *po++=0x00000000; 
	}
...
// END -- MotionBWThresholdFilter.cpp --
