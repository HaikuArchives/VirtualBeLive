...
const char* const RGBThresholdFilter::s_nodeName = "RGBThresholdFilter";
...
RGBThresholdFilter::~RGBThresholdFilter() 
{
	// shut down
	Quit();
...
RGBThresholdFilter::RGBThresholdFilter(BMediaAddOn* pAddOn) :
	// * init base classes
...
{	
	...
	BContinuousParameter *red = main->MakeContinuousParameter(P_RED, B_MEDIA_RAW_VIDEO, "Red Threshold", "RGBThreshold Balance", "", 0.0, 256.0, 1.0);
	BContinuousParameter *green = main->MakeContinuousParameter(P_GREEN, B_MEDIA_RAW_VIDEO, "Green Threshold", "RGBThreshold Balance", "", 0.0, 256.0, 1.0);
	BContinuousParameter *blue = main->MakeContinuousParameter(P_BLUE, B_MEDIA_RAW_VIDEO, "Blue Threshold", "RGBThreshold Balance", "", 0.0, 256.0, 1.0);
	
	RThreshold = GThreshold = BThreshold = 128;
...
status_t RGBThresholdFilter::HandleMessage(
	int32 code,
	const void* pData,
	size_t size) {
...
BMediaAddOn* RGBThresholdFilter::AddOn(
	int32* poID) const {
...
void RGBThresholdFilter::SetRunMode(
	run_mode mode) {
...
void RGBThresholdFilter::HandleEvent(const media_timed_event* pEvent, bigtime_t howLate, bool realTimeEvent)
{
	ASSERT(pEvent);
...
void RGBThresholdFilter::NodeRegistered() {

	PRINT(("RGBThresholdFilter::NodeRegistered()\n"));
...
bigtime_t RGBThresholdFilter::OfflineTime()
{
...
status_t RGBThresholdFilter::AcceptFormat(const media_destination& destination, media_format* format) 
{
...
void RGBThresholdFilter::BufferReceived(BBuffer* pBuffer)
{
	ASSERT(pBuffer);
	...
	{
		PRINT(("RGBThresholdFilter::BufferReceived():\n"
			"\tBad destination.\n"));
		pBuffer->Recycle();
		return;
	}
...
status_t RGBThresholdFilter::Connected(
	const media_source& source,
	const media_destination& destination,
	const media_format& format,
	media_input* poInput) {
	
	PRINT(("RGBThresholdFilter::Connected()\n""\tto source %ld\n", source.id));
...
void RGBThresholdFilter::Disconnected(const media_source& source, const media_destination& destination)
{	
	PRINT(("RGBThresholdFilter::Disconnected()\n"));
...
void RGBThresholdFilter::DisposeInputCookie(int32 cookie)
{
...
status_t RGBThresholdFilter::FormatChanged(
	const media_source& source,
	const media_destination& destination,
	int32 changeTag,
...
status_t RGBThresholdFilter::GetLatencyFor(
	const media_destination& destination,
	bigtime_t* poLatency,
	media_node_id* poTimeSource) {
	
	PRINT(("RGBThresholdFilter::GetLatencyFor()\n"));
...
status_t RGBThresholdFilter::GetNextInput(int32* pioCookie, media_input* poInput)
{
	if(*pioCookie != 0)
		return B_BAD_INDEX;
...
void RGBThresholdFilter::ProducerDataStatus(const media_destination& destination, int32 status, bigtime_t tpWhen)
{
	PRINT(("RGBThresholdFilter::ProducerDataStatus()\n"));
...
status_t RGBThresholdFilter::SeekTagRequested(
	const media_destination& destination,
	...
	uint32* poFlags)
{
	...
	PRINT(("RGBThresholdFilter::SeekTagRequested()\n"
		"\tNot implemented.\n"));
	return B_ERROR;
}
...
void RGBThresholdFilter::AdditionalBufferRequested(
	const media_source& source,
...
//	additionalbufferrequested = true;
	PRINT(("RGBThresholdFilter::AdditionalBufferRequested\n"
		"\tOffline mode not implemented."));
}
		
void RGBThresholdFilter::Connect(status_t status, const media_source& source, const media_destination& destination, const media_format& format,	char* pioName)
{
	
	PRINT(("RGBThresholdFilter::Connect()\n"));
	status_t err;
...
void RGBThresholdFilter::Disconnect(const media_source& source, const media_destination& destination)
{
	PRINT(("RGBThresholdFilter::Disconnect()\n"));
...
status_t RGBThresholdFilter::DisposeOutputCookie(int32 cookie)
{
	return B_OK;
}
		
void RGBThresholdFilter::EnableOutput(
	const media_source& source,
	bool enabled,
	int32* _deprecated_)
{
	PRINT(("RGBThresholdFilter::EnableOutput()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tbad source\n"));
		return;
...
status_t RGBThresholdFilter::FormatChangeRequested(
	const media_source& source,
	const media_destination& destination,
	...
	int32* _deprecated_)
{
	...
	// deny
	PRINT(("RGBThresholdFilter::FormatChangeRequested()\n"
		"\tNot supported.\n"));
		
	return B_MEDIA_BAD_FORMAT;
}
		
status_t RGBThresholdFilter::FormatProposal(
	const media_source& source,
	media_format* pioFormat)
{

	PRINT(("RGBThresholdFilter::FormatProposal()\n"));
...
status_t RGBThresholdFilter::FormatSuggestionRequested(
	media_type type,
	int32 quality,
	media_format* poFormat)
{	
	PRINT(("RGBThresholdFilter::FormatSuggestionRequested()\n"));
...
status_t RGBThresholdFilter::GetLatency(bigtime_t* poLatency)
{	
	PRINT(("RGBThresholdFilter::GetLatency()\n"));
...
status_t RGBThresholdFilter::GetNextOutput(int32* pioCookie, media_output* poOutput)
{
	if (*pioCookie != 0)
		return B_BAD_INDEX;
...
void RGBThresholdFilter::LatencyChanged(
	const media_source& source,
	const media_destination& destination,
	bigtime_t newLatency,
	uint32 flags)
{
	PRINT(("RGBThresholdFilter::LatencyChanged()\n"));
...
void RGBThresholdFilter::LateNoticeReceived(
	const media_source& source,
	bigtime_t howLate,
	bigtime_t tpWhen)
{
	PRINT(("RGBThresholdFilter::LateNoticeReceived()\n"
		"\thowLate == %Ld\n"
		"\twhen    == %Ld\n", howLate, tpWhen));
...
status_t RGBThresholdFilter::PrepareToConnect(
	const media_source& source,
	const media_destination& destination,
	media_format* pioFormat,
	...
	char* poName)
{
	...
	PRINT(("RGBThresholdFilter::PrepareToConnect()\n"
		"\tproposed format: %s\n", formatStr));
...
status_t RGBThresholdFilter::SetBufferGroup(
	const media_source& source,
	BBufferGroup* pGroup)
{
	PRINT(("RGBThresholdFilter::SetBufferGroup()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tBad source.\n"));
		return B_MEDIA_BAD_SOURCE;
...
status_t RGBThresholdFilter::SetPlayRate(int32 numerator, int32 denominator)
{
	// not supported
	return B_ERROR;
}
	
status_t RGBThresholdFilter::VideoClippingChanged(
	const media_source& source,
	int16 numShorts,
	int16* pClipData,
...
status_t RGBThresholdFilter::GetParameterValue(int32 id, bigtime_t* last_change, void* value, size_t* size)
{
	switch (id)
	{
		default:
...
void RGBThresholdFilter::SetParameterValue(int32 id, bigtime_t when, const void *value,	size_t size)
{
	float	tmp;
...
void RGBThresholdFilter::handleStartEvent(
	const media_timed_event* pEvent) {
	PRINT(("RGBThresholdFilter::handleStartEvent\n"));

	startFilter();	
}
		
void RGBThresholdFilter::handleStopEvent(
	const media_timed_event* pEvent) {
	PRINT(("RGBThresholdFilter::handleStopEvent\n"));
	
	stopFilter();
}
		
void RGBThresholdFilter::ignoreEvent(const media_timed_event* pEvent)
{
	PRINT(("RGBThresholdFilter::ignoreEvent\n"));	
}
...
//void RGBThresholdFilter::specializeOutputFormat(media_format& ioFormat)
//{
//	char formatStr[256];
//	string_for_format(ioFormat, formatStr, 255);
//	PRINT(("RGBThresholdFilter::specializeOutputFormat()\n"
//		"\tinput format: %s\n", formatStr));
//	
...
void RGBThresholdFilter::initParameterValues() {
	m_fMixRatio = 0.5;
...
//void RGBThresholdFilter::initParameterWeb() {
//	BParameterWeb* pWeb = new BParameterWeb();
//	BParameterGroup* pTopGroup = pWeb->MakeGroup("RGBThresholdFilter Parameters");
//
...
void RGBThresholdFilter::initFilter() {
	PRINT(("RGBThresholdFilter::initFilter()\n"));
...
void RGBThresholdFilter::startFilter() {
	PRINT(("RGBThresholdFilter::startFilter()\n"));
}
void RGBThresholdFilter::stopFilter() {
	PRINT(("RGBThresholdFilter::stopFilter()\n"));
}

// figure processing latency by doing 'dry runs' of filterBuffer()
bigtime_t RGBThresholdFilter::calcProcessingLatency() {
	PRINT(("RGBThresholdFilter::calcProcessingLatency()\n"));
...
void RGBThresholdFilter::filterBuffer(BBuffer* inBuffer)
{
	if (!inBuffer) 
		return;
...
/* Sans BBitmap  */
	...
	while (po < inData + inHeader->size_used/4){
		...
		g=(in&0x0000ff00)>>8;
		b=in&0x000000ff;
		if (r>=RThreshold) r=255; else  r=0;
		if (g>=GThreshold) g=255; else  g=0;
		if (b>=BThreshold) b=255; else  b=0;
		*po++=(r<<16)+(g<<8)+b;
	}
