...
#define MAX3(max,a,b,c) { if ((a)>(b)) { max=(a); } else { max=(b); } if ((c)>max) { max=(c); } }
...
const char* const MotionMaskFilter::s_nodeName = "MotionMaskFilter";
...
MotionMaskFilter::~MotionMaskFilter() 
{
	// shut down
	Quit();
...
MotionMaskFilter::MotionMaskFilter(BMediaAddOn* pAddOn) :
	// * init base classes
...
{	
	...
	BContinuousParameter *pthreshold = main->MakeContinuousParameter(P_THRESHOLD, B_MEDIA_RAW_VIDEO, "Threshold", "ThresholdBalance", "", 0.0, 255.0, 1.0);
...
status_t MotionMaskFilter::HandleMessage(
	int32 code,
	const void* pData,
	size_t size) {
...
BMediaAddOn* MotionMaskFilter::AddOn(
	int32* poID) const {
...
void MotionMaskFilter::SetRunMode(
	run_mode mode) {
...
void MotionMaskFilter::HandleEvent(const media_timed_event* pEvent, bigtime_t howLate, bool realTimeEvent)
{
	ASSERT(pEvent);
...
void MotionMaskFilter::NodeRegistered() {

	PRINT(("MotionMaskFilter::NodeRegistered()\n"));
...
bigtime_t MotionMaskFilter::OfflineTime()
{
...
status_t MotionMaskFilter::AcceptFormat(const media_destination& destination, media_format* format) 
{
...
void MotionMaskFilter::BufferReceived(BBuffer* pBuffer)
{
	ASSERT(pBuffer);
	...
	{
		PRINT(("MotionMaskFilter::BufferReceived():\n"
			"\tBad destination.\n"));
		pBuffer->Recycle();
		return;
	}
...
status_t MotionMaskFilter::Connected(
	const media_source& source,
	const media_destination& destination,
	const media_format& format,
	media_input* poInput) {
	
	PRINT(("MotionMaskFilter::Connected()\n""\tto source %ld\n", source.id));
...
void MotionMaskFilter::Disconnected(const media_source& source, const media_destination& destination)
{	
	PRINT(("MotionMaskFilter::Disconnected()\n"));
...
void MotionMaskFilter::DisposeInputCookie(int32 cookie)
{
...
status_t MotionMaskFilter::FormatChanged(
	const media_source& source,
	const media_destination& destination,
	int32 changeTag,
...
status_t MotionMaskFilter::GetLatencyFor(
	const media_destination& destination,
	bigtime_t* poLatency,
	media_node_id* poTimeSource) {
	
	PRINT(("MotionMaskFilter::GetLatencyFor()\n"));
...
status_t MotionMaskFilter::GetNextInput(int32* pioCookie, media_input* poInput)
{
	if(*pioCookie != 0)
		return B_BAD_INDEX;
...
void MotionMaskFilter::ProducerDataStatus(const media_destination& destination, int32 status, bigtime_t tpWhen)
{
	PRINT(("MotionMaskFilter::ProducerDataStatus()\n"));
...
status_t MotionMaskFilter::SeekTagRequested(
	const media_destination& destination,
	...
	uint32* poFlags)
{
	...
	PRINT(("MotionMaskFilter::SeekTagRequested()\n"
		"\tNot implemented.\n"));
	return B_ERROR;
}
...
void MotionMaskFilter::AdditionalBufferRequested(
	const media_source& source,
...
//	additionalbufferrequested = true;
	PRINT(("MotionMaskFilter::AdditionalBufferRequested\n"
		"\tOffline mode not implemented."));
}
		
void MotionMaskFilter::Connect(status_t status, const media_source& source, const media_destination& destination, const media_format& format,	char* pioName)
{
	
	PRINT(("MotionMaskFilter::Connect()\n"));
	status_t err;
...
void MotionMaskFilter::Disconnect(const media_source& source, const media_destination& destination)
{
	PRINT(("MotionMaskFilter::Disconnect()\n"));
...
status_t MotionMaskFilter::DisposeOutputCookie(int32 cookie)
{
	return B_OK;
}
		
void MotionMaskFilter::EnableOutput(
	const media_source& source,
	bool enabled,
	int32* _deprecated_)
{
	PRINT(("MotionMaskFilter::EnableOutput()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tbad source\n"));
		return;
...
status_t MotionMaskFilter::FormatChangeRequested(
	const media_source& source,
	const media_destination& destination,
	...
	int32* _deprecated_)
{
	...
	// deny
	PRINT(("MotionMaskFilter::FormatChangeRequested()\n"
		"\tNot supported.\n"));
		
	return B_MEDIA_BAD_FORMAT;
}
		
status_t MotionMaskFilter::FormatProposal(
	const media_source& source,
	media_format* pioFormat)
{

	PRINT(("MotionMaskFilter::FormatProposal()\n"));
...
status_t MotionMaskFilter::FormatSuggestionRequested(
	media_type type,
	int32 quality,
	media_format* poFormat)
{	
	PRINT(("MotionMaskFilter::FormatSuggestionRequested()\n"));
...
status_t MotionMaskFilter::GetLatency(bigtime_t* poLatency)
{	
	PRINT(("MotionMaskFilter::GetLatency()\n"));
...
status_t MotionMaskFilter::GetNextOutput(int32* pioCookie, media_output* poOutput)
{
	if (*pioCookie != 0)
		return B_BAD_INDEX;
...
void MotionMaskFilter::LatencyChanged(
	const media_source& source,
	const media_destination& destination,
	bigtime_t newLatency,
	uint32 flags)
{
	PRINT(("MotionMaskFilter::LatencyChanged()\n"));
...
void MotionMaskFilter::LateNoticeReceived(
	const media_source& source,
	bigtime_t howLate,
	bigtime_t tpWhen)
{
	PRINT(("MotionMaskFilter::LateNoticeReceived()\n"
		"\thowLate == %Ld\n"
		"\twhen    == %Ld\n", howLate, tpWhen));
...
status_t MotionMaskFilter::PrepareToConnect(
	const media_source& source,
	const media_destination& destination,
	media_format* pioFormat,
	...
	char* poName)
{
	...
	PRINT(("MotionMaskFilter::PrepareToConnect()\n"
		"\tproposed format: %s\n", formatStr));
...
status_t MotionMaskFilter::SetBufferGroup(
	const media_source& source,
	BBufferGroup* pGroup)
{
	PRINT(("MotionMaskFilter::SetBufferGroup()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tBad source.\n"));
		return B_MEDIA_BAD_SOURCE;
...
status_t MotionMaskFilter::SetPlayRate(int32 numerator, int32 denominator)
{
	// not supported
	return B_ERROR;
}
	
status_t MotionMaskFilter::VideoClippingChanged(
	const media_source& source,
	int16 numShorts,
	int16* pClipData,
...
status_t MotionMaskFilter::GetParameterValue(int32 id, bigtime_t* last_change, void* value, size_t* size)
{
	switch (id)
	{
		default:
...
void MotionMaskFilter::SetParameterValue(int32 id, bigtime_t when, const void *value,	size_t size)
{
	float	tmp;
	...
	switch (id)
	{
		case P_THRESHOLD:
			tmp = *((float*)value);
			threshold = (int32)tmp;
			BroadcastNewParameterValue(fLastthresholdChange, id, &threshold, sizeof(int32));
			break;
...
void MotionMaskFilter::handleStartEvent(
	const media_timed_event* pEvent) {
	PRINT(("MotionMaskFilter::handleStartEvent\n"));

	startFilter();	
}
		
void MotionMaskFilter::handleStopEvent(
	const media_timed_event* pEvent) {
	PRINT(("MotionMaskFilter::handleStopEvent\n"));
	
	stopFilter();
}
		
void MotionMaskFilter::ignoreEvent(const media_timed_event* pEvent)
{
	PRINT(("MotionMaskFilter::ignoreEvent\n"));	
}
...
//void MotionMaskFilter::specializeOutputFormat(media_format& ioFormat)
//{
//	char formatStr[256];
//	string_for_format(ioFormat, formatStr, 255);
//	PRINT(("MotionMaskFilter::specializeOutputFormat()\n"
//		"\tinput format: %s\n", formatStr));
//	
...
void MotionMaskFilter::initParameterValues() {
	m_fMixRatio = 0.5;
...
//void MotionMaskFilter::initParameterWeb() {
//	BParameterWeb* pWeb = new BParameterWeb();
//	BParameterGroup* pTopGroup = pWeb->MakeGroup("MotionMaskFilter Parameters");
//
...
void MotionMaskFilter::initFilter() {
	PRINT(("MotionMaskFilter::initFilter()\n"));
...
void MotionMaskFilter::startFilter() {
	PRINT(("MotionMaskFilter::startFilter()\n"));
}
void MotionMaskFilter::stopFilter() {
	PRINT(("MotionMaskFilter::stopFilter()\n"));
}

// figure processing latency by doing 'dry runs' of filterBuffer()
bigtime_t MotionMaskFilter::calcProcessingLatency() {
	PRINT(("MotionMaskFilter::calcProcessingLatency()\n"));
...
void MotionMaskFilter::filterBuffer(BBuffer* inBuffer)
{
	if (!inBuffer) 
		return;
...
/* Sans BBitmap  */
	...
	while (po < inData + inHeader->size_used / 4){
		...
		rabs=abs(( new_r = ((new_in&0x00ff0000)>>16))-((old_in&0x00ff0000)>>16));
		gabs=abs(( new_g = ((new_in&0x0000ff00)>>8))-((old_in&0x0000ff00)>>8));
		babs=abs(( new_b = (new_in&0x000000ff))-(old_in&0x000000ff));
				
		MAX3(delta,rabs,gabs,babs);
		
		if (delta>=threshold)
			*po++=new_in;
		else{
			new_r = (delta*new_r)/(threshold);
			new_g = (delta*new_g)/(threshold);
			new_b = (delta*new_b)/(threshold);
