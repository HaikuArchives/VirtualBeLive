...
const char* const TestTransition::s_nodeName = "TestTransition";
...
TestTransition::~TestTransition() 
{
	// shut down
	if (fOutputConnected)
		Disconnect(m_output.source, m_output.destination);
	if (buffers)
	{
...
TestTransition::TestTransition(BMediaAddOn* pAddOn) :
	// * init base classes
...
{	
	...
	fFirstInputConnected = false;
	fSecondInputConnected =false;
...
status_t TestTransition::HandleMessage(
	int32 code,
	const void* pData,
	size_t size) {
...
BMediaAddOn* TestTransition::AddOn(
	int32* poID) const {
...
void TestTransition::SetRunMode(run_mode mode)
{
...
void TestTransition::HandleEvent(const media_timed_event* pEvent, bigtime_t howLate, bool realTimeEvent)
{
	ASSERT(pEvent);
...
void TestTransition::NodeRegistered()
{
	PRINT(("TestTransition::NodeRegistered()\n"));
...
bigtime_t TestTransition::OfflineTime()
{
...
status_t TestTransition::AcceptFormat(const media_destination& destination, media_format* format) 
{
...
void TestTransition::BufferReceived(BBuffer* pBuffer)
{
	ASSERT(pBuffer);
	...
	{
		PRINT(("TestTransition::BufferReceived():\n"
			"\tBad destination.\n"));
		pBuffer->Recycle();
		return;
	}
	...
	if (firstInputBufferHere && secondInputBufferHere) // que ce passe-t-il si l'un des producteurs n'est plus valable ?
	{
		...
		else printf("TestTransition: Buffer Sent!\n");
...
status_t TestTransition::Connected(const media_source& source, const media_destination& destination, const media_format& format, media_input* poInput)
{	
	PRINT(("TestTransition::Connected()\n""\tto source %ld\n", source.id));
	...
	if (second_input.destination  == destination)
	{
		...
		*poInput = second_input;
		fSecondInputConnected = true;
	}	

	return B_OK;
}

void TestTransition::Disconnected(const media_source& source, const media_destination& destination)
{	
	PRINT(("TestTransition::Disconnected()\n"));
...
void TestTransition::DisposeInputCookie(int32 cookie)
{
...
status_t TestTransition::FormatChanged(
...
status_t TestTransition::GetLatencyFor(
	const media_destination& destination,
	bigtime_t* poLatency,
	media_node_id* poTimeSource) {
	
	PRINT(("TestTransition::GetLatencyFor()\n"));
...
status_t TestTransition::GetNextInput(int32* pioCookie, media_input* poInput)
{
	if (*pioCookie > 1)
		return B_BAD_INDEX;
	if (*pioCookie == 0)
...
void TestTransition::ProducerDataStatus(const media_destination& destination, int32 status, bigtime_t tpWhen)
{
	PRINT(("TestTransition::ProducerDataStatus()\n"));
...
status_t TestTransition::SeekTagRequested(
	const media_destination& destination,
	...
	uint32* poFlags)
{
	...
	PRINT(("TestTransition::SeekTagRequested()\n"
		"\tNot implemented.\n"));
	return B_ERROR;
}
...
void TestTransition::AdditionalBufferRequested(
	const media_source& source,
...
//	additionalbufferrequested = true;
	PRINT(("TestTransition::AdditionalBufferRequested\n"));
}
		
void TestTransition::Connect(status_t status, const media_source& source, const media_destination& destination, const media_format& format,	char* pioName)
{
	
	PRINT(("TestTransition::Connect()\n"));
	status_t err;
...
void TestTransition::Disconnect(const media_source& source, const media_destination& destination)
{
	PRINT(("TestTransition::Disconnect()\n"));
...
status_t TestTransition::DisposeOutputCookie(int32 cookie)
{
	return B_OK;
}
		
void TestTransition::EnableOutput(
	const media_source& source,
	bool enabled,
	int32* _deprecated_)
{
	PRINT(("TestTransition::EnableOutput()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tbad source\n"));
		return;
...
status_t TestTransition::FormatChangeRequested(
	const media_source& source,
	const media_destination& destination,
	...
	int32* _deprecated_)
{
	...
	// deny
	PRINT(("TestTransition::FormatChangeRequested()\n"
		"\tNot supported.\n"));
		
	return B_MEDIA_BAD_FORMAT;
}
		
status_t TestTransition::FormatProposal(
	const media_source& source,
	media_format* pioFormat)
{

	PRINT(("TestTransition::FormatProposal()\n"));
...
status_t TestTransition::FormatSuggestionRequested(
	media_type type,
	int32 quality,
	media_format* poFormat)
{	
	PRINT(("TestTransition::FormatSuggestionRequested()\n"));
...
status_t TestTransition::GetLatency(bigtime_t* poLatency)
{	
	PRINT(("TestTransition::GetLatency()\n"));
...
status_t TestTransition::GetNextOutput(int32* pioCookie, media_output* poOutput)
{
	if (*pioCookie != 0)
		return B_BAD_INDEX;
...
void TestTransition::LatencyChanged(
	const media_source& source,
	const media_destination& destination,
	bigtime_t newLatency,
	uint32 flags)
{
	PRINT(("TestTransition::LatencyChanged()\n"));
...
void TestTransition::LateNoticeReceived(
	const media_source& source,
	bigtime_t howLate,
	bigtime_t tpWhen)
{
	PRINT(("TestTransition::LateNoticeReceived()\n"
		"\thowLate == %Ld\n"
		"\twhen    == %Ld\n", howLate, tpWhen));
...
status_t TestTransition::PrepareToConnect(
	const media_source& source,
	const media_destination& destination,
	media_format* pioFormat,
	...
	char* poName)
{
	...
	PRINT(("TestTransition::PrepareToConnect()\n"
		"\tproposed format: %s\n", formatStr));
...
status_t TestTransition::SetBufferGroup(const media_source& source, BBufferGroup* pGroup)
{
	PRINT(("TestTransition::SetBufferGroup()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tBad source.\n"));
		return B_MEDIA_BAD_SOURCE;
...
status_t TestTransition::SetPlayRate(int32 numerator, int32 denominator)
{
	// not supported
	return B_ERROR;
}
	
status_t TestTransition::VideoClippingChanged(
	const media_source& source,
	int16 numShorts,
	int16* pClipData,
...
status_t TestTransition::GetParameterValue(int32 id, bigtime_t* last_change, void* value, size_t* size)
{
	if (id != P_COLOR)
		return B_BAD_VALUE;
...
void TestTransition::SetParameterValue(int32 id, bigtime_t when, const void *value,	size_t size)
{
	...
	BroadcastNewParameterValue(fLastColorChange, P_COLOR, &fColor, sizeof(fColor));	
}

void TestTransition::Start(bigtime_t performance_time)
{
	BMediaEventLooper::Start(performance_time);
}

void TestTransition::Stop(bigtime_t performance_time, bool immediate)
{
	BMediaEventLooper::Stop(performance_time, immediate);
}

void TestTransition::Seek(bigtime_t media_time, bigtime_t performance_time)
{
	BMediaEventLooper::Seek(media_time, performance_time);
}

void TestTransition::TimeWarp(bigtime_t at_real_time, bigtime_t to_performance_time)
{
...
void TestTransition::handleStartEvent(
	const media_timed_event* pEvent) {
	PRINT(("TestTransition::handleStartEvent\n"));

	startFilter();	
}
		
void TestTransition::handleStopEvent(
	const media_timed_event* pEvent) {
	PRINT(("TestTransition::handleStopEvent\n"));
	
	stopFilter();
}
		
void TestTransition::ignoreEvent(const media_timed_event* pEvent)
{
	PRINT(("TestTransition::ignoreEvent\n"));	
}
...
//void TestTransition::specializeOutputFormat(media_format& ioFormat)
//{
//	char formatStr[256];
//	string_for_format(ioFormat, formatStr, 255);
//	PRINT(("TestTransition::specializeOutputFormat()\n"
//		"\tinput format: %s\n", formatStr));
//	
...
void TestTransition::initFilter() {
	PRINT(("TestTransition::initFilter()\n"));
...
void TestTransition::startFilter() {
	PRINT(("TestTransition::startFilter()\n"));
}
void TestTransition::stopFilter() {
	PRINT(("TestTransition::stopFilter()\n"));
}

// figure processing latency by doing 'dry runs' of filterBuffer()
bigtime_t TestTransition::calcProcessingLatency()
{
	PRINT(("TestTransition::calcProcessingLatency()\n"));
...
void TestTransition::MakeTransition(BBuffer* inBuffer, BBuffer *inBuffer2)
{
	status_t	err;
...
//	CALL("now: %Ld start_time: %Ld\n", TimeSource()->Now(), inHeader->start_time);
	...
	/* Sans BBitmap  */
		...
		uint32	*ptr = (uint32*)finalData;
		while ( ptr < finalData + firstHeader->size_used/4)
		{
			*ptr = *inData1;
			inData1++;
			inData2++;
