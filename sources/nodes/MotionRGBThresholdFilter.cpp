...
const char* const MotionRGBThresholdFilter::s_nodeName = "MotionRGBThresholdFilter";
...
MotionRGBThresholdFilter::~MotionRGBThresholdFilter() 
{
	// shut down
	Quit();
...
MotionRGBThresholdFilter::MotionRGBThresholdFilter(BMediaAddOn* pAddOn) :
	// * init base classes
...
{	
	...
	BContinuousParameter *red = main->MakeContinuousParameter(P_RED, B_MEDIA_RAW_VIDEO, "Red Threshold", "RGBThreshold Balance", "", 0.0, 255.0, 1.0);
	BContinuousParameter *green = main->MakeContinuousParameter(P_GREEN, B_MEDIA_RAW_VIDEO, "Green Threshold", "RGBThreshold Balance", "", 0.0, 255.0, 1.0);
	BContinuousParameter *blue = main->MakeContinuousParameter(P_BLUE, B_MEDIA_RAW_VIDEO, "Blue Threshold", "RGBThreshold Balance", "", 0.0, 255.0, 1.0);
	
	RThreshold = GThreshold = BThreshold = 32;
	fLastRGBThresholdChange = system_time();
...
status_t MotionRGBThresholdFilter::HandleMessage(
	int32 code,
	const void* pData,
	size_t size) {
...
BMediaAddOn* MotionRGBThresholdFilter::AddOn(
	int32* poID) const {
...
void MotionRGBThresholdFilter::SetRunMode(
	run_mode mode) {
...
void MotionRGBThresholdFilter::HandleEvent(const media_timed_event* pEvent, bigtime_t howLate, bool realTimeEvent)
{
	ASSERT(pEvent);
...
void MotionRGBThresholdFilter::NodeRegistered() {

	PRINT(("MotionRGBThresholdFilter::NodeRegistered()\n"));
...
bigtime_t MotionRGBThresholdFilter::OfflineTime()
{
...
status_t MotionRGBThresholdFilter::AcceptFormat(const media_destination& destination, media_format* format) 
{
...
void MotionRGBThresholdFilter::BufferReceived(BBuffer* pBuffer)
{
	ASSERT(pBuffer);
	...
	{
		PRINT(("MotionRGBThresholdFilter::BufferReceived():\n"
			"\tBad destination.\n"));
		pBuffer->Recycle();
		return;
	}
...
status_t MotionRGBThresholdFilter::Connected(
	const media_source& source,
	const media_destination& destination,
	const media_format& format,
	media_input* poInput) {
	
	PRINT(("MotionRGBThresholdFilter::Connected()\n""\tto source %ld\n", source.id));
...
void MotionRGBThresholdFilter::Disconnected(const media_source& source, const media_destination& destination)
{	
	PRINT(("MotionRGBThresholdFilter::Disconnected()\n"));
...
void MotionRGBThresholdFilter::DisposeInputCookie(int32 cookie)
{
...
status_t MotionRGBThresholdFilter::FormatChanged(
	const media_source& source,
	const media_destination& destination,
	int32 changeTag,
...
status_t MotionRGBThresholdFilter::GetLatencyFor(
	const media_destination& destination,
	bigtime_t* poLatency,
	media_node_id* poTimeSource) {
	
	PRINT(("MotionRGBThresholdFilter::GetLatencyFor()\n"));
...
status_t MotionRGBThresholdFilter::GetNextInput(int32* pioCookie, media_input* poInput)
{
	if(*pioCookie != 0)
		return B_BAD_INDEX;
...
void MotionRGBThresholdFilter::ProducerDataStatus(const media_destination& destination, int32 status, bigtime_t tpWhen)
{
	PRINT(("MotionRGBThresholdFilter::ProducerDataStatus()\n"));
...
status_t MotionRGBThresholdFilter::SeekTagRequested(
	const media_destination& destination,
	...
	uint32* poFlags)
{
	...
	PRINT(("MotionRGBThresholdFilter::SeekTagRequested()\n"
		"\tNot implemented.\n"));
	return B_ERROR;
}
...
void MotionRGBThresholdFilter::AdditionalBufferRequested(
	const media_source& source,
...
//	additionalbufferrequested = true;
	PRINT(("MotionRGBThresholdFilter::AdditionalBufferRequested\n"
		"\tOffline mode not implemented."));
}
		
void MotionRGBThresholdFilter::Connect(status_t status, const media_source& source, const media_destination& destination, const media_format& format,	char* pioName)
{
	
	PRINT(("MotionRGBThresholdFilter::Connect()\n"));
	status_t err;
...
void MotionRGBThresholdFilter::Disconnect(const media_source& source, const media_destination& destination)
{
	PRINT(("MotionRGBThresholdFilter::Disconnect()\n"));
...
status_t MotionRGBThresholdFilter::DisposeOutputCookie(int32 cookie)
{
	return B_OK;
}
		
void MotionRGBThresholdFilter::EnableOutput(
	const media_source& source,
	bool enabled,
	int32* _deprecated_)
{
	PRINT(("MotionRGBThresholdFilter::EnableOutput()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tbad source\n"));
		return;
...
status_t MotionRGBThresholdFilter::FormatChangeRequested(
	const media_source& source,
	const media_destination& destination,
	...
	int32* _deprecated_)
{
	...
	// deny
	PRINT(("MotionRGBThresholdFilter::FormatChangeRequested()\n"
		"\tNot supported.\n"));
		
	return B_MEDIA_BAD_FORMAT;
}
		
status_t MotionRGBThresholdFilter::FormatProposal(
	const media_source& source,
	media_format* pioFormat)
{

	PRINT(("MotionRGBThresholdFilter::FormatProposal()\n"));
...
status_t MotionRGBThresholdFilter::FormatSuggestionRequested(
	media_type type,
	int32 quality,
	media_format* poFormat)
{	
	PRINT(("MotionRGBThresholdFilter::FormatSuggestionRequested()\n"));
...
status_t MotionRGBThresholdFilter::GetLatency(bigtime_t* poLatency)
{	
	PRINT(("MotionRGBThresholdFilter::GetLatency()\n"));
...
status_t MotionRGBThresholdFilter::GetNextOutput(int32* pioCookie, media_output* poOutput)
{
	if (*pioCookie != 0)
		return B_BAD_INDEX;
...
void MotionRGBThresholdFilter::LatencyChanged(
	const media_source& source,
	const media_destination& destination,
	bigtime_t newLatency,
	uint32 flags)
{
	PRINT(("MotionRGBThresholdFilter::LatencyChanged()\n"));
...
void MotionRGBThresholdFilter::LateNoticeReceived(
	const media_source& source,
	bigtime_t howLate,
	bigtime_t tpWhen)
{
	PRINT(("MotionRGBThresholdFilter::LateNoticeReceived()\n"
		"\thowLate == %Ld\n"
		"\twhen    == %Ld\n", howLate, tpWhen));
...
status_t MotionRGBThresholdFilter::PrepareToConnect(
	const media_source& source,
	const media_destination& destination,
	media_format* pioFormat,
	...
	char* poName)
{
	...
	PRINT(("MotionRGBThresholdFilter::PrepareToConnect()\n"
		"\tproposed format: %s\n", formatStr));
...
status_t MotionRGBThresholdFilter::SetBufferGroup(
	const media_source& source,
	BBufferGroup* pGroup)
{
	PRINT(("MotionRGBThresholdFilter::SetBufferGroup()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tBad source.\n"));
		return B_MEDIA_BAD_SOURCE;
...
status_t MotionRGBThresholdFilter::SetPlayRate(int32 numerator, int32 denominator)
{
	// not supported
	return B_ERROR;
}
	
status_t MotionRGBThresholdFilter::VideoClippingChanged(
	const media_source& source,
	int16 numShorts,
	int16* pClipData,
...
status_t MotionRGBThresholdFilter::GetParameterValue(int32 id, bigtime_t* last_change, void* value, size_t* size)
{
	switch (id)
	{
		default:
			...
			break;
		case P_RED:
			*((float *)value) = RThreshold;
			break;
		case P_GREEN:
			*((float *)value) = GThreshold;
			break;
		case P_BLUE:
			*((float *)value) = BThreshold;
			break;
	}

	*last_change = fLastRGBThresholdChange;
	*size = sizeof(float);

	return B_OK;
}
		
void MotionRGBThresholdFilter::SetParameterValue(int32 id, bigtime_t when, const void *value,	size_t size)
{
	float	tmp;
	...
	switch (id)
	{
		case P_RED:
			tmp = *((float*)value);
			RThreshold = (int32)tmp;
			BroadcastNewParameterValue(fLastRGBThresholdChange, id, &RThreshold, sizeof(int32));
			break;
		case P_GREEN:
			tmp = *((float*)value);
			GThreshold = (int32)tmp;
			BroadcastNewParameterValue(fLastRGBThresholdChange, id, &GThreshold, sizeof(int32));
			break;
		case P_BLUE:
			tmp = *((float*)value);
			BThreshold = (int32)tmp;
			BroadcastNewParameterValue(fLastRGBThresholdChange, id, &BThreshold, sizeof(int32));
			break;
		default:
			break;
	}
	fLastRGBThresholdChange = when;	
}
...
void MotionRGBThresholdFilter::handleStartEvent(
	const media_timed_event* pEvent) {
	PRINT(("MotionRGBThresholdFilter::handleStartEvent\n"));

	startFilter();	
}
		
void MotionRGBThresholdFilter::handleStopEvent(
	const media_timed_event* pEvent) {
	PRINT(("MotionRGBThresholdFilter::handleStopEvent\n"));
	
	stopFilter();
}
		
void MotionRGBThresholdFilter::ignoreEvent(const media_timed_event* pEvent)
{
	PRINT(("MotionRGBThresholdFilter::ignoreEvent\n"));	
}
...
//void MotionRGBThresholdFilter::specializeOutputFormat(media_format& ioFormat)
//{
//	char formatStr[256];
//	string_for_format(ioFormat, formatStr, 255);
//	PRINT(("MotionRGBThresholdFilter::specializeOutputFormat()\n"
//		"\tinput format: %s\n", formatStr));
//	
...
void MotionRGBThresholdFilter::initParameterValues() {
	m_fMixRatio = 0.5;
...
//void MotionRGBThresholdFilter::initParameterWeb() {
//	BParameterWeb* pWeb = new BParameterWeb();
//	BParameterGroup* pTopGroup = pWeb->MakeGroup("MotionRGBThresholdFilter Parameters");
//
...
void MotionRGBThresholdFilter::initFilter() {
	PRINT(("MotionRGBThresholdFilter::initFilter()\n"));
...
void MotionRGBThresholdFilter::startFilter() {
	PRINT(("MotionRGBThresholdFilter::startFilter()\n"));
}
void MotionRGBThresholdFilter::stopFilter() {
	PRINT(("MotionRGBThresholdFilter::stopFilter()\n"));
}

// figure processing latency by doing 'dry runs' of filterBuffer()
bigtime_t MotionRGBThresholdFilter::calcProcessingLatency() {
	PRINT(("MotionRGBThresholdFilter::calcProcessingLatency()\n"));
...
void MotionRGBThresholdFilter::filterBuffer(BBuffer* inBuffer)
{
	if (!inBuffer) 
		return;
...
/* Sans BBitmap  */
	...
	uint32 *po = inData;
	uint32 *t=tmp_buffer;
	uint32 new_in,old_in;
	uint32 new_r,new_g,new_b,old_r,old_g,old_b;
	...
	while (po < inData + inHeader->size_used/4){
		new_in = *pi++;
		old_in = *t++;
		new_r = ((new_in&0x00ff0000)>>16);
		new_g = ((new_in&0x0000ff00)>>8);
		new_b = (new_in&0x000000ff);
		old_r = ((old_in&0x00ff0000)>>16);
		old_g = ((old_in&0x0000ff00)>>8);
		old_b = (old_in&0x000000ff);
		
		if (abs(new_r-old_r)>RThreshold) new_r=255; else new_r=0;
		if (abs(new_g-old_g)>GThreshold) new_g=255; else new_g=0;
		if (abs(new_b-old_b)>BThreshold) new_b=255; else new_b=0;
		
		*po++=(new_r<<16)+(new_g<<8)+new_b;	
	}
...
// END -- MotionRGBThresholdFilter.cpp --
