...
#define FLOOR(val, low)			{if(val<low) val=low;}
#define CEIL(val, high)			{if(val>high) val=high;}
...
const char* const BlurFilter::s_nodeName = "BlurFilter";
...
BlurFilter::BlurFilter(BMediaAddOn* pAddOn) :
	// * init base classes
...
status_t BlurFilter::HandleMessage(
	int32 code,
	const void* pData,
	size_t size) {
...
void BlurFilter::HandleEvent(const media_timed_event* pEvent, bigtime_t howLate, bool realTimeEvent)
{
	ASSERT(pEvent);
...
void BlurFilter::NodeRegistered() {

	PRINT(("BlurFilter::NodeRegistered()\n"));
...
bigtime_t BlurFilter::OfflineTime()
{
...
status_t BlurFilter::AcceptFormat(const media_destination& destination, media_format* format) 
{
...
void BlurFilter::BufferReceived(BBuffer* pBuffer)
{
	ASSERT(pBuffer);
	...
	{
		PRINT(("BlurFilter::BufferReceived():\n"
			"\tBad destination.\n"));
		pBuffer->Recycle();
		return;
	}
...
status_t BlurFilter::Connected(
	const media_source& source,
	const media_destination& destination,
	const media_format& format,
	media_input* poInput) {
	
	PRINT(("BlurFilter::Connected()\n""\tto source %ld\n", source.id));
...
void BlurFilter::Disconnected(const media_source& source, const media_destination& destination)
{	
	PRINT(("BlurFilter::Disconnected()\n"));
...
void BlurFilter::DisposeInputCookie(int32 cookie)
{
...
status_t BlurFilter::FormatChanged(
	const media_source& source,
	const media_destination& destination,
	int32 changeTag,
...
status_t BlurFilter::GetLatencyFor(
	const media_destination& destination,
	bigtime_t* poLatency,
	media_node_id* poTimeSource) {
	
	PRINT(("BlurFilter::GetLatencyFor()\n"));
...
status_t BlurFilter::GetNextInput(int32* pioCookie, media_input* poInput)
{
	if(*pioCookie != 0)
		return B_BAD_INDEX;
...
void BlurFilter::ProducerDataStatus(const media_destination& destination, int32 status, bigtime_t tpWhen)
{
	PRINT(("BlurFilter::ProducerDataStatus()\n"));
...
status_t BlurFilter::SeekTagRequested(
	const media_destination& destination,
	...
	uint32* poFlags)
{
	...
	PRINT(("BlurFilter::SeekTagRequested()\n"
		"\tNot implemented.\n"));
	return B_ERROR;
}
...
void BlurFilter::AdditionalBufferRequested(
	const media_source& source,
...
//	additionalbufferrequested = true;
	PRINT(("BlurFilter::AdditionalBufferRequested\n"
		"\tOffline mode not implemented."));
}
		
void BlurFilter::Connect(status_t status, const media_source& source, const media_destination& destination, const media_format& format,	char* pioName)
{
	
	PRINT(("BlurFilter::Connect()\n"));
	status_t err;
...
void BlurFilter::Disconnect(const media_source& source, const media_destination& destination)
{
	PRINT(("BlurFilter::Disconnect()\n"));
...
status_t BlurFilter::DisposeOutputCookie(int32 cookie)
{
	return B_OK;
}
		
void BlurFilter::EnableOutput(
	const media_source& source,
	bool enabled,
	int32* _deprecated_)
{
	PRINT(("BlurFilter::EnableOutput()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tbad source\n"));
		return;
...
status_t BlurFilter::FormatChangeRequested(
	const media_source& source,
	const media_destination& destination,
	...
	int32* _deprecated_)
{
	...
	// deny
	PRINT(("BlurFilter::FormatChangeRequested()\n"
		"\tNot supported.\n"));
		
	return B_MEDIA_BAD_FORMAT;
}
		
status_t BlurFilter::FormatProposal(
	const media_source& source,
	media_format* pioFormat)
{

	PRINT(("BlurFilter::FormatProposal()\n"));
...
status_t BlurFilter::FormatSuggestionRequested(
	media_type type,
	int32 quality,
	media_format* poFormat)
{	
	PRINT(("BlurFilter::FormatSuggestionRequested()\n"));
...
status_t BlurFilter::GetLatency(bigtime_t* poLatency)
{	
	PRINT(("BlurFilter::GetLatency()\n"));
...
status_t BlurFilter::GetNextOutput(int32* pioCookie, media_output* poOutput)
{
	if (*pioCookie != 0)
		return B_BAD_INDEX;
...
void BlurFilter::LatencyChanged(
	const media_source& source,
	const media_destination& destination,
	bigtime_t newLatency,
	uint32 flags)
{
	PRINT(("BlurFilter::LatencyChanged()\n"));
...
void BlurFilter::LateNoticeReceived(
	const media_source& source,
	bigtime_t howLate,
	bigtime_t tpWhen)
{
	PRINT(("BlurFilter::LateNoticeReceived()\n"
		"\thowLate == %Ld\n"
		"\twhen    == %Ld\n", howLate, tpWhen));
...
status_t BlurFilter::PrepareToConnect(
	const media_source& source,
	const media_destination& destination,
	media_format* pioFormat,
	...
	char* poName)
{
	...
	PRINT(("BlurFilter::PrepareToConnect()\n"
		"\tproposed format: %s\n", formatStr));
...
status_t BlurFilter::SetBufferGroup(
	const media_source& source,
	BBufferGroup* pGroup)
{
	PRINT(("BlurFilter::SetBufferGroup()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tBad source.\n"));
		return B_MEDIA_BAD_SOURCE;
...
status_t BlurFilter::SetPlayRate(int32 numerator, int32 denominator)
{
	// not supported
	return B_ERROR;
}
	
status_t BlurFilter::VideoClippingChanged(
	const media_source& source,
	int16 numShorts,
	int16* pClipData,
...
status_t BlurFilter::GetParameterValue(int32 id, bigtime_t* last_change, void* value, size_t* size)
{
	switch (id)
	{
		case P_RANGE:
...
void BlurFilter::SetParameterValue(int32 id, bigtime_t when, const void *value,	size_t size)
{
	float	tmp;
...
void BlurFilter::handleStartEvent(
	const media_timed_event* pEvent) {
	PRINT(("BlurFilter::handleStartEvent\n"));

	startFilter();	
}
		
void BlurFilter::handleStopEvent(
	const media_timed_event* pEvent) {
	PRINT(("BlurFilter::handleStopEvent\n"));
	
	stopFilter();
}
		
void BlurFilter::ignoreEvent(const media_timed_event* pEvent)
{
	PRINT(("BlurFilter::ignoreEvent\n"));	
}
...
//void BlurFilter::specializeOutputFormat(media_format& ioFormat)
//{
//	char formatStr[256];
//	string_for_format(ioFormat, formatStr, 255);
//	PRINT(("BlurFilter::specializeOutputFormat()\n"
//		"\tinput format: %s\n", formatStr));
//	
...
void BlurFilter::initParameterValues() {
	m_fMixRatio = 0.5;
...
//void BlurFilter::initParameterWeb() {
//	BParameterWeb* pWeb = new BParameterWeb();
//	BParameterGroup* pTopGroup = pWeb->MakeGroup("BlurFilter Parameters");
//
...
void BlurFilter::initFilter() {
	PRINT(("BlurFilter::initFilter()\n"));
...
void BlurFilter::startFilter() {
	PRINT(("BlurFilter::startFilter()\n"));
}
void BlurFilter::stopFilter() {
	PRINT(("BlurFilter::stopFilter()\n"));
}

// figure processing latency by doing 'dry runs' of filterBuffer()
bigtime_t BlurFilter::calcProcessingLatency() {
	PRINT(("BlurFilter::calcProcessingLatency()\n"));
...
void BlurFilter::filterBuffer(BBuffer* inBuffer)
{
	if (!inBuffer) 
		return;
...
/* Sans BBitmap  */
	...
	uint32	*po = (uint32*)inData;
	memcpy(pi,po,inHeader->size_used);
	
	uint32 r,g,b,DIM=(RANGE*2+1),in,out;
	int32 c,l,lCOL,lin,col;

	const int32 COL=m_format.u.raw_video.display.line_width;
	const int32 LIN=m_format.u.raw_video.display.line_count;
	...
	for (l=0;l<LIN;l++){
		r=g=b=0;
		lCOL=l*COL;
		for (c=-(int32)RANGE;c<=(int32)RANGE;c++){
			//if (c<=0) in=pi[l*COL]; else in=pi[l*COL+c];
			col=c;
			FLOOR(col,0);
			in=po[lCOL+col];
			r+=(in&0x00FF0000)>>16;
		...
		pi[lCOL]=((r/DIM)<<16)+((g/DIM)<<8)+b/DIM;
		
		for (c=((int32)RANGE+1);c<(COL+(int32)RANGE);c++){
			//if (c>=(COL-1)) in=pi[l*COL+COL-1]; else in=pi[l*COL+c];
			//if (c<=0) out=pi[l*COL]; else out=pi[l*COL+c-R];
			col=c;
			CEIL(col,COL-1);
			in = po[lCOL+col];
			col=c-DIM;
			...
			g-=(out&0x0000FF00)>>8;
			b-=(out&0x000000FF);
			pi[lCOL+c-RANGE]=((r/DIM)<<16)+((g/DIM)<<8)+b/DIM;
		}
	}
...
//columns (FOR)
	...
	for (c=0;c<COL;c++){
		r=g=b=0;
		for (l=-(int32)RANGE;l<=(int32)RANGE;l++){
			lin=l;
			FLOOR(lin,0);
			in=pi[lin*COL+c];
			r+=(in&0x00FF0000)>>16;
		...
		po[c]=((r/DIM)<<16)+((g/DIM)<<8)+b/DIM;
		
		for (l=((int32)RANGE+1);l<(LIN+(int32)RANGE);l++){
			lin=l;
			CEIL(lin,LIN-1);
			in=pi[lin*COL+c];
			lin=l-DIM;
			...
			g-=(out&0x0000FF00)>>8;
			b-=(out&0x000000FF);
			po[(l-RANGE)*COL+c]=((r/DIM)<<16)+((g/DIM)<<8)+b/DIM;
		}
	}	
...
/*
	...
	while (l<LIN){
		lCOL=l*COL;
		in = po[lCOL];
		r=(RANGE+1)*((in&0x00FF0000)>>16);
		g=(RANGE+1)*((in&0x0000FF00)>>8);
		b=(RANGE+1)*(in&0x000000FF);
		c=1;
		while (c<=RANGE){
			in=po[lCOL+c];
			r+=(in&0x00FF0000)>>16;
