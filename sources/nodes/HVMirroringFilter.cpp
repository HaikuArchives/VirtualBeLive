...
const char* const HVMirroringFilter::s_nodeName = "HVMirroringFilter";
...
HVMirroringFilter::~HVMirroringFilter() 
{
	// shut down
	Quit();
...
HVMirroringFilter::HVMirroringFilter(BMediaAddOn* pAddOn) :
	// * init base classes
...
{	
	...
	BDiscreteParameter *pHVMode = main->MakeDiscreteParameter(P_MODE, B_MEDIA_RAW_VIDEO, "Mirror mode", "Mirror");
		pHVMode->AddItem(0,"Horizontal");
		pHVMode->AddItem(1,"Vertical");
		pHVMode->AddItem(2,"Both");
	
	HVMode=0;
	fLastModeChange = system_time();
...
status_t HVMirroringFilter::HandleMessage(
	int32 code,
	const void* pData,
	size_t size) {
...
BMediaAddOn* HVMirroringFilter::AddOn(
	int32* poID) const {
...
void HVMirroringFilter::SetRunMode(
	run_mode mode) {
...
void HVMirroringFilter::HandleEvent(const media_timed_event* pEvent, bigtime_t howLate, bool realTimeEvent)
{
	ASSERT(pEvent);
...
void HVMirroringFilter::NodeRegistered() {

	PRINT(("HVMirroringFilter::NodeRegistered()\n"));
...
bigtime_t HVMirroringFilter::OfflineTime()
{
...
status_t HVMirroringFilter::AcceptFormat(const media_destination& destination, media_format* format) 
{
...
void HVMirroringFilter::BufferReceived(BBuffer* pBuffer)
{
	ASSERT(pBuffer);
...
status_t HVMirroringFilter::Connected(
	const media_source& source,
	const media_destination& destination,
	const media_format& format,
	media_input* poInput) {
	
	PRINT(("HVMirroringFilter::Connected()\n""\tto source %ld\n", source.id));
...
void HVMirroringFilter::Disconnected(const media_source& source, const media_destination& destination)
{	
	PRINT(("HVMirroringFilter::Disconnected()\n"));
...
void HVMirroringFilter::DisposeInputCookie(int32 cookie)
{
...
status_t HVMirroringFilter::FormatChanged(
	const media_source& source,
	const media_destination& destination,
	int32 changeTag,
...
status_t HVMirroringFilter::GetLatencyFor(
	const media_destination& destination,
	bigtime_t* poLatency,
	media_node_id* poTimeSource) {
	
	PRINT(("HVMirroringFilter::GetLatencyFor()\n"));
...
status_t HVMirroringFilter::GetNextInput(int32* pioCookie, media_input* poInput)
{
	if(*pioCookie != 0)
		return B_BAD_INDEX;
...
void HVMirroringFilter::ProducerDataStatus(const media_destination& destination, int32 status, bigtime_t tpWhen)
{
	PRINT(("HVMirroringFilter::ProducerDataStatus()\n"));
...
status_t HVMirroringFilter::SeekTagRequested(
	const media_destination& destination,
	...
	uint32* poFlags)
{
	...
	PRINT(("HVMirroringFilter::SeekTagRequested()\n"
		"\tNot implemented.\n"));
	return B_ERROR;
}
...
void HVMirroringFilter::AdditionalBufferRequested(
	const media_source& source,
...
//	additionalbufferrequested = true;
	PRINT(("HVMirroringFilter::AdditionalBufferRequested\n"
		"\tOffline mode not implemented."));
}
		
void HVMirroringFilter::Connect(status_t status, const media_source& source, const media_destination& destination, const media_format& format,	char* pioName)
{
	
	PRINT(("HVMirroringFilter::Connect()\n"));
	status_t err;
...
void HVMirroringFilter::Disconnect(const media_source& source, const media_destination& destination)
{
	PRINT(("HVMirroringFilter::Disconnect()\n"));
...
status_t HVMirroringFilter::DisposeOutputCookie(int32 cookie)
{
	return B_OK;
}
		
void HVMirroringFilter::EnableOutput(
	const media_source& source,
	bool enabled,
	int32* _deprecated_)
{
	PRINT(("HVMirroringFilter::EnableOutput()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tbad source\n"));
		return;
...
status_t HVMirroringFilter::FormatChangeRequested(
	const media_source& source,
	const media_destination& destination,
	...
	int32* _deprecated_)
{
	...
	// deny
	PRINT(("HVMirroringFilter::FormatChangeRequested()\n"
		"\tNot supported.\n"));
		
	return B_MEDIA_BAD_FORMAT;
}
		
status_t HVMirroringFilter::FormatProposal(
	const media_source& source,
	media_format* pioFormat)
{

	PRINT(("HVMirroringFilter::FormatProposal()\n"));
...
status_t HVMirroringFilter::FormatSuggestionRequested(
	media_type type,
	int32 quality,
	media_format* poFormat)
{	
	PRINT(("HVMirroringFilter::FormatSuggestionRequested()\n"));
...
status_t HVMirroringFilter::GetLatency(bigtime_t* poLatency)
{	
	PRINT(("HVMirroringFilter::GetLatency()\n"));
...
status_t HVMirroringFilter::GetNextOutput(int32* pioCookie, media_output* poOutput)
{
	if (*pioCookie != 0)
		return B_BAD_INDEX;
...
void HVMirroringFilter::LatencyChanged(
	const media_source& source,
	const media_destination& destination,
	bigtime_t newLatency,
	uint32 flags)
{
	PRINT(("HVMirroringFilter::LatencyChanged()\n"));
...
void HVMirroringFilter::LateNoticeReceived(
	const media_source& source,
	bigtime_t howLate,
	bigtime_t tpWhen)
{
	PRINT(("HVMirroringFilter::LateNoticeReceived()\n"
		"\thowLate == %Ld\n"
		"\twhen    == %Ld\n", howLate, tpWhen));
...
status_t HVMirroringFilter::PrepareToConnect(
	const media_source& source,
	const media_destination& destination,
	media_format* pioFormat,
	...
	char* poName)
{
	...
	PRINT(("HVMirroringFilter::PrepareToConnect()\n"
		"\tproposed format: %s\n", formatStr));
...
status_t HVMirroringFilter::SetBufferGroup(
	const media_source& source,
	BBufferGroup* pGroup)
{
	PRINT(("HVMirroringFilter::SetBufferGroup()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tBad source.\n"));
		return B_MEDIA_BAD_SOURCE;
...
status_t HVMirroringFilter::SetPlayRate(int32 numerator, int32 denominator)
{
	// not supported
	return B_ERROR;
}
	
status_t HVMirroringFilter::VideoClippingChanged(
	const media_source& source,
	int16 numShorts,
	int16* pClipData,
...
status_t HVMirroringFilter::GetParameterValue(int32 id, bigtime_t* last_change, void* value, size_t* size)
{
	switch (id)
	{
		case P_MODE:
	...
	*last_change = fLastModeChange;
	*size = sizeof(uint32);

	return B_OK;
}
		
void HVMirroringFilter::SetParameterValue(int32 id, bigtime_t when, const void *value,	size_t size)
{
	...
	switch (id)
	{
		case P_MODE:
			HVMode = *(uint32*)value;
			BroadcastNewParameterValue(fLastModeChange, P_MODE, &HVMode, sizeof(uint32));
			break;
	
		default:
			break;
	}
	fLastModeChange = when;
}
...
void HVMirroringFilter::handleStartEvent(
	const media_timed_event* pEvent) {
	PRINT(("HVMirroringFilter::handleStartEvent\n"));

	startFilter();	
}
		
void HVMirroringFilter::handleStopEvent(
	const media_timed_event* pEvent) {
	PRINT(("HVMirroringFilter::handleStopEvent\n"));
	
	stopFilter();
}
		
void HVMirroringFilter::ignoreEvent(const media_timed_event* pEvent)
{
	PRINT(("HVMirroringFilter::ignoreEvent\n"));	
}
...
//void HVMirroringFilter::specializeOutputFormat(media_format& ioFormat)
//{
//	char formatStr[256];
//	string_for_format(ioFormat, formatStr, 255);
//	PRINT(("HVMirroringFilter::specializeOutputFormat()\n"
//		"\tinput format: %s\n", formatStr));
//	
...
void HVMirroringFilter::initParameterValues() {
	m_fMixRatio = 0.5;
...
//void HVMirroringFilter::initParameterWeb() {
//	BParameterWeb* pWeb = new BParameterWeb();
//	BParameterGroup* pTopGroup = pWeb->MakeGroup("HVMirroringFilter Parameters");
//
...
void HVMirroringFilter::initFilter() {
	PRINT(("HVMirroringFilter::initFilter()\n"));
...
void HVMirroringFilter::startFilter() {
	PRINT(("HVMirroringFilter::startFilter()\n"));
}
void HVMirroringFilter::stopFilter() {
	PRINT(("HVMirroringFilter::stopFilter()\n"));
}

// figure processing latency by doing 'dry runs' of filterBuffer()
bigtime_t HVMirroringFilter::calcProcessingLatency() {
	PRINT(("HVMirroringFilter::calcProcessingLatency()\n"));
...
void HVMirroringFilter::filterBuffer(BBuffer* inBuffer)
{
	if (!inBuffer) 
		return;
