...
const char* const MixFilter::s_nodeName = "MixFilter";
...
MixFilter::MixFilter(BMediaAddOn* pAddOn) :
	// * init base classes
...
{	
	...
	BDiscreteParameter *state = main->MakeDiscreteParameter(P_MIX, B_MEDIA_RAW_VIDEO, "Mix", "Mix");
	state->AddItem(B_HOST_TO_LENDIAN_INT32(1), "RGB -> RGB");
	state->AddItem(B_HOST_TO_LENDIAN_INT32(2), "RGB -> RBG");
	state->AddItem(B_HOST_TO_LENDIAN_INT32(3), "RGB -> GRB");
	state->AddItem(B_HOST_TO_LENDIAN_INT32(4), "RGB -> GBR");
	state->AddItem(B_HOST_TO_LENDIAN_INT32(5), "RGB -> BRG");
	state->AddItem(B_HOST_TO_LENDIAN_INT32(6), "RGB -> BGR");
	
	mix = B_HOST_TO_LENDIAN_INT32(1);
	
	fLastMixChange = system_time();
...
status_t MixFilter::HandleMessage(
	int32 code,
	const void* pData,
	size_t size) {
...
void MixFilter::HandleEvent(const media_timed_event* pEvent, bigtime_t howLate, bool realTimeEvent)
{
	ASSERT(pEvent);
...
void MixFilter::NodeRegistered() {

	PRINT(("MixFilter::NodeRegistered()\n"));
...
bigtime_t MixFilter::OfflineTime()
{
...
status_t MixFilter::AcceptFormat(const media_destination& destination, media_format* format) 
{
...
void MixFilter::BufferReceived(BBuffer* pBuffer)
{
	ASSERT(pBuffer);
	...
	{
		PRINT(("MixFilter::BufferReceived():\n"
			"\tBad destination.\n"));
		pBuffer->Recycle();
		return;
	}
...
status_t MixFilter::Connected(
	const media_source& source,
	const media_destination& destination,
	const media_format& format,
	media_input* poInput) {
	
	PRINT(("MixFilter::Connected()\n""\tto source %ld\n", source.id));
...
void MixFilter::Disconnected(const media_source& source, const media_destination& destination)
{	
	PRINT(("MixFilter::Disconnected()\n"));
...
void MixFilter::DisposeInputCookie(int32 cookie)
{
...
status_t MixFilter::FormatChanged(
	const media_source& source,
	const media_destination& destination,
	int32 changeTag,
...
status_t MixFilter::GetLatencyFor(
	const media_destination& destination,
	bigtime_t* poLatency,
	media_node_id* poTimeSource) {
	
	PRINT(("MixFilter::GetLatencyFor()\n"));
...
status_t MixFilter::GetNextInput(int32* pioCookie, media_input* poInput)
{
	if(*pioCookie != 0)
		return B_BAD_INDEX;
...
void MixFilter::ProducerDataStatus(const media_destination& destination, int32 status, bigtime_t tpWhen)
{
	PRINT(("MixFilter::ProducerDataStatus()\n"));
...
status_t MixFilter::SeekTagRequested(
	const media_destination& destination,
	...
	uint32* poFlags)
{
	...
	PRINT(("MixFilter::SeekTagRequested()\n"
		"\tNot implemented.\n"));
	return B_ERROR;
}
...
void MixFilter::AdditionalBufferRequested(
	const media_source& source,
...
//	additionalbufferrequested = true;
	PRINT(("MixFilter::AdditionalBufferRequested\n"
		"\tOffline mode not implemented."));
}
		
void MixFilter::Connect(status_t status, const media_source& source, const media_destination& destination, const media_format& format,	char* pioName)
{
	
	PRINT(("MixFilter::Connect()\n"));
	status_t err;
...
void MixFilter::Disconnect(const media_source& source, const media_destination& destination)
{
	PRINT(("MixFilter::Disconnect()\n"));
...
status_t MixFilter::DisposeOutputCookie(int32 cookie)
{
	return B_OK;
}
		
void MixFilter::EnableOutput(
	const media_source& source,
	bool enabled,
	int32* _deprecated_)
{
	PRINT(("MixFilter::EnableOutput()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tbad source\n"));
		return;
...
status_t MixFilter::FormatChangeRequested(
	const media_source& source,
	const media_destination& destination,
	...
	int32* _deprecated_)
{
	...
	// deny
	PRINT(("MixFilter::FormatChangeRequested()\n"
		"\tNot supported.\n"));
		
	return B_MEDIA_BAD_FORMAT;
}
		
status_t MixFilter::FormatProposal(
	const media_source& source,
	media_format* pioFormat)
{

	PRINT(("MixFilter::FormatProposal()\n"));
...
status_t MixFilter::FormatSuggestionRequested(
	media_type type,
	int32 quality,
	media_format* poFormat)
{	
	PRINT(("MixFilter::FormatSuggestionRequested()\n"));
...
status_t MixFilter::GetLatency(bigtime_t* poLatency)
{	
	PRINT(("MixFilter::GetLatency()\n"));
...
status_t MixFilter::GetNextOutput(int32* pioCookie, media_output* poOutput)
{
	if (*pioCookie != 0)
		return B_BAD_INDEX;
...
void MixFilter::LatencyChanged(
	const media_source& source,
	const media_destination& destination,
	bigtime_t newLatency,
	uint32 flags)
{
	PRINT(("MixFilter::LatencyChanged()\n"));
...
void MixFilter::LateNoticeReceived(
	const media_source& source,
	bigtime_t howLate,
	bigtime_t tpWhen)
{
	PRINT(("MixFilter::LateNoticeReceived()\n"
		"\thowLate == %Ld\n"
		"\twhen    == %Ld\n", howLate, tpWhen));
...
status_t MixFilter::PrepareToConnect(
	const media_source& source,
	const media_destination& destination,
	media_format* pioFormat,
	...
	char* poName)
{
	...
	PRINT(("MixFilter::PrepareToConnect()\n"
		"\tproposed format: %s\n", formatStr));
...
status_t MixFilter::SetBufferGroup(
	const media_source& source,
	BBufferGroup* pGroup)
{
	PRINT(("MixFilter::SetBufferGroup()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tBad source.\n"));
		return B_MEDIA_BAD_SOURCE;
...
status_t MixFilter::SetPlayRate(int32 numerator, int32 denominator)
{
	// not supported
	return B_ERROR;
}
	
status_t MixFilter::VideoClippingChanged(
	const media_source& source,
	int16 numShorts,
	int16* pClipData,
...
status_t MixFilter::GetParameterValue(int32 id, bigtime_t* last_change, void* value, size_t* size)
{
	if (id != P_MIX)
		return B_BAD_VALUE;

	*last_change = fLastMixChange;
	*size = sizeof(uint32);
	*((uint32 *)value) = mix;
...
void MixFilter::SetParameterValue(int32 id, bigtime_t when, const void *value,	size_t size)
{
	if ((id != P_MIX) || !value || (size != sizeof(uint32)))
		return;
	...
	mix = *(uint32 *)value;
	fLastMixChange = when;
	BroadcastNewParameterValue(fLastMixChange, P_MIX, &mix, sizeof(mix));
}
...
void MixFilter::handleStartEvent(
	const media_timed_event* pEvent) {
	PRINT(("MixFilter::handleStartEvent\n"));

	startFilter();	
}
		
void MixFilter::handleStopEvent(
	const media_timed_event* pEvent) {
	PRINT(("MixFilter::handleStopEvent\n"));
	
	stopFilter();
}
		
void MixFilter::ignoreEvent(const media_timed_event* pEvent)
{
	PRINT(("MixFilter::ignoreEvent\n"));	
}
...
//void MixFilter::specializeOutputFormat(media_format& ioFormat)
//{
//	char formatStr[256];
//	string_for_format(ioFormat, formatStr, 255);
//	PRINT(("MixFilter::specializeOutputFormat()\n"
//		"\tinput format: %s\n", formatStr));
//	
...
void MixFilter::initParameterValues() {
	m_fMixRatio = 0.5;
...
//void MixFilter::initParameterWeb() {
//	BParameterWeb* pWeb = new BParameterWeb();
//	BParameterGroup* pTopGroup = pWeb->MakeGroup("MixFilter Parameters");
//
...
void MixFilter::initFilter() {
	PRINT(("MixFilter::initFilter()\n"));
...
void MixFilter::startFilter() {
	PRINT(("MixFilter::startFilter()\n"));
}
void MixFilter::stopFilter() {
	PRINT(("MixFilter::stopFilter()\n"));
}

// figure processing latency by doing 'dry runs' of filterBuffer()
bigtime_t MixFilter::calcProcessingLatency() {
	PRINT(("MixFilter::calcProcessingLatency()\n"));
...
void MixFilter::filterBuffer(BBuffer* inBuffer)
{
	if (!inBuffer) 
		return;
...
/* Sans BBitmap  */
	...
	while ( po < inData + inHeader->size_used / 4 ){
		...
		switch (mix){
			case 1:	*po++=(r<<16)+(g<<8)+b;
					break;
			case 2:	*po++=(r<<16)+(b<<8)+g;
					break;
			case 3:	*po++=(g<<16)+(r<<8)+b;
					break;
			case 4:	*po++=(g<<16)+(b<<8)+r;
					break;
			case 5:	*po++=(b<<16)+(r<<8)+g;
					break;
			case 6:	*po++=(b<<16)+(g<<8)+r;
					break;
		}
	}
