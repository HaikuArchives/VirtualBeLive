...
const char* const ContrastBrightnessFilter::s_nodeName = "ContrastBrightnessFilter";
...
ContrastBrightnessFilter::~ContrastBrightnessFilter() 
{
	// shut down
	Quit();
...
ContrastBrightnessFilter::ContrastBrightnessFilter(BMediaAddOn* pAddOn) :
	// * init base classes
...
{	
	...
	BContinuousParameter *pcfactor = main->MakeContinuousParameter(P_CFACTOR, B_MEDIA_RAW_VIDEO, "Contrast", "FactorBalance", "", -100.0, 100.0, 1.0);
	BContinuousParameter *pbfactor = main->MakeContinuousParameter(P_BFACTOR, B_MEDIA_RAW_VIDEO, "Brightness", "FactorBalance", "", -255.0, 255.0, 1.0);
	
	CFACTOR = 0;
	BFACTOR = 0;
	
	fLastFactorChange = system_time();
...
status_t ContrastBrightnessFilter::HandleMessage(
	int32 code,
	const void* pData,
	size_t size) {
...
BMediaAddOn* ContrastBrightnessFilter::AddOn(
	int32* poID) const {
...
void ContrastBrightnessFilter::SetRunMode(
	run_mode mode) {
...
void ContrastBrightnessFilter::HandleEvent(const media_timed_event* pEvent, bigtime_t howLate, bool realTimeEvent)
{
	ASSERT(pEvent);
...
void ContrastBrightnessFilter::NodeRegistered() {

	PRINT(("ContrastBrightnessFilter::NodeRegistered()\n"));
...
bigtime_t ContrastBrightnessFilter::OfflineTime()
{
...
status_t ContrastBrightnessFilter::AcceptFormat(const media_destination& destination, media_format* format) 
{
...
void ContrastBrightnessFilter::BufferReceived(BBuffer* pBuffer)
{
	ASSERT(pBuffer);
	...
	{
		PRINT(("ContrastBrightnessFilter::BufferReceived():\n"
			"\tBad destination.\n"));
		pBuffer->Recycle();
		return;
	}
...
status_t ContrastBrightnessFilter::Connected(
	const media_source& source,
	const media_destination& destination,
	const media_format& format,
	media_input* poInput) {
	
	PRINT(("ContrastBrightnessFilter::Connected()\n""\tto source %ld\n", source.id));
...
void ContrastBrightnessFilter::Disconnected(const media_source& source, const media_destination& destination)
{	
	PRINT(("ContrastBrightnessFilter::Disconnected()\n"));
...
void ContrastBrightnessFilter::DisposeInputCookie(int32 cookie)
{
...
status_t ContrastBrightnessFilter::FormatChanged(
	const media_source& source,
	const media_destination& destination,
	int32 changeTag,
...
status_t ContrastBrightnessFilter::GetLatencyFor(
	const media_destination& destination,
	bigtime_t* poLatency,
	media_node_id* poTimeSource) {
	
	PRINT(("ContrastBrightnessFilter::GetLatencyFor()\n"));
...
status_t ContrastBrightnessFilter::GetNextInput(int32* pioCookie, media_input* poInput)
{
	if(*pioCookie != 0)
		return B_BAD_INDEX;
...
void ContrastBrightnessFilter::ProducerDataStatus(const media_destination& destination, int32 status, bigtime_t tpWhen)
{
	PRINT(("ContrastBrightnessFilter::ProducerDataStatus()\n"));
...
status_t ContrastBrightnessFilter::SeekTagRequested(
	const media_destination& destination,
	...
	uint32* poFlags)
{
	...
	PRINT(("ContrastBrightnessFilter::SeekTagRequested()\n"
		"\tNot implemented.\n"));
	return B_ERROR;
}
...
void ContrastBrightnessFilter::AdditionalBufferRequested(
	const media_source& source,
...
//	additionalbufferrequested = true;
	PRINT(("ContrastBrightnessFilter::AdditionalBufferRequested\n"
		"\tOffline mode not implemented."));
}
		
void ContrastBrightnessFilter::Connect(status_t status, const media_source& source, const media_destination& destination, const media_format& format,	char* pioName)
{
	
	PRINT(("ContrastBrightnessFilter::Connect()\n"));
	status_t err;
...
void ContrastBrightnessFilter::Disconnect(const media_source& source, const media_destination& destination)
{
	PRINT(("ContrastBrightnessFilter::Disconnect()\n"));
...
status_t ContrastBrightnessFilter::DisposeOutputCookie(int32 cookie)
{
	return B_OK;
}
		
void ContrastBrightnessFilter::EnableOutput(
	const media_source& source,
	bool enabled,
	int32* _deprecated_)
{
	PRINT(("ContrastBrightnessFilter::EnableOutput()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tbad source\n"));
		return;
...
status_t ContrastBrightnessFilter::FormatChangeRequested(
	const media_source& source,
	const media_destination& destination,
	...
	int32* _deprecated_)
{
	...
	// deny
	PRINT(("ContrastBrightnessFilter::FormatChangeRequested()\n"
		"\tNot supported.\n"));
		
	return B_MEDIA_BAD_FORMAT;
}
		
status_t ContrastBrightnessFilter::FormatProposal(
	const media_source& source,
	media_format* pioFormat)
{

	PRINT(("ContrastBrightnessFilter::FormatProposal()\n"));
...
status_t ContrastBrightnessFilter::FormatSuggestionRequested(
	media_type type,
	int32 quality,
	media_format* poFormat)
{	
	PRINT(("ContrastBrightnessFilter::FormatSuggestionRequested()\n"));
...
status_t ContrastBrightnessFilter::GetLatency(bigtime_t* poLatency)
{	
	PRINT(("ContrastBrightnessFilter::GetLatency()\n"));
...
status_t ContrastBrightnessFilter::GetNextOutput(int32* pioCookie, media_output* poOutput)
{
	if (*pioCookie != 0)
		return B_BAD_INDEX;
...
void ContrastBrightnessFilter::LatencyChanged(
	const media_source& source,
	const media_destination& destination,
	bigtime_t newLatency,
	uint32 flags)
{
	PRINT(("ContrastBrightnessFilter::LatencyChanged()\n"));
...
void ContrastBrightnessFilter::LateNoticeReceived(
	const media_source& source,
	bigtime_t howLate,
	bigtime_t tpWhen)
{
	PRINT(("ContrastBrightnessFilter::LateNoticeReceived()\n"
		"\thowLate == %Ld\n"
		"\twhen    == %Ld\n", howLate, tpWhen));
...
status_t ContrastBrightnessFilter::PrepareToConnect(
	const media_source& source,
	const media_destination& destination,
	media_format* pioFormat,
	...
	char* poName)
{
	...
	PRINT(("ContrastBrightnessFilter::PrepareToConnect()\n"
		"\tproposed format: %s\n", formatStr));
...
status_t ContrastBrightnessFilter::SetBufferGroup(
	const media_source& source,
	BBufferGroup* pGroup)
{
	PRINT(("ContrastBrightnessFilter::SetBufferGroup()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tBad source.\n"));
		return B_MEDIA_BAD_SOURCE;
...
status_t ContrastBrightnessFilter::SetPlayRate(int32 numerator, int32 denominator)
{
	// not supported
	return B_ERROR;
}
	
status_t ContrastBrightnessFilter::VideoClippingChanged(
	const media_source& source,
	int16 numShorts,
	int16* pClipData,
...
status_t ContrastBrightnessFilter::GetParameterValue(int32 id, bigtime_t* last_change, void* value, size_t* size)
{
	switch (id)
	{
		case P_CFACTOR:
	...
	*last_change = fLastFactorChange;
	*size = sizeof(float);

	return B_OK;
}
		
void ContrastBrightnessFilter::SetParameterValue(int32 id, bigtime_t when, const void *value,	size_t size)
{
	float	tmp;
	...
	switch (id)
	{
		case P_CFACTOR:
			tmp = *((float*)value);
			CFACTOR = (int32)tmp;
			BroadcastNewParameterValue(fLastFactorChange, id, &CFACTOR, sizeof(int32));
			break;
			
		case P_BFACTOR:
			tmp = *((float*)value);
			BFACTOR = (int32)tmp;
			BroadcastNewParameterValue(fLastFactorChange, id, &BFACTOR, sizeof(int32));
			break;
			
		default:
			break;
	}
	fLastFactorChange = when;	
}
...
void ContrastBrightnessFilter::handleStartEvent(
	const media_timed_event* pEvent) {
	PRINT(("ContrastBrightnessFilter::handleStartEvent\n"));

	startFilter();	
}
		
void ContrastBrightnessFilter::handleStopEvent(
	const media_timed_event* pEvent) {
	PRINT(("ContrastBrightnessFilter::handleStopEvent\n"));
	
	stopFilter();
}
		
void ContrastBrightnessFilter::ignoreEvent(const media_timed_event* pEvent)
{
	PRINT(("ContrastBrightnessFilter::ignoreEvent\n"));	
}
...
//void ContrastBrightnessFilter::specializeOutputFormat(media_format& ioFormat)
//{
//	char formatStr[256];
//	string_for_format(ioFormat, formatStr, 255);
//	PRINT(("ContrastBrightnessFilter::specializeOutputFormat()\n"
//		"\tinput format: %s\n", formatStr));
//	
...
void ContrastBrightnessFilter::initParameterValues() {
	m_fMixRatio = 0.5;
...
//void ContrastBrightnessFilter::initParameterWeb() {
//	BParameterWeb* pWeb = new BParameterWeb();
//	BParameterGroup* pTopGroup = pWeb->MakeGroup("ContrastBrightnessFilter Parameters");
//
...
void ContrastBrightnessFilter::initFilter() {
	PRINT(("ContrastBrightnessFilter::initFilter()\n"));
...
void ContrastBrightnessFilter::startFilter() {
	PRINT(("ContrastBrightnessFilter::startFilter()\n"));
}
void ContrastBrightnessFilter::stopFilter() {
	PRINT(("ContrastBrightnessFilter::stopFilter()\n"));
}

// figure processing latency by doing 'dry runs' of filterBuffer()
bigtime_t ContrastBrightnessFilter::calcProcessingLatency() {
	PRINT(("ContrastBrightnessFilter::calcProcessingLatency()\n"));
...
void ContrastBrightnessFilter::filterBuffer(BBuffer* inBuffer)
{
	if (!inBuffer) 
		return;
...
/* Sans BBitmap  */
	...
	//uint32	*po = outData;
	uint32	*pi = (uint32*)inData;
	
	int r,g,b,CFACTOR2;
	
	if (CFACTOR<=0) CFACTOR2=CFACTOR+100;
	else CFACTOR2=CFACTOR*CFACTOR/10+100;
	
	while ( pi < inData + inHeader->size_used/4)
	{
		r = ((*pi & 0x00ff0000)>>16)+BFACTOR;
		g = ((*pi & 0x0000ff00)>>8)+BFACTOR;
		b = (*pi & 0x000000ff)+BFACTOR;
			
		r = (((r-128)*CFACTOR2)/100)+128;
		g = (((g-128)*CFACTOR2)/100)+128;
		b = (((b-128)*CFACTOR2)/100)+128;
...
// END -- ContrastBrightnessFilter.cpp --
