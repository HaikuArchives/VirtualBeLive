...
const char* const MotionBlurFilter::s_nodeName = "MotionBlurFilter";
...
MotionBlurFilter::MotionBlurFilter(BMediaAddOn* pAddOn) :
	// * init base classes
...
{	
	...
	BContinuousParameter *range = main->MakeContinuousParameter(P_IMPACT, B_MEDIA_RAW_VIDEO, "Blur latency (%)", "BiasBalance", "", 0.0, 100.0, 1.0);
...
status_t MotionBlurFilter::HandleMessage(
	int32 code,
	const void* pData,
	size_t size) {
...
BMediaAddOn* MotionBlurFilter::AddOn(
	int32* poID) const {
...
void MotionBlurFilter::SetRunMode(
	run_mode mode) {
...
void MotionBlurFilter::HandleEvent(const media_timed_event* pEvent, bigtime_t howLate, bool realTimeEvent)
{
	ASSERT(pEvent);
...
void MotionBlurFilter::NodeRegistered() {

	PRINT(("MotionBlurFilter::NodeRegistered()\n"));
...
bigtime_t MotionBlurFilter::OfflineTime()
{
...
status_t MotionBlurFilter::AcceptFormat(const media_destination& destination, media_format* format) 
{
...
void MotionBlurFilter::BufferReceived(BBuffer* pBuffer)
{
	ASSERT(pBuffer);
	...
	{
		PRINT(("MotionBlurFilter::BufferReceived():\n"
			"\tBad destination.\n"));
		pBuffer->Recycle();
		return;
	}
...
status_t MotionBlurFilter::Connected(
	const media_source& source,
	const media_destination& destination,
	const media_format& format,
	media_input* poInput) {
	
	PRINT(("MotionBlurFilter::Connected()\n""\tto source %ld\n", source.id));
...
void MotionBlurFilter::Disconnected(const media_source& source, const media_destination& destination)
{	
	PRINT(("MotionBlurFilter::Disconnected()\n"));
...
void MotionBlurFilter::DisposeInputCookie(int32 cookie)
{
...
status_t MotionBlurFilter::FormatChanged(
	const media_source& source,
	const media_destination& destination,
	int32 changeTag,
...
status_t MotionBlurFilter::GetLatencyFor(
	const media_destination& destination,
	bigtime_t* poLatency,
	media_node_id* poTimeSource) {
	
	PRINT(("MotionBlurFilter::GetLatencyFor()\n"));
...
status_t MotionBlurFilter::GetNextInput(int32* pioCookie, media_input* poInput)
{
	if(*pioCookie != 0)
		return B_BAD_INDEX;
...
void MotionBlurFilter::ProducerDataStatus(const media_destination& destination, int32 status, bigtime_t tpWhen)
{
	PRINT(("MotionBlurFilter::ProducerDataStatus()\n"));
...
status_t MotionBlurFilter::SeekTagRequested(
	const media_destination& destination,
	...
	uint32* poFlags)
{
	...
	PRINT(("MotionBlurFilter::SeekTagRequested()\n"
		"\tNot implemented.\n"));
	return B_ERROR;
}
...
void MotionBlurFilter::AdditionalBufferRequested(
	const media_source& source,
...
//	additionalbufferrequested = true;
	PRINT(("MotionBlurFilter::AdditionalBufferRequested\n"
		"\tOffline mode not implemented."));
}
		
void MotionBlurFilter::Connect(status_t status, const media_source& source, const media_destination& destination, const media_format& format,	char* pioName)
{
	
	PRINT(("MotionBlurFilter::Connect()\n"));
	status_t err;
...
void MotionBlurFilter::Disconnect(const media_source& source, const media_destination& destination)
{
	PRINT(("MotionBlurFilter::Disconnect()\n"));
...
status_t MotionBlurFilter::DisposeOutputCookie(int32 cookie)
{
	return B_OK;
}
		
void MotionBlurFilter::EnableOutput(
	const media_source& source,
	bool enabled,
	int32* _deprecated_)
{
	PRINT(("MotionBlurFilter::EnableOutput()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tbad source\n"));
		return;
...
status_t MotionBlurFilter::FormatChangeRequested(
	const media_source& source,
	const media_destination& destination,
	...
	int32* _deprecated_)
{
	...
	// deny
	PRINT(("MotionBlurFilter::FormatChangeRequested()\n"
		"\tNot supported.\n"));
		
	return B_MEDIA_BAD_FORMAT;
}
		
status_t MotionBlurFilter::FormatProposal(
	const media_source& source,
	media_format* pioFormat)
{

	PRINT(("MotionBlurFilter::FormatProposal()\n"));
...
status_t MotionBlurFilter::FormatSuggestionRequested(
	media_type type,
	int32 quality,
	media_format* poFormat)
{	
	PRINT(("MotionBlurFilter::FormatSuggestionRequested()\n"));
...
status_t MotionBlurFilter::GetLatency(bigtime_t* poLatency)
{	
	PRINT(("MotionBlurFilter::GetLatency()\n"));
...
status_t MotionBlurFilter::GetNextOutput(int32* pioCookie, media_output* poOutput)
{
	if (*pioCookie != 0)
		return B_BAD_INDEX;
...
void MotionBlurFilter::LatencyChanged(
	const media_source& source,
	const media_destination& destination,
	bigtime_t newLatency,
	uint32 flags)
{
	PRINT(("MotionBlurFilter::LatencyChanged()\n"));
...
void MotionBlurFilter::LateNoticeReceived(
	const media_source& source,
	bigtime_t howLate,
	bigtime_t tpWhen)
{
	PRINT(("MotionBlurFilter::LateNoticeReceived()\n"
		"\thowLate == %Ld\n"
		"\twhen    == %Ld\n", howLate, tpWhen));
...
status_t MotionBlurFilter::PrepareToConnect(
	const media_source& source,
	const media_destination& destination,
	media_format* pioFormat,
	...
	char* poName)
{
	...
	PRINT(("MotionBlurFilter::PrepareToConnect()\n"
		"\tproposed format: %s\n", formatStr));
...
status_t MotionBlurFilter::SetBufferGroup(
	const media_source& source,
	BBufferGroup* pGroup)
{
	PRINT(("MotionBlurFilter::SetBufferGroup()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tBad source.\n"));
		return B_MEDIA_BAD_SOURCE;
...
status_t MotionBlurFilter::SetPlayRate(int32 numerator, int32 denominator)
{
	// not supported
	return B_ERROR;
}
	
status_t MotionBlurFilter::VideoClippingChanged(
	const media_source& source,
	int16 numShorts,
	int16* pClipData,
...
status_t MotionBlurFilter::GetParameterValue(int32 id, bigtime_t* last_change, void* value, size_t* size)
{
	switch (id)
	{
		default:
	...
	*last_change = fLastImpactChange;
	*size = sizeof(float);

	return B_OK;
}
		
void MotionBlurFilter::SetParameterValue(int32 id, bigtime_t when, const void *value,	size_t size)
{
	float	tmp;
...
void MotionBlurFilter::handleStartEvent(
	const media_timed_event* pEvent) {
	PRINT(("MotionBlurFilter::handleStartEvent\n"));

	startFilter();	
}
		
void MotionBlurFilter::handleStopEvent(
	const media_timed_event* pEvent) {
	PRINT(("MotionBlurFilter::handleStopEvent\n"));
	
	stopFilter();
}
		
void MotionBlurFilter::ignoreEvent(const media_timed_event* pEvent)
{
	PRINT(("MotionBlurFilter::ignoreEvent\n"));	
}
...
//void MotionBlurFilter::specializeOutputFormat(media_format& ioFormat)
//{
//	char formatStr[256];
//	string_for_format(ioFormat, formatStr, 255);
//	PRINT(("MotionBlurFilter::specializeOutputFormat()\n"
//		"\tinput format: %s\n", formatStr));
//	
...
void MotionBlurFilter::initParameterValues() {
	m_fMixRatio = 0.5;
...
//void MotionBlurFilter::initParameterWeb() {
//	BParameterWeb* pWeb = new BParameterWeb();
//	BParameterGroup* pTopGroup = pWeb->MakeGroup("MotionBlurFilter Parameters");
//
...
void MotionBlurFilter::initFilter() {
	PRINT(("MotionBlurFilter::initFilter()\n"));
...
void MotionBlurFilter::startFilter() {
	PRINT(("MotionBlurFilter::startFilter()\n"));
}
void MotionBlurFilter::stopFilter() {
	PRINT(("MotionBlurFilter::stopFilter()\n"));
}

// figure processing latency by doing 'dry runs' of filterBuffer()
bigtime_t MotionBlurFilter::calcProcessingLatency() {
	PRINT(("MotionBlurFilter::calcProcessingLatency()\n"));
...
void MotionBlurFilter::filterBuffer(BBuffer* inBuffer)
{
	if (!inBuffer) 
		return;
...
/* Sans BBitmap  */
	...
	if (!tmp_buffer){
		...
		memcpy(tmp_buffer, inData, inHeader->size_used);
	}
	...
	while (po < inData + inHeader->size_used / 4){
		new_in = *po;
		old_in = *t++;
		r = impact*((old_in&0x00ff0000)>>16)+(100-impact)*((new_in&0x00ff0000)>>16);
		g = impact*((old_in&0x0000ff00)>>8)+(100-impact)*((new_in&0x0000ff00)>>8);
		b = impact*(old_in&0x000000ff)+(100-impact)*(new_in&0x000000ff);
		r/=100;
		g/=100;
		b/=100;
		*po++=(r<<16)+(g<<8)+b;
