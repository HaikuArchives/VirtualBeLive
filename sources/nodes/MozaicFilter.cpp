...
const char* const MozaicFilter::s_nodeName = "MozaicFilter";
...
MozaicFilter::MozaicFilter(BMediaAddOn* pAddOn) :
	// * init base classes
...
{	
	...
	BContinuousParameter *sqsize = main->MakeContinuousParameter(P_SQUARE, B_MEDIA_RAW_VIDEO, "Square Size", "MozaicBalance", "", 1, 100.0, 1.0);
	
	SQ_SIZE=1;
	fLastMozaicChange = system_time();
...
status_t MozaicFilter::HandleMessage(
	int32 code,
	const void* pData,
	size_t size) {
...
BMediaAddOn* MozaicFilter::AddOn(
	int32* poID) const {
...
void MozaicFilter::HandleEvent(const media_timed_event* pEvent, bigtime_t howLate, bool realTimeEvent)
{
	ASSERT(pEvent);
...
void MozaicFilter::NodeRegistered() {

	PRINT(("MozaicFilter::NodeRegistered()\n"));
...
bigtime_t MozaicFilter::OfflineTime()
{
...
status_t MozaicFilter::AcceptFormat(const media_destination& destination, media_format* format) 
{
...
void MozaicFilter::BufferReceived(BBuffer* pBuffer)
{
	ASSERT(pBuffer);
	...
	{
		PRINT(("MozaicFilter::BufferReceived():\n"
			"\tBad destination.\n"));
		pBuffer->Recycle();
		return;
	}
...
status_t MozaicFilter::Connected(
	const media_source& source,
	const media_destination& destination,
	const media_format& format,
	media_input* poInput) {
	
	PRINT(("MozaicFilter::Connected()\n""\tto source %ld\n", source.id));
...
void MozaicFilter::Disconnected(const media_source& source, const media_destination& destination)
{	
	PRINT(("MozaicFilter::Disconnected()\n"));
...
void MozaicFilter::DisposeInputCookie(int32 cookie)
{
...
status_t MozaicFilter::FormatChanged(
	const media_source& source,
	const media_destination& destination,
	int32 changeTag,
...
status_t MozaicFilter::GetLatencyFor(
	const media_destination& destination,
	bigtime_t* poLatency,
	media_node_id* poTimeSource) {
	
	PRINT(("MozaicFilter::GetLatencyFor()\n"));
...
status_t MozaicFilter::GetNextInput(int32* pioCookie, media_input* poInput)
{
	if(*pioCookie != 0)
		return B_BAD_INDEX;
...
void MozaicFilter::ProducerDataStatus(const media_destination& destination, int32 status, bigtime_t tpWhen)
{
	PRINT(("MozaicFilter::ProducerDataStatus()\n"));
...
status_t MozaicFilter::SeekTagRequested(
	const media_destination& destination,
	...
	uint32* poFlags)
{
	...
	PRINT(("MozaicFilter::SeekTagRequested()\n"
		"\tNot implemented.\n"));
	return B_ERROR;
}
...
void MozaicFilter::AdditionalBufferRequested(
	const media_source& source,
...
//	additionalbufferrequested = true;
	PRINT(("MozaicFilter::AdditionalBufferRequested\n"
		"\tOffline mode not implemented."));
}
		
void MozaicFilter::Connect(status_t status, const media_source& source, const media_destination& destination, const media_format& format,	char* pioName)
{
	
	PRINT(("MozaicFilter::Connect()\n"));
	status_t err;
...
void MozaicFilter::Disconnect(const media_source& source, const media_destination& destination)
{
	PRINT(("MozaicFilter::Disconnect()\n"));
...
status_t MozaicFilter::DisposeOutputCookie(int32 cookie)
{
	return B_OK;
}
		
void MozaicFilter::EnableOutput(
	const media_source& source,
	bool enabled,
	int32* _deprecated_)
{
	PRINT(("MozaicFilter::EnableOutput()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tbad source\n"));
		return;
...
status_t MozaicFilter::FormatChangeRequested(
	const media_source& source,
	const media_destination& destination,
	...
	int32* _deprecated_)
{
	...
	// deny
	PRINT(("MozaicFilter::FormatChangeRequested()\n"
		"\tNot supported.\n"));
		
	return B_MEDIA_BAD_FORMAT;
}
		
status_t MozaicFilter::FormatProposal(
	const media_source& source,
	media_format* pioFormat)
{

	PRINT(("MozaicFilter::FormatProposal()\n"));
...
status_t MozaicFilter::FormatSuggestionRequested(
	media_type type,
	int32 quality,
	media_format* poFormat)
{	
	PRINT(("MozaicFilter::FormatSuggestionRequested()\n"));
...
status_t MozaicFilter::GetLatency(bigtime_t* poLatency)
{	
	PRINT(("MozaicFilter::GetLatency()\n"));
...
status_t MozaicFilter::GetNextOutput(int32* pioCookie, media_output* poOutput)
{
	if (*pioCookie != 0)
		return B_BAD_INDEX;
...
void MozaicFilter::LatencyChanged(
	const media_source& source,
	const media_destination& destination,
	bigtime_t newLatency,
	uint32 flags)
{
	PRINT(("MozaicFilter::LatencyChanged()\n"));
...
void MozaicFilter::LateNoticeReceived(
	const media_source& source,
	bigtime_t howLate,
	bigtime_t tpWhen)
{
	PRINT(("MozaicFilter::LateNoticeReceived()\n"
		"\thowLate == %Ld\n"
		"\twhen    == %Ld\n", howLate, tpWhen));
...
status_t MozaicFilter::PrepareToConnect(
	const media_source& source,
	const media_destination& destination,
	media_format* pioFormat,
	...
	char* poName)
{
	...
	PRINT(("MozaicFilter::PrepareToConnect()\n"
		"\tproposed format: %s\n", formatStr));
...
status_t MozaicFilter::SetBufferGroup(
	const media_source& source,
	BBufferGroup* pGroup)
{
	PRINT(("MozaicFilter::SetBufferGroup()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tBad source.\n"));
		return B_MEDIA_BAD_SOURCE;
...
status_t MozaicFilter::SetPlayRate(int32 numerator, int32 denominator)
{
	// not supported
	return B_ERROR;
}
	
status_t MozaicFilter::VideoClippingChanged(
	const media_source& source,
	int16 numShorts,
	int16* pClipData,
...
status_t MozaicFilter::GetParameterValue(int32 id, bigtime_t* last_change, void* value, size_t* size)
{
	switch (id)
	{
		default:
	...
	*last_change = fLastMozaicChange;
	*size = sizeof(float);

	return B_OK;
}
		
void MozaicFilter::SetParameterValue(int32 id, bigtime_t when, const void *value,	size_t size)
{
	float	tmp;
	...
	switch (id)
	{
		case P_SQUARE:
			tmp = *((float*)value);
			SQ_SIZE = (int32)tmp;
			BroadcastNewParameterValue(fLastMozaicChange, id, &SQ_SIZE, sizeof(int32));
			break;
		default:
			break;
	}
	fLastMozaicChange = when;
}
...
void MozaicFilter::handleStartEvent(
	const media_timed_event* pEvent) {
	PRINT(("MozaicFilter::handleStartEvent\n"));

	startFilter();	
}
		
void MozaicFilter::handleStopEvent(
	const media_timed_event* pEvent) {
	PRINT(("MozaicFilter::handleStopEvent\n"));
	
	stopFilter();
}
		
void MozaicFilter::ignoreEvent(const media_timed_event* pEvent)
{
	PRINT(("MozaicFilter::ignoreEvent\n"));	
}
...
//void MozaicFilter::specializeOutputFormat(media_format& ioFormat)
//{
//	char formatStr[256];
//	string_for_format(ioFormat, formatStr, 255);
//	PRINT(("MozaicFilter::specializeOutputFormat()\n"
//		"\tinput format: %s\n", formatStr));
//	
...
void MozaicFilter::initParameterValues() {
	m_fMixRatio = 0.5;
...
//void MozaicFilter::initParameterWeb() {
//	BParameterWeb* pWeb = new BParameterWeb();
//	BParameterGroup* pTopGroup = pWeb->MakeGroup("MozaicFilter Parameters");
//
...
void MozaicFilter::initFilter() {
	PRINT(("MozaicFilter::initFilter()\n"));
...
void MozaicFilter::startFilter() {
	PRINT(("MozaicFilter::startFilter()\n"));
}
void MozaicFilter::stopFilter() {
	PRINT(("MozaicFilter::stopFilter()\n"));
}

// figure processing latency by doing 'dry runs' of filterBuffer()
bigtime_t MozaicFilter::calcProcessingLatency() {
	PRINT(("MozaicFilter::calcProcessingLatency()\n"));
...
void MozaicFilter::filterBuffer(BBuffer* inBuffer)
{
	if (!inBuffer) 
		return;
...
/* Sans BBitmap  */
	...
	col=m_format.u.raw_video.display.line_width;
	lin=m_format.u.raw_video.display.line_count;
	
	for (i=0;i<lin*col;i++)
		po[i]=po[SQ_SIZE*(((i%col)/SQ_SIZE)+((i/col)/SQ_SIZE)*col)];

// Fin Sans Bitmap	
}

// END -- MozaicFilter.cpp --
