...
const char* const OffsetFilter::s_nodeName = "OffsetFilter";
...
status_t OffsetFilter::HandleMessage(
	int32 code,
	const void* pData,
	size_t size) {
...
BMediaAddOn* OffsetFilter::AddOn(
	int32* poID) const {
...
void OffsetFilter::HandleEvent(const media_timed_event* pEvent, bigtime_t howLate, bool realTimeEvent)
{
	ASSERT(pEvent);
...
void OffsetFilter::NodeRegistered() {

	PRINT(("OffsetFilter::NodeRegistered()\n"));
...
bigtime_t OffsetFilter::OfflineTime()
{
...
status_t OffsetFilter::AcceptFormat(const media_destination& destination, media_format* format) 
{
...
void OffsetFilter::BufferReceived(BBuffer* pBuffer)
{
	ASSERT(pBuffer);
	...
	{
		PRINT(("OffsetFilter::BufferReceived():\n"
			"\tBad destination.\n"));
		pBuffer->Recycle();
		return;
	}
...
status_t OffsetFilter::Connected(
	const media_source& source,
	const media_destination& destination,
	const media_format& format,
	media_input* poInput) {
	
	PRINT(("OffsetFilter::Connected()\n""\tto source %ld\n", source.id));
...
void OffsetFilter::Disconnected(const media_source& source, const media_destination& destination)
{	
	PRINT(("OffsetFilter::Disconnected()\n"));
...
void OffsetFilter::DisposeInputCookie(int32 cookie)
{
...
status_t OffsetFilter::FormatChanged(
	const media_source& source,
	const media_destination& destination,
	int32 changeTag,
...
status_t OffsetFilter::GetLatencyFor(
	const media_destination& destination,
	bigtime_t* poLatency,
	media_node_id* poTimeSource) {
	
	PRINT(("OffsetFilter::GetLatencyFor()\n"));
...
status_t OffsetFilter::GetNextInput(int32* pioCookie, media_input* poInput)
{
	if(*pioCookie != 0)
		return B_BAD_INDEX;
...
void OffsetFilter::ProducerDataStatus(const media_destination& destination, int32 status, bigtime_t tpWhen)
{
	PRINT(("OffsetFilter::ProducerDataStatus()\n"));
...
status_t OffsetFilter::SeekTagRequested(
	const media_destination& destination,
	...
	uint32* poFlags)
{
	...
	PRINT(("OffsetFilter::SeekTagRequested()\n"
		"\tNot implemented.\n"));
	return B_ERROR;
}
...
void OffsetFilter::AdditionalBufferRequested(
	const media_source& source,
...
//	additionalbufferrequested = true;
	PRINT(("OffsetFilter::AdditionalBufferRequested\n"
		"\tOffline mode not implemented."));
}
		
void OffsetFilter::Connect(status_t status, const media_source& source, const media_destination& destination, const media_format& format,	char* pioName)
{
	
	PRINT(("OffsetFilter::Connect()\n"));
	status_t err;
...
void OffsetFilter::Disconnect(const media_source& source, const media_destination& destination)
{
	PRINT(("OffsetFilter::Disconnect()\n"));
...
status_t OffsetFilter::DisposeOutputCookie(int32 cookie)
{
	return B_OK;
}
		
void OffsetFilter::EnableOutput(
	const media_source& source,
	bool enabled,
	int32* _deprecated_)
{
	PRINT(("OffsetFilter::EnableOutput()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tbad source\n"));
		return;
...
status_t OffsetFilter::FormatChangeRequested(
	const media_source& source,
	const media_destination& destination,
	...
	int32* _deprecated_)
{
	...
	// deny
	PRINT(("OffsetFilter::FormatChangeRequested()\n"
		"\tNot supported.\n"));
		
	return B_MEDIA_BAD_FORMAT;
}
		
status_t OffsetFilter::FormatProposal(
	const media_source& source,
	media_format* pioFormat)
{

	PRINT(("OffsetFilter::FormatProposal()\n"));
...
status_t OffsetFilter::FormatSuggestionRequested(
	media_type type,
	int32 quality,
	media_format* poFormat)
{	
	PRINT(("OffsetFilter::FormatSuggestionRequested()\n"));
...
status_t OffsetFilter::GetNextOutput(int32* pioCookie, media_output* poOutput)
{
	if (*pioCookie != 0)
		return B_BAD_INDEX;
...
void OffsetFilter::LatencyChanged(
	const media_source& source,
	const media_destination& destination,
	bigtime_t newLatency,
	uint32 flags)
{
	PRINT(("OffsetFilter::LatencyChanged()\n"));
...
void OffsetFilter::LateNoticeReceived(
	const media_source& source,
	bigtime_t howLate,
	bigtime_t tpWhen)
{
	PRINT(("OffsetFilter::LateNoticeReceived()\n"
		"\thowLate == %Ld\n"
		"\twhen    == %Ld\n", howLate, tpWhen));
...
status_t OffsetFilter::PrepareToConnect(
	const media_source& source,
	const media_destination& destination,
	media_format* pioFormat,
	...
	char* poName)
{
	...
	PRINT(("OffsetFilter::PrepareToConnect()\n"
		"\tproposed format: %s\n", formatStr));
...
status_t OffsetFilter::SetBufferGroup(
	const media_source& source,
	BBufferGroup* pGroup)
{
	PRINT(("OffsetFilter::SetBufferGroup()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tBad source.\n"));
		return B_MEDIA_BAD_SOURCE;
...
status_t OffsetFilter::SetPlayRate(int32 numerator, int32 denominator)
{
	// not supported
	return B_ERROR;
}
	
status_t OffsetFilter::VideoClippingChanged(
	const media_source& source,
	int16 numShorts,
	int16* pClipData,
...
status_t OffsetFilter::GetParameterValue(int32 id, bigtime_t* last_change, void* value, size_t* size)
{
	switch (id)
	{
		default:
...
void OffsetFilter::ignoreEvent(const media_timed_event* pEvent)
{
	PRINT(("OffsetFilter::ignoreEvent\n"));	
}
...
//void OffsetFilter::specializeOutputFormat(media_format& ioFormat)
//{
//	char formatStr[256];
//	string_for_format(ioFormat, formatStr, 255);
//	PRINT(("OffsetFilter::specializeOutputFormat()\n"
//		"\tinput format: %s\n", formatStr));
//	
...
void OffsetFilter::initParameterValues() {
	m_fMixRatio = 0.5;
...
//void OffsetFilter::initParameterWeb() {
//	BParameterWeb* pWeb = new BParameterWeb();
//	BParameterGroup* pTopGroup = pWeb->MakeGroup("OffsetFilter Parameters");
//
...
void OffsetFilter::initFilter() {
	PRINT(("OffsetFilter::initFilter()\n"));
...
void OffsetFilter::startFilter() {
	PRINT(("OffsetFilter::startFilter()\n"));
}
void OffsetFilter::stopFilter() {
	PRINT(("OffsetFilter::stopFilter()\n"));
}

// figure processing latency by doing 'dry runs' of filterBuffer()
bigtime_t OffsetFilter::calcProcessingLatency() {
	PRINT(("OffsetFilter::calcProcessingLatency()\n"));
