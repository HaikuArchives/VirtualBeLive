...
const char* const SolarizeFilter::s_nodeName = "SolarizeFilter";
...
SolarizeFilter::SolarizeFilter(BMediaAddOn* pAddOn) :
	// * init base classes
...
{	
	...
	BContinuousParameter *pthreshold = main->MakeContinuousParameter(P_THRESHOLD, B_MEDIA_RAW_VIDEO, "Threshold", "SolarBalance", "", 0.0, 255.0, 1.0);
	BContinuousParameter *pmode = main->MakeContinuousParameter(P_MODE, B_MEDIA_RAW_VIDEO, "Mode", "SolarBalance", "", 1.0, 2.0, 1.0);
	
	threshold=128;
	mode=1;
	fLastSolarChange = system_time();
...
status_t SolarizeFilter::HandleMessage(
	int32 code,
	const void* pData,
	size_t size) {
...
BMediaAddOn* SolarizeFilter::AddOn(
	int32* poID) const {
...
void SolarizeFilter::SetRunMode(
	run_mode mode) {
...
void SolarizeFilter::HandleEvent(const media_timed_event* pEvent, bigtime_t howLate, bool realTimeEvent)
{
	ASSERT(pEvent);
...
void SolarizeFilter::NodeRegistered() {

	PRINT(("SolarizeFilter::NodeRegistered()\n"));
...
bigtime_t SolarizeFilter::OfflineTime()
{
...
status_t SolarizeFilter::AcceptFormat(const media_destination& destination, media_format* format) 
{
...
void SolarizeFilter::BufferReceived(BBuffer* pBuffer)
{
	ASSERT(pBuffer);
	...
	{
		PRINT(("SolarizeFilter::BufferReceived():\n"
			"\tBad destination.\n"));
		pBuffer->Recycle();
		return;
	}
...
status_t SolarizeFilter::Connected(
	const media_source& source,
	const media_destination& destination,
	const media_format& format,
	media_input* poInput) {
	
	PRINT(("SolarizeFilter::Connected()\n""\tto source %ld\n", source.id));
...
void SolarizeFilter::Disconnected(const media_source& source, const media_destination& destination)
{	
	PRINT(("SolarizeFilter::Disconnected()\n"));
...
void SolarizeFilter::DisposeInputCookie(int32 cookie)
{
...
status_t SolarizeFilter::FormatChanged(
	const media_source& source,
	const media_destination& destination,
	int32 changeTag,
...
status_t SolarizeFilter::GetLatencyFor(
	const media_destination& destination,
	bigtime_t* poLatency,
	media_node_id* poTimeSource) {
	
	PRINT(("SolarizeFilter::GetLatencyFor()\n"));
...
status_t SolarizeFilter::GetNextInput(int32* pioCookie, media_input* poInput)
{
	if(*pioCookie != 0)
		return B_BAD_INDEX;
...
void SolarizeFilter::ProducerDataStatus(const media_destination& destination, int32 status, bigtime_t tpWhen)
{
	PRINT(("SolarizeFilter::ProducerDataStatus()\n"));
...
status_t SolarizeFilter::SeekTagRequested(
	const media_destination& destination,
	...
	uint32* poFlags)
{
	...
	PRINT(("SolarizeFilter::SeekTagRequested()\n"
		"\tNot implemented.\n"));
	return B_ERROR;
}
...
void SolarizeFilter::AdditionalBufferRequested(
	const media_source& source,
...
//	additionalbufferrequested = true;
	PRINT(("SolarizeFilter::AdditionalBufferRequested\n"
		"\tOffline mode not implemented."));
}
		
void SolarizeFilter::Connect(status_t status, const media_source& source, const media_destination& destination, const media_format& format,	char* pioName)
{
	
	PRINT(("SolarizeFilter::Connect()\n"));
	status_t err;
...
void SolarizeFilter::Disconnect(const media_source& source, const media_destination& destination)
{
	PRINT(("SolarizeFilter::Disconnect()\n"));
...
status_t SolarizeFilter::DisposeOutputCookie(int32 cookie)
{
	return B_OK;
}
		
void SolarizeFilter::EnableOutput(
	const media_source& source,
	bool enabled,
	int32* _deprecated_)
{
	PRINT(("SolarizeFilter::EnableOutput()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tbad source\n"));
		return;
...
status_t SolarizeFilter::FormatChangeRequested(
	const media_source& source,
	const media_destination& destination,
	...
	int32* _deprecated_)
{
	...
	// deny
	PRINT(("SolarizeFilter::FormatChangeRequested()\n"
		"\tNot supported.\n"));
		
	return B_MEDIA_BAD_FORMAT;
}
		
status_t SolarizeFilter::FormatProposal(
	const media_source& source,
	media_format* pioFormat)
{

	PRINT(("SolarizeFilter::FormatProposal()\n"));
...
status_t SolarizeFilter::FormatSuggestionRequested(
	media_type type,
	int32 quality,
	media_format* poFormat)
{	
	PRINT(("SolarizeFilter::FormatSuggestionRequested()\n"));
...
status_t SolarizeFilter::GetLatency(bigtime_t* poLatency)
{	
	PRINT(("SolarizeFilter::GetLatency()\n"));
...
status_t SolarizeFilter::GetNextOutput(int32* pioCookie, media_output* poOutput)
{
	if (*pioCookie != 0)
		return B_BAD_INDEX;
...
void SolarizeFilter::LatencyChanged(
	const media_source& source,
	const media_destination& destination,
	bigtime_t newLatency,
	uint32 flags)
{
	PRINT(("SolarizeFilter::LatencyChanged()\n"));
...
void SolarizeFilter::LateNoticeReceived(
	const media_source& source,
	bigtime_t howLate,
	bigtime_t tpWhen)
{
	PRINT(("SolarizeFilter::LateNoticeReceived()\n"
		"\thowLate == %Ld\n"
		"\twhen    == %Ld\n", howLate, tpWhen));
...
status_t SolarizeFilter::PrepareToConnect(
	const media_source& source,
	const media_destination& destination,
	media_format* pioFormat,
	...
	char* poName)
{
	...
	PRINT(("SolarizeFilter::PrepareToConnect()\n"
		"\tproposed format: %s\n", formatStr));
...
status_t SolarizeFilter::SetBufferGroup(
	const media_source& source,
	BBufferGroup* pGroup)
{
	PRINT(("SolarizeFilter::SetBufferGroup()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tBad source.\n"));
		return B_MEDIA_BAD_SOURCE;
...
status_t SolarizeFilter::SetPlayRate(int32 numerator, int32 denominator)
{
	// not supported
	return B_ERROR;
}
	
status_t SolarizeFilter::VideoClippingChanged(
	const media_source& source,
	int16 numShorts,
	int16* pClipData,
...
status_t SolarizeFilter::GetParameterValue(int32 id, bigtime_t* last_change, void* value, size_t* size)
{
	switch (id)
	{
		default:
	...
	*last_change = fLastSolarChange;
	*size = sizeof(float);

	return B_OK;
}
		
void SolarizeFilter::SetParameterValue(int32 id, bigtime_t when, const void *value,	size_t size)
{
	float	tmp;
	...
	switch (id)
	{
		case P_THRESHOLD:
			tmp = *((float*)value);
			threshold = (int32)tmp;
			BroadcastNewParameterValue(fLastSolarChange, id, &threshold, sizeof(int32));
			break;
		case P_MODE:
			tmp = *((float*)value);
			mode = (int32)tmp;
			BroadcastNewParameterValue(fLastSolarChange, id, &mode, sizeof(int32));
			break;
		default:
			break;
	}
	fLastSolarChange = when;
}
...
void SolarizeFilter::handleStartEvent(
	const media_timed_event* pEvent) {
	PRINT(("SolarizeFilter::handleStartEvent\n"));

	startFilter();	
}
		
void SolarizeFilter::handleStopEvent(
	const media_timed_event* pEvent) {
	PRINT(("SolarizeFilter::handleStopEvent\n"));
	
	stopFilter();
}
		
void SolarizeFilter::ignoreEvent(const media_timed_event* pEvent)
{
	PRINT(("SolarizeFilter::ignoreEvent\n"));	
}
...
//void SolarizeFilter::specializeOutputFormat(media_format& ioFormat)
//{
//	char formatStr[256];
//	string_for_format(ioFormat, formatStr, 255);
//	PRINT(("SolarizeFilter::specializeOutputFormat()\n"
//		"\tinput format: %s\n", formatStr));
//	
...
void SolarizeFilter::initParameterValues() {
	m_fMixRatio = 0.5;
...
//void SolarizeFilter::initParameterWeb() {
//	BParameterWeb* pWeb = new BParameterWeb();
//	BParameterGroup* pTopGroup = pWeb->MakeGroup("SolarizeFilter Parameters");
//
...
void SolarizeFilter::initFilter() {
	PRINT(("SolarizeFilter::initFilter()\n"));
...
void SolarizeFilter::startFilter() {
	PRINT(("SolarizeFilter::startFilter()\n"));
}
void SolarizeFilter::stopFilter() {
	PRINT(("SolarizeFilter::stopFilter()\n"));
}

// figure processing latency by doing 'dry runs' of filterBuffer()
bigtime_t SolarizeFilter::calcProcessingLatency() {
	PRINT(("SolarizeFilter::calcProcessingLatency()\n"));
...
void SolarizeFilter::filterBuffer(BBuffer* inBuffer)
{
	if (!inBuffer) 
		return;
	...
	switch (mode){
		...
		case 2:
			while (po < inData + inHeader->size_used/4){
				...
				if (g>threshold) r=255-g;
				if (b>threshold) r=255-b;
				*po++=(r<<16)+(g<<8)+b;
			}
			break;
	}
