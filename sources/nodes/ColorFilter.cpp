...
const char* const ColorFilter::s_nodeName = "ColorFilter";
...
ColorFilter::ColorFilter(BMediaAddOn* pAddOn) :
	// * init base classes
...
status_t ColorFilter::HandleMessage(
	int32 code,
	const void* pData,
	size_t size) {
...
void ColorFilter::HandleEvent(const media_timed_event* pEvent, bigtime_t howLate, bool realTimeEvent)
{
	ASSERT(pEvent);
...
void ColorFilter::NodeRegistered() {

	PRINT(("ColorFilter::NodeRegistered()\n"));
...
bigtime_t ColorFilter::OfflineTime()
{
...
status_t ColorFilter::AcceptFormat(const media_destination& destination, media_format* format) 
{
...
void ColorFilter::BufferReceived(BBuffer* pBuffer)
{
	ASSERT(pBuffer);
	...
	{
		PRINT(("ColorFilter::BufferReceived():\n"
			"\tBad destination.\n"));
		pBuffer->Recycle();
		return;
	}
...
status_t ColorFilter::Connected(
	const media_source& source,
	const media_destination& destination,
	const media_format& format,
	media_input* poInput) {
	
	PRINT(("ColorFilter::Connected()\n""\tto source %ld\n", source.id));
...
void ColorFilter::Disconnected(const media_source& source, const media_destination& destination)
{	
	PRINT(("ColorFilter::Disconnected()\n"));
...
void ColorFilter::DisposeInputCookie(int32 cookie)
{
...
status_t ColorFilter::FormatChanged(
	const media_source& source,
	const media_destination& destination,
	int32 changeTag,
...
status_t ColorFilter::GetLatencyFor(
	const media_destination& destination,
	bigtime_t* poLatency,
	media_node_id* poTimeSource) {
	
	PRINT(("ColorFilter::GetLatencyFor()\n"));
...
status_t ColorFilter::GetNextInput(int32* pioCookie, media_input* poInput)
{
	if(*pioCookie != 0)
		return B_BAD_INDEX;
...
void ColorFilter::ProducerDataStatus(const media_destination& destination, int32 status, bigtime_t tpWhen)
{
	PRINT(("ColorFilter::ProducerDataStatus()\n"));
...
status_t ColorFilter::SeekTagRequested(
	const media_destination& destination,
	...
	uint32* poFlags)
{
	...
	PRINT(("ColorFilter::SeekTagRequested()\n"
		"\tNot implemented.\n"));
	return B_ERROR;
}
...
void ColorFilter::AdditionalBufferRequested(
	const media_source& source,
...
//	additionalbufferrequested = true;
	PRINT(("ColorFilter::AdditionalBufferRequested\n"
		"\tOffline mode not implemented."));
}
		
void ColorFilter::Connect(status_t status, const media_source& source, const media_destination& destination, const media_format& format,	char* pioName)
{
	
	PRINT(("ColorFilter::Connect()\n"));
	status_t err;
...
void ColorFilter::Disconnect(const media_source& source, const media_destination& destination)
{
	PRINT(("ColorFilter::Disconnect()\n"));
...
status_t ColorFilter::DisposeOutputCookie(int32 cookie)
{
	return B_OK;
}
		
void ColorFilter::EnableOutput(
	const media_source& source,
	bool enabled,
	int32* _deprecated_)
{
	PRINT(("ColorFilter::EnableOutput()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tbad source\n"));
		return;
...
status_t ColorFilter::FormatChangeRequested(
	const media_source& source,
	const media_destination& destination,
	...
	int32* _deprecated_)
{
	...
	// deny
	PRINT(("ColorFilter::FormatChangeRequested()\n"
		"\tNot supported.\n"));
		
	return B_MEDIA_BAD_FORMAT;
}
		
status_t ColorFilter::FormatProposal(
	const media_source& source,
	media_format* pioFormat)
{

	PRINT(("ColorFilter::FormatProposal()\n"));
...
status_t ColorFilter::FormatSuggestionRequested(
	media_type type,
	int32 quality,
	media_format* poFormat)
{	
	PRINT(("ColorFilter::FormatSuggestionRequested()\n"));
...
status_t ColorFilter::GetLatency(bigtime_t* poLatency)
{	
	PRINT(("ColorFilter::GetLatency()\n"));
...
status_t ColorFilter::GetNextOutput(int32* pioCookie, media_output* poOutput)
{
	if (*pioCookie != 0)
		return B_BAD_INDEX;
...
void ColorFilter::LatencyChanged(
	const media_source& source,
	const media_destination& destination,
	bigtime_t newLatency,
	uint32 flags)
{
	PRINT(("ColorFilter::LatencyChanged()\n"));
...
void ColorFilter::LateNoticeReceived(
	const media_source& source,
	bigtime_t howLate,
	bigtime_t tpWhen)
{
	PRINT(("ColorFilter::LateNoticeReceived()\n"
		"\thowLate == %Ld\n"
		"\twhen    == %Ld\n", howLate, tpWhen));
...
status_t ColorFilter::PrepareToConnect(
	const media_source& source,
	const media_destination& destination,
	media_format* pioFormat,
	...
	char* poName)
{
	...
	PRINT(("ColorFilter::PrepareToConnect()\n"
		"\tproposed format: %s\n", formatStr));
...
status_t ColorFilter::SetBufferGroup(
	const media_source& source,
	BBufferGroup* pGroup)
{
	PRINT(("ColorFilter::SetBufferGroup()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tBad source.\n"));
		return B_MEDIA_BAD_SOURCE;
...
status_t ColorFilter::SetPlayRate(int32 numerator, int32 denominator)
{
	// not supported
	return B_ERROR;
}
	
status_t ColorFilter::VideoClippingChanged(
	const media_source& source,
	int16 numShorts,
	int16* pClipData,
...
status_t ColorFilter::GetParameterValue(int32 id, bigtime_t* last_change, void* value, size_t* size)
{
	if (id != P_COLOR)
		return B_BAD_VALUE;
...
void ColorFilter::SetParameterValue(int32 id, bigtime_t when, const void *value,	size_t size)
{
...
void ColorFilter::handleStartEvent(
	const media_timed_event* pEvent) {
	PRINT(("ColorFilter::handleStartEvent\n"));

	startFilter();	
}
		
void ColorFilter::handleStopEvent(
	const media_timed_event* pEvent) {
	PRINT(("ColorFilter::handleStopEvent\n"));
	
	stopFilter();
}
		
void ColorFilter::ignoreEvent(const media_timed_event* pEvent)
{
	PRINT(("ColorFilter::ignoreEvent\n"));	
}
...
//void ColorFilter::specializeOutputFormat(media_format& ioFormat)
//{
//	char formatStr[256];
//	string_for_format(ioFormat, formatStr, 255);
//	PRINT(("ColorFilter::specializeOutputFormat()\n"
//		"\tinput format: %s\n", formatStr));
//	
...
void ColorFilter::initParameterValues() {
	m_fMixRatio = 0.5;
...
//void ColorFilter::initParameterWeb() {
//	BParameterWeb* pWeb = new BParameterWeb();
//	BParameterGroup* pTopGroup = pWeb->MakeGroup("ColorFilter Parameters");
//
...
void ColorFilter::initFilter() {
	PRINT(("ColorFilter::initFilter()\n"));
...
void ColorFilter::startFilter() {
	PRINT(("ColorFilter::startFilter()\n"));
}
void ColorFilter::stopFilter() {
	PRINT(("ColorFilter::stopFilter()\n"));
}

// figure processing latency by doing 'dry runs' of filterBuffer()
bigtime_t ColorFilter::calcProcessingLatency() {
	PRINT(("ColorFilter::calcProcessingLatency()\n"));
	...
	BBufferGroup* pTestGroup = new BBufferGroup(m_output.format.u.raw_video.display.line_width * m_output.format.u.raw_video.display.line_count, 1);
	
	// fetch a buffer
	BBuffer* pBuffer = pTestGroup->RequestBuffer(m_output.format.u.raw_video.display.line_width * m_output.format.u.raw_video.display.line_count);
	ASSERT(pBuffer);
...
void ColorFilter::filterBuffer(BBuffer* inBuffer)
{
	if (!inBuffer) 
		return;
