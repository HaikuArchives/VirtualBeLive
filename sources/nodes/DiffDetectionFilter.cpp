...
const char* const DiffDetectionFilter::s_nodeName = "DiffDetectionFilter";
...
DiffDetectionFilter::~DiffDetectionFilter() 
{
	// shut down
	Quit();
...
DiffDetectionFilter::DiffDetectionFilter(BMediaAddOn* pAddOn) :
	// * init base classes
...
{	
	...
	BContinuousParameter *pbias = main->MakeContinuousParameter(P_BIAS, B_MEDIA_RAW_VIDEO, "Bias", "BiasBalance", "", 0.0, 255.0, 1.0);
	
	bias=128;
	tmp_buffer=NULL;
	fLastDiffChange = system_time();
...
status_t DiffDetectionFilter::HandleMessage(
	int32 code,
	const void* pData,
	size_t size) {
...
BMediaAddOn* DiffDetectionFilter::AddOn(
	int32* poID) const {
...
void DiffDetectionFilter::SetRunMode(
	run_mode mode) {
...
void DiffDetectionFilter::HandleEvent(const media_timed_event* pEvent, bigtime_t howLate, bool realTimeEvent)
{
	ASSERT(pEvent);
...
void DiffDetectionFilter::NodeRegistered() {

	PRINT(("DiffDetectionFilter::NodeRegistered()\n"));
...
bigtime_t DiffDetectionFilter::OfflineTime()
{
...
status_t DiffDetectionFilter::AcceptFormat(const media_destination& destination, media_format* format) 
{
...
void DiffDetectionFilter::BufferReceived(BBuffer* pBuffer)
{
	ASSERT(pBuffer);
	...
	{
		PRINT(("DiffDetectionFilter::BufferReceived():\n"
			"\tBad destination.\n"));
		pBuffer->Recycle();
		return;
	}
...
status_t DiffDetectionFilter::Connected(
	const media_source& source,
	const media_destination& destination,
	const media_format& format,
	media_input* poInput) {
	
	PRINT(("DiffDetectionFilter::Connected()\n""\tto source %ld\n", source.id));
...
void DiffDetectionFilter::Disconnected(const media_source& source, const media_destination& destination)
{	
	PRINT(("DiffDetectionFilter::Disconnected()\n"));
...
void DiffDetectionFilter::DisposeInputCookie(int32 cookie)
{
...
status_t DiffDetectionFilter::FormatChanged(
	const media_source& source,
	const media_destination& destination,
	int32 changeTag,
...
status_t DiffDetectionFilter::GetLatencyFor(
	const media_destination& destination,
	bigtime_t* poLatency,
	media_node_id* poTimeSource) {
	
	PRINT(("DiffDetectionFilter::GetLatencyFor()\n"));
...
status_t DiffDetectionFilter::GetNextInput(int32* pioCookie, media_input* poInput)
{
	if(*pioCookie != 0)
		return B_BAD_INDEX;
...
void DiffDetectionFilter::ProducerDataStatus(const media_destination& destination, int32 status, bigtime_t tpWhen)
{
	PRINT(("DiffDetectionFilter::ProducerDataStatus()\n"));
...
status_t DiffDetectionFilter::SeekTagRequested(
	const media_destination& destination,
	...
	uint32* poFlags)
{
	...
	PRINT(("DiffDetectionFilter::SeekTagRequested()\n"
		"\tNot implemented.\n"));
	return B_ERROR;
}
...
void DiffDetectionFilter::AdditionalBufferRequested(
	const media_source& source,
...
//	additionalbufferrequested = true;
	PRINT(("DiffDetectionFilter::AdditionalBufferRequested\n"
		"\tOffline mode not implemented."));
}
		
void DiffDetectionFilter::Connect(status_t status, const media_source& source, const media_destination& destination, const media_format& format,	char* pioName)
{
	
	PRINT(("DiffDetectionFilter::Connect()\n"));
	status_t err;
...
void DiffDetectionFilter::Disconnect(const media_source& source, const media_destination& destination)
{
	PRINT(("DiffDetectionFilter::Disconnect()\n"));
...
status_t DiffDetectionFilter::DisposeOutputCookie(int32 cookie)
{
	return B_OK;
}
		
void DiffDetectionFilter::EnableOutput(
	const media_source& source,
	bool enabled,
	int32* _deprecated_)
{
	PRINT(("DiffDetectionFilter::EnableOutput()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tbad source\n"));
		return;
...
status_t DiffDetectionFilter::FormatChangeRequested(
	const media_source& source,
	const media_destination& destination,
	...
	int32* _deprecated_)
{
	...
	// deny
	PRINT(("DiffDetectionFilter::FormatChangeRequested()\n"
		"\tNot supported.\n"));
		
	return B_MEDIA_BAD_FORMAT;
}
		
status_t DiffDetectionFilter::FormatProposal(
	const media_source& source,
	media_format* pioFormat)
{

	PRINT(("DiffDetectionFilter::FormatProposal()\n"));
...
status_t DiffDetectionFilter::FormatSuggestionRequested(
	media_type type,
	int32 quality,
	media_format* poFormat)
{	
	PRINT(("DiffDetectionFilter::FormatSuggestionRequested()\n"));
...
status_t DiffDetectionFilter::GetLatency(bigtime_t* poLatency)
{	
	PRINT(("DiffDetectionFilter::GetLatency()\n"));
...
status_t DiffDetectionFilter::GetNextOutput(int32* pioCookie, media_output* poOutput)
{
	if (*pioCookie != 0)
		return B_BAD_INDEX;
...
void DiffDetectionFilter::LatencyChanged(
	const media_source& source,
	const media_destination& destination,
	bigtime_t newLatency,
	uint32 flags)
{
	PRINT(("DiffDetectionFilter::LatencyChanged()\n"));
...
void DiffDetectionFilter::LateNoticeReceived(
	const media_source& source,
	bigtime_t howLate,
	bigtime_t tpWhen)
{
	PRINT(("DiffDetectionFilter::LateNoticeReceived()\n"
		"\thowLate == %Ld\n"
		"\twhen    == %Ld\n", howLate, tpWhen));
...
status_t DiffDetectionFilter::PrepareToConnect(
	const media_source& source,
	const media_destination& destination,
	media_format* pioFormat,
	...
	char* poName)
{
	...
	PRINT(("DiffDetectionFilter::PrepareToConnect()\n"
		"\tproposed format: %s\n", formatStr));
...
status_t DiffDetectionFilter::SetBufferGroup(
	const media_source& source,
	BBufferGroup* pGroup)
{
	PRINT(("DiffDetectionFilter::SetBufferGroup()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tBad source.\n"));
		return B_MEDIA_BAD_SOURCE;
...
status_t DiffDetectionFilter::SetPlayRate(int32 numerator, int32 denominator)
{
	// not supported
	return B_ERROR;
}
	
status_t DiffDetectionFilter::VideoClippingChanged(
	const media_source& source,
	int16 numShorts,
	int16* pClipData,
...
status_t DiffDetectionFilter::GetParameterValue(int32 id, bigtime_t* last_change, void* value, size_t* size)
{
	switch (id)
	{
		default:
	...
	*last_change = fLastDiffChange;
	*size = sizeof(float);

	return B_OK;
}
		
void DiffDetectionFilter::SetParameterValue(int32 id, bigtime_t when, const void *value,	size_t size)
{
	float	tmp;
	...
	switch (id)
	{
		case P_BIAS:
			tmp = *((float*)value);
			bias = (int32)tmp;
			BroadcastNewParameterValue(fLastDiffChange, id, &bias, sizeof(int32));
			break;
		
		default:
			break;
	}
	fLastDiffChange = when;	
}
...
void DiffDetectionFilter::handleStartEvent(
	const media_timed_event* pEvent) {
	PRINT(("DiffDetectionFilter::handleStartEvent\n"));

	startFilter();	
}
		
void DiffDetectionFilter::handleStopEvent(
	const media_timed_event* pEvent) {
	PRINT(("DiffDetectionFilter::handleStopEvent\n"));
	
	stopFilter();
}
		
void DiffDetectionFilter::ignoreEvent(const media_timed_event* pEvent)
{
	PRINT(("DiffDetectionFilter::ignoreEvent\n"));	
}
...
//void DiffDetectionFilter::specializeOutputFormat(media_format& ioFormat)
//{
//	char formatStr[256];
//	string_for_format(ioFormat, formatStr, 255);
//	PRINT(("DiffDetectionFilter::specializeOutputFormat()\n"
//		"\tinput format: %s\n", formatStr));
//	
...
void DiffDetectionFilter::initParameterValues() {
	m_fMixRatio = 0.5;
...
//void DiffDetectionFilter::initParameterWeb() {
//	BParameterWeb* pWeb = new BParameterWeb();
//	BParameterGroup* pTopGroup = pWeb->MakeGroup("DiffDetectionFilter Parameters");
//
...
void DiffDetectionFilter::initFilter() {
	PRINT(("DiffDetectionFilter::initFilter()\n"));
...
void DiffDetectionFilter::startFilter() {
	PRINT(("DiffDetectionFilter::startFilter()\n"));
}
void DiffDetectionFilter::stopFilter() {
	PRINT(("DiffDetectionFilter::stopFilter()\n"));
}

// figure processing latency by doing 'dry runs' of filterBuffer()
bigtime_t DiffDetectionFilter::calcProcessingLatency() {
	PRINT(("DiffDetectionFilter::calcProcessingLatency()\n"));
...
void DiffDetectionFilter::filterBuffer(BBuffer* inBuffer)
{
	if (!inBuffer) 
		return;
