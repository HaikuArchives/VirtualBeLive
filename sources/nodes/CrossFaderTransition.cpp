...
const char* const CrossFaderTransition::s_nodeName = "CrossFaderTransition";
...
CrossFaderTransition::CrossFaderTransition(BMediaAddOn* pAddOn) :
	// * init base classes
...
status_t CrossFaderTransition::HandleMessage(
	int32 code,
	const void* pData,
	size_t size) {
...
BMediaAddOn* CrossFaderTransition::AddOn(
	int32* poID) const {
...
void CrossFaderTransition::SetRunMode(run_mode mode)
{
...
void CrossFaderTransition::HandleEvent(const media_timed_event* pEvent, bigtime_t howLate, bool realTimeEvent)
{
	ASSERT(pEvent);
...
void CrossFaderTransition::NodeRegistered()
{
	PRINT(("CrossFaderTransition::NodeRegistered()\n"));
...
bigtime_t CrossFaderTransition::OfflineTime()
{
...
status_t CrossFaderTransition::AcceptFormat(const media_destination& destination, media_format* format) 
{
...
void CrossFaderTransition::BufferReceived(BBuffer* pBuffer)
{
	ASSERT(pBuffer);
	...
	{
		PRINT(("CrossFaderTransition::BufferReceived():\n"
			"\tBad destination.\n"));
		pBuffer->Recycle();
		return;
	}
...
status_t CrossFaderTransition::Connected(
	const media_source& source,
	const media_destination& destination,
	const media_format& format,
	media_input* poInput) {
	
	PRINT(("CrossFaderTransition::Connected()\n""\tto source %ld\n", source.id));
...
void CrossFaderTransition::Disconnected(const media_source& source, const media_destination& destination)
{	
	PRINT(("CrossFaderTransition::Disconnected()\n"));
...
void CrossFaderTransition::DisposeInputCookie(int32 cookie)
{
...
status_t CrossFaderTransition::FormatChanged(
	const media_source& source,
	const media_destination& destination,
	int32 changeTag,
...
status_t CrossFaderTransition::GetLatencyFor(
	const media_destination& destination,
	bigtime_t* poLatency,
	media_node_id* poTimeSource) {
	
	PRINT(("CrossFaderTransition::GetLatencyFor()\n"));
...
status_t CrossFaderTransition::GetNextInput(int32* pioCookie, media_input* poInput)
{
	if (*pioCookie > 1)
		return B_BAD_INDEX;
	if (*pioCookie == 0)
...
void CrossFaderTransition::ProducerDataStatus(const media_destination& destination, int32 status, bigtime_t tpWhen)
{
	PRINT(("CrossFaderTransition::ProducerDataStatus()\n"));
...
status_t CrossFaderTransition::SeekTagRequested(
	const media_destination& destination,
	...
	uint32* poFlags)
{
	...
	PRINT(("CrossFaderTransition::SeekTagRequested()\n"
		"\tNot implemented.\n"));
	return B_ERROR;
}
...
void CrossFaderTransition::AdditionalBufferRequested(
	const media_source& source,
...
//	additionalbufferrequested = true;
	PRINT(("CrossFaderTransition::AdditionalBufferRequested\n"));
}
		
void CrossFaderTransition::Connect(status_t status, const media_source& source, const media_destination& destination, const media_format& format,	char* pioName)
{
	
	PRINT(("CrossFaderTransition::Connect()\n"));
	status_t err;
...
void CrossFaderTransition::Disconnect(const media_source& source, const media_destination& destination)
{
	PRINT(("CrossFaderTransition::Disconnect()\n"));
...
status_t CrossFaderTransition::DisposeOutputCookie(int32 cookie)
{
	return B_OK;
}
		
void CrossFaderTransition::EnableOutput(
	const media_source& source,
	bool enabled,
	int32* _deprecated_)
{
	PRINT(("CrossFaderTransition::EnableOutput()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tbad source\n"));
		return;
...
status_t CrossFaderTransition::FormatChangeRequested(
	const media_source& source,
	const media_destination& destination,
	...
	int32* _deprecated_)
{
	...
	// deny
	PRINT(("CrossFaderTransition::FormatChangeRequested()\n"
		"\tNot supported.\n"));
		
	return B_MEDIA_BAD_FORMAT;
}
		
status_t CrossFaderTransition::FormatProposal(
	const media_source& source,
	media_format* pioFormat)
{

	PRINT(("CrossFaderTransition::FormatProposal()\n"));
...
status_t CrossFaderTransition::FormatSuggestionRequested(
	media_type type,
	int32 quality,
	media_format* poFormat)
{	
	PRINT(("CrossFaderTransition::FormatSuggestionRequested()\n"));
...
status_t CrossFaderTransition::GetLatency(bigtime_t* poLatency)
{	
	PRINT(("CrossFaderTransition::GetLatency()\n"));
...
status_t CrossFaderTransition::GetNextOutput(int32* pioCookie, media_output* poOutput)
{
	if (*pioCookie != 0)
		return B_BAD_INDEX;
...
void CrossFaderTransition::LatencyChanged(
	const media_source& source,
	const media_destination& destination,
	bigtime_t newLatency,
	uint32 flags)
{
	PRINT(("CrossFaderTransition::LatencyChanged()\n"));
...
void CrossFaderTransition::LateNoticeReceived(
	const media_source& source,
	bigtime_t howLate,
	bigtime_t tpWhen)
{
	PRINT(("CrossFaderTransition::LateNoticeReceived()\n"
		"\thowLate == %Ld\n"
		"\twhen    == %Ld\n", howLate, tpWhen));
...
status_t CrossFaderTransition::PrepareToConnect(
	const media_source& source,
	const media_destination& destination,
	media_format* pioFormat,
	...
	char* poName)
{
	...
	PRINT(("CrossFaderTransition::PrepareToConnect()\n"
		"\tproposed format: %s\n", formatStr));
...
status_t CrossFaderTransition::SetBufferGroup(const media_source& source, BBufferGroup* pGroup)
{
	PRINT(("CrossFaderTransition::SetBufferGroup()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tBad source.\n"));
		return B_MEDIA_BAD_SOURCE;
...
status_t CrossFaderTransition::SetPlayRate(int32 numerator, int32 denominator)
{
	// not supported
	return B_ERROR;
}
	
status_t CrossFaderTransition::VideoClippingChanged(
	const media_source& source,
	int16 numShorts,
	int16* pClipData,
...
status_t CrossFaderTransition::GetParameterValue(int32 id, bigtime_t* last_change, void* value, size_t* size)
{
	switch (id)
	{
		case P_STATE:
...
void CrossFaderTransition::SetParameterValue(int32 id, bigtime_t when, const void *value,	size_t size)
{
	float	tmp;
...
void CrossFaderTransition::Start(bigtime_t performance_time)
{
	BMediaEventLooper::Start(performance_time);
}

void CrossFaderTransition::Stop(bigtime_t performance_time, bool immediate)
{
	BMediaEventLooper::Stop(performance_time, immediate);
}

void CrossFaderTransition::Seek(bigtime_t media_time, bigtime_t performance_time)
{
	BMediaEventLooper::Seek(media_time, performance_time);
}

void CrossFaderTransition::TimeWarp(bigtime_t at_real_time, bigtime_t to_performance_time)
{
...
void CrossFaderTransition::handleStartEvent(
	const media_timed_event* pEvent) {
	PRINT(("CrossFaderTransition::handleStartEvent\n"));

	startFilter();	
}
		
void CrossFaderTransition::handleStopEvent(
	const media_timed_event* pEvent) {
	PRINT(("CrossFaderTransition::handleStopEvent\n"));
	
	stopFilter();
}
		
void CrossFaderTransition::ignoreEvent(const media_timed_event* pEvent)
{
	PRINT(("CrossFaderTransition::ignoreEvent\n"));	
}
...
//void CrossFaderTransition::specializeOutputFormat(media_format& ioFormat)
//{
//	char formatStr[256];
//	string_for_format(ioFormat, formatStr, 255);
//	PRINT(("CrossFaderTransition::specializeOutputFormat()\n"
//		"\tinput format: %s\n", formatStr));
//	
...
void CrossFaderTransition::initFilter() {
	PRINT(("CrossFaderTransition::initFilter()\n"));
...
void CrossFaderTransition::startFilter() {
	PRINT(("CrossFaderTransition::startFilter()\n"));
}
void CrossFaderTransition::stopFilter() {
	PRINT(("CrossFaderTransition::stopFilter()\n"));
}

// figure processing latency by doing 'dry runs' of filterBuffer()
bigtime_t CrossFaderTransition::calcProcessingLatency() {
	PRINT(("CrossFaderTransition::calcProcessingLatency()\n"));
...
void CrossFaderTransition::MakeTransition(BBuffer* inBuffer, BBuffer *inBuffer2)
{
	status_t	err;
...
//	CALL("now: %Ld start_time: %Ld\n", TimeSource()->Now(), inHeader->start_time);
	...
	/* WORK IT OUT  */
		...
		while ( po < finalData + firstHeader->size_used/4) {
			in1 = *p1++;
			in2 = *p2++;
			r=(((in1&0x00ff0000)>>16)*(100-TState)+((in2&0x00ff0000)>>16)*TState)/100;
			g=(((in1&0x0000ff00)>>8)*(100-TState)+((in2&0x0000ff00)>>8)*TState)/100;
			b=(((in1&0x000000ff))*(100-TState)+((in2&0x000000ff))*TState)/100;
			*po++=(r<<16)+(g<<8)+b;
		}
