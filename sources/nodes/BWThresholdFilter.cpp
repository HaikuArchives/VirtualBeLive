...
const char* const BWThresholdFilter::s_nodeName = "BWThresholdFilter";
...
BWThresholdFilter::~BWThresholdFilter() 
{
	// shut down
	Quit();
...
BWThresholdFilter::BWThresholdFilter(BMediaAddOn* pAddOn) :
	// * init base classes
...
{	
	...
	BContinuousParameter *range = main->MakeContinuousParameter(P_BWTHRESHOLD, B_MEDIA_RAW_VIDEO, "Threshold", "ThresholdBalance", "", 0.0, 255.0, 1.0);
...
status_t BWThresholdFilter::HandleMessage(
	int32 code,
	const void* pData,
	size_t size) {
...
BMediaAddOn* BWThresholdFilter::AddOn(
	int32* poID) const {
...
void BWThresholdFilter::SetRunMode(
	run_mode mode) {
...
void BWThresholdFilter::HandleEvent(const media_timed_event* pEvent, bigtime_t howLate, bool realTimeEvent)
{
	ASSERT(pEvent);
...
void BWThresholdFilter::NodeRegistered() {

	PRINT(("BWThresholdFilter::NodeRegistered()\n"));
...
bigtime_t BWThresholdFilter::OfflineTime()
{
...
status_t BWThresholdFilter::AcceptFormat(const media_destination& destination, media_format* format) 
{
...
void BWThresholdFilter::BufferReceived(BBuffer* pBuffer)
{
	ASSERT(pBuffer);
	...
	{
		PRINT(("BWThresholdFilter::BufferReceived():\n"
			"\tBad destination.\n"));
		pBuffer->Recycle();
		return;
	}
...
status_t BWThresholdFilter::Connected(
	const media_source& source,
	const media_destination& destination,
	const media_format& format,
	media_input* poInput) {
	
	PRINT(("BWThresholdFilter::Connected()\n""\tto source %ld\n", source.id));
...
void BWThresholdFilter::Disconnected(const media_source& source, const media_destination& destination)
{	
	PRINT(("BWThresholdFilter::Disconnected()\n"));
...
void BWThresholdFilter::DisposeInputCookie(int32 cookie)
{
...
status_t BWThresholdFilter::FormatChanged(
	const media_source& source,
	const media_destination& destination,
	int32 changeTag,
...
status_t BWThresholdFilter::GetLatencyFor(
	const media_destination& destination,
	bigtime_t* poLatency,
	media_node_id* poTimeSource) {
	
	PRINT(("BWThresholdFilter::GetLatencyFor()\n"));
...
status_t BWThresholdFilter::GetNextInput(int32* pioCookie, media_input* poInput)
{
	if(*pioCookie != 0)
		return B_BAD_INDEX;
...
void BWThresholdFilter::ProducerDataStatus(const media_destination& destination, int32 status, bigtime_t tpWhen)
{
	PRINT(("BWThresholdFilter::ProducerDataStatus()\n"));
...
status_t BWThresholdFilter::SeekTagRequested(
	const media_destination& destination,
	...
	uint32* poFlags)
{
	...
	PRINT(("BWThresholdFilter::SeekTagRequested()\n"
		"\tNot implemented.\n"));
	return B_ERROR;
}
...
void BWThresholdFilter::AdditionalBufferRequested(
	const media_source& source,
...
//	additionalbufferrequested = true;
	PRINT(("BWThresholdFilter::AdditionalBufferRequested\n"
		"\tOffline mode not implemented."));
}
		
void BWThresholdFilter::Connect(status_t status, const media_source& source, const media_destination& destination, const media_format& format,	char* pioName)
{
	
	PRINT(("BWThresholdFilter::Connect()\n"));
	status_t err;
...
void BWThresholdFilter::Disconnect(const media_source& source, const media_destination& destination)
{
	PRINT(("BWThresholdFilter::Disconnect()\n"));
...
status_t BWThresholdFilter::DisposeOutputCookie(int32 cookie)
{
	return B_OK;
}
		
void BWThresholdFilter::EnableOutput(
	const media_source& source,
	bool enabled,
	int32* _deprecated_)
{
	PRINT(("BWThresholdFilter::EnableOutput()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tbad source\n"));
		return;
...
status_t BWThresholdFilter::FormatChangeRequested(
	const media_source& source,
	const media_destination& destination,
	...
	int32* _deprecated_)
{
	...
	// deny
	PRINT(("BWThresholdFilter::FormatChangeRequested()\n"
		"\tNot supported.\n"));
		
	return B_MEDIA_BAD_FORMAT;
}
		
status_t BWThresholdFilter::FormatProposal(
	const media_source& source,
	media_format* pioFormat)
{

	PRINT(("BWThresholdFilter::FormatProposal()\n"));
...
status_t BWThresholdFilter::FormatSuggestionRequested(
	media_type type,
	int32 quality,
	media_format* poFormat)
{	
	PRINT(("BWThresholdFilter::FormatSuggestionRequested()\n"));
...
status_t BWThresholdFilter::GetLatency(bigtime_t* poLatency)
{	
	PRINT(("BWThresholdFilter::GetLatency()\n"));
...
status_t BWThresholdFilter::GetNextOutput(int32* pioCookie, media_output* poOutput)
{
	if (*pioCookie != 0)
		return B_BAD_INDEX;
...
void BWThresholdFilter::LatencyChanged(
	const media_source& source,
	const media_destination& destination,
	bigtime_t newLatency,
	uint32 flags)
{
	PRINT(("BWThresholdFilter::LatencyChanged()\n"));
...
void BWThresholdFilter::LateNoticeReceived(
	const media_source& source,
	bigtime_t howLate,
	bigtime_t tpWhen)
{
	PRINT(("BWThresholdFilter::LateNoticeReceived()\n"
		"\thowLate == %Ld\n"
		"\twhen    == %Ld\n", howLate, tpWhen));
...
status_t BWThresholdFilter::PrepareToConnect(
	const media_source& source,
	const media_destination& destination,
	media_format* pioFormat,
	...
	char* poName)
{
	...
	PRINT(("BWThresholdFilter::PrepareToConnect()\n"
		"\tproposed format: %s\n", formatStr));
...
status_t BWThresholdFilter::SetBufferGroup(
	const media_source& source,
	BBufferGroup* pGroup)
{
	PRINT(("BWThresholdFilter::SetBufferGroup()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tBad source.\n"));
		return B_MEDIA_BAD_SOURCE;
...
status_t BWThresholdFilter::SetPlayRate(int32 numerator, int32 denominator)
{
	// not supported
	return B_ERROR;
}
	
status_t BWThresholdFilter::VideoClippingChanged(
	const media_source& source,
	int16 numShorts,
	int16* pClipData,
...
status_t BWThresholdFilter::GetParameterValue(int32 id, bigtime_t* last_change, void* value, size_t* size)
{
	switch (id)
	{
		case P_BWTHRESHOLD:
...
void BWThresholdFilter::SetParameterValue(int32 id, bigtime_t when, const void *value,	size_t size)
{
	float	tmp;
	...
	switch (id)
	{
		case P_BWTHRESHOLD:
			tmp = *((float*)value);
			bwthreshold = (uint32)tmp;
			BroadcastNewParameterValue(fLastthresholdChange, id, &bwthreshold, sizeof(uint32));
			break;
...
void BWThresholdFilter::handleStartEvent(
	const media_timed_event* pEvent) {
	PRINT(("BWThresholdFilter::handleStartEvent\n"));

	startFilter();	
}
		
void BWThresholdFilter::handleStopEvent(
	const media_timed_event* pEvent) {
	PRINT(("BWThresholdFilter::handleStopEvent\n"));
	
	stopFilter();
}
		
void BWThresholdFilter::ignoreEvent(const media_timed_event* pEvent)
{
	PRINT(("BWThresholdFilter::ignoreEvent\n"));	
}
...
//void BWThresholdFilter::specializeOutputFormat(media_format& ioFormat)
//{
//	char formatStr[256];
//	string_for_format(ioFormat, formatStr, 255);
//	PRINT(("BWThresholdFilter::specializeOutputFormat()\n"
//		"\tinput format: %s\n", formatStr));
//	
...
void BWThresholdFilter::initParameterValues() {
	m_fMixRatio = 0.5;
...
//void BWThresholdFilter::initParameterWeb() {
//	BParameterWeb* pWeb = new BParameterWeb();
//	BParameterGroup* pTopGroup = pWeb->MakeGroup("BWThresholdFilter Parameters");
//
...
void BWThresholdFilter::initFilter() {
	PRINT(("BWThresholdFilter::initFilter()\n"));
...
void BWThresholdFilter::startFilter() {
	PRINT(("BWThresholdFilter::startFilter()\n"));
}
void BWThresholdFilter::stopFilter() {
	PRINT(("BWThresholdFilter::stopFilter()\n"));
}

// figure processing latency by doing 'dry runs' of filterBuffer()
bigtime_t BWThresholdFilter::calcProcessingLatency() {
	PRINT(("BWThresholdFilter::calcProcessingLatency()\n"));
...
void BWThresholdFilter::filterBuffer(BBuffer* inBuffer)
{
	if (!inBuffer) 
		return;
...
/* Sans BBitmap  */
	...
	while (pi < inData + inHeader->size_used/4){
		...
		b=in&0x000000ff;
		intensity = (r+g+b)/3;
		if (intensity>=bwthreshold) *pi++=0x00ffffff;
		else  *pi++=0x00000000;	
	}
