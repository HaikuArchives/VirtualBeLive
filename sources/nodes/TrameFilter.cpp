...
const char* const TrameFilter::s_nodeName = "TrameFilter";
...
TrameFilter::TrameFilter(BMediaAddOn* pAddOn) :
	// * init base classes
...
{	
	...
	BContinuousParameter *red = main->MakeContinuousParameter(P_RED, B_MEDIA_RAW_VIDEO, "BackGround Red", "ColorBalance", "", 0.0, 255.0, 1.0);
	BContinuousParameter *green = main->MakeContinuousParameter(P_GREEN, B_MEDIA_RAW_VIDEO, "BackGround Green", "ColorBalance", "", 0.0, 255.0, 1.0);
	BContinuousParameter *blue = main->MakeContinuousParameter(P_BLUE, B_MEDIA_RAW_VIDEO, "BackGround Blue", "ColorBalance", "", 0.0, 255.0, 1.0);
	BContinuousParameter *pTHICKNESS = main->MakeContinuousParameter(P_THICKNESS, B_MEDIA_RAW_VIDEO, "Thickness", "ColorBalance", "", 1, 100, 1.0);
	
	alphaR = alphaG = alphaB = 0;
	THICKNESS=1;
	fLastTrameChange = system_time();
...
status_t TrameFilter::HandleMessage(
	int32 code,
	const void* pData,
	size_t size) {
...
void TrameFilter::HandleEvent(const media_timed_event* pEvent, bigtime_t howLate, bool realTimeEvent)
{
	ASSERT(pEvent);
...
void TrameFilter::NodeRegistered() {

	PRINT(("TrameFilter::NodeRegistered()\n"));
...
bigtime_t TrameFilter::OfflineTime()
{
...
status_t TrameFilter::AcceptFormat(const media_destination& destination, media_format* format) 
{
...
void TrameFilter::BufferReceived(BBuffer* pBuffer)
{
	ASSERT(pBuffer);
	...
	{
		PRINT(("TrameFilter::BufferReceived():\n"
			"\tBad destination.\n"));
		pBuffer->Recycle();
		return;
	}
...
status_t TrameFilter::Connected(
	const media_source& source,
	const media_destination& destination,
	const media_format& format,
	media_input* poInput) {
	
	PRINT(("TrameFilter::Connected()\n""\tto source %ld\n", source.id));
...
void TrameFilter::Disconnected(const media_source& source, const media_destination& destination)
{	
	PRINT(("TrameFilter::Disconnected()\n"));
...
void TrameFilter::DisposeInputCookie(int32 cookie)
{
...
status_t TrameFilter::FormatChanged(
	const media_source& source,
	const media_destination& destination,
	int32 changeTag,
...
status_t TrameFilter::GetLatencyFor(
	const media_destination& destination,
	bigtime_t* poLatency,
	media_node_id* poTimeSource) {
	
	PRINT(("TrameFilter::GetLatencyFor()\n"));
...
status_t TrameFilter::GetNextInput(int32* pioCookie, media_input* poInput)
{
	if(*pioCookie != 0)
		return B_BAD_INDEX;
...
void TrameFilter::ProducerDataStatus(const media_destination& destination, int32 status, bigtime_t tpWhen)
{
	PRINT(("TrameFilter::ProducerDataStatus()\n"));
...
status_t TrameFilter::SeekTagRequested(
	const media_destination& destination,
	...
	uint32* poFlags)
{
	...
	PRINT(("TrameFilter::SeekTagRequested()\n"
		"\tNot implemented.\n"));
	return B_ERROR;
}
...
void TrameFilter::AdditionalBufferRequested(
	const media_source& source,
...
//	additionalbufferrequested = true;
	PRINT(("TrameFilter::AdditionalBufferRequested\n"
		"\tOffline mode not implemented."));
}
		
void TrameFilter::Connect(status_t status, const media_source& source, const media_destination& destination, const media_format& format,	char* pioName)
{
	
	PRINT(("TrameFilter::Connect()\n"));
	status_t err;
...
void TrameFilter::Disconnect(const media_source& source, const media_destination& destination)
{
	PRINT(("TrameFilter::Disconnect()\n"));
...
status_t TrameFilter::DisposeOutputCookie(int32 cookie)
{
	return B_OK;
}
		
void TrameFilter::EnableOutput(
	const media_source& source,
	bool enabled,
	int32* _deprecated_)
{
	PRINT(("TrameFilter::EnableOutput()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tbad source\n"));
		return;
...
status_t TrameFilter::FormatChangeRequested(
	const media_source& source,
	const media_destination& destination,
	...
	int32* _deprecated_)
{
	...
	// deny
	PRINT(("TrameFilter::FormatChangeRequested()\n"
		"\tNot supported.\n"));
		
	return B_MEDIA_BAD_FORMAT;
}
		
status_t TrameFilter::FormatProposal(
	const media_source& source,
	media_format* pioFormat)
{

	PRINT(("TrameFilter::FormatProposal()\n"));
...
status_t TrameFilter::FormatSuggestionRequested(
	media_type type,
	int32 quality,
	media_format* poFormat)
{	
	PRINT(("TrameFilter::FormatSuggestionRequested()\n"));
...
status_t TrameFilter::GetLatency(bigtime_t* poLatency)
{	
	PRINT(("TrameFilter::GetLatency()\n"));
...
status_t TrameFilter::GetNextOutput(int32* pioCookie, media_output* poOutput)
{
	if (*pioCookie != 0)
		return B_BAD_INDEX;
...
void TrameFilter::LatencyChanged(
	const media_source& source,
	const media_destination& destination,
	bigtime_t newLatency,
	uint32 flags)
{
	PRINT(("TrameFilter::LatencyChanged()\n"));
...
void TrameFilter::LateNoticeReceived(
	const media_source& source,
	bigtime_t howLate,
	bigtime_t tpWhen)
{
	PRINT(("TrameFilter::LateNoticeReceived()\n"
		"\thowLate == %Ld\n"
		"\twhen    == %Ld\n", howLate, tpWhen));
...
status_t TrameFilter::PrepareToConnect(
	const media_source& source,
	const media_destination& destination,
	media_format* pioFormat,
	...
	char* poName)
{
	...
	PRINT(("TrameFilter::PrepareToConnect()\n"
		"\tproposed format: %s\n", formatStr));
...
status_t TrameFilter::SetBufferGroup(
	const media_source& source,
	BBufferGroup* pGroup)
{
	PRINT(("TrameFilter::SetBufferGroup()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tBad source.\n"));
		return B_MEDIA_BAD_SOURCE;
...
status_t TrameFilter::SetPlayRate(int32 numerator, int32 denominator)
{
	// not supported
	return B_ERROR;
}
	
status_t TrameFilter::VideoClippingChanged(
	const media_source& source,
	int16 numShorts,
	int16* pClipData,
...
status_t TrameFilter::GetParameterValue(int32 id, bigtime_t* last_change, void* value, size_t* size)
{
	switch (id)
	{
		default:
		...
		case P_BLUE:
			*((float *)value) = alphaB;
			break;
		case P_THICKNESS:
			*((float *)value) = THICKNESS;
			break;
	}

	*last_change = fLastTrameChange;
	*size = sizeof(float);

	return B_OK;
}
		
void TrameFilter::SetParameterValue(int32 id, bigtime_t when, const void *value,	size_t size)
{
	float	tmp;
	...
	switch (id)
	{
		case P_RED:
			tmp = *((float*)value);
			alphaR = (int32)tmp;
			BroadcastNewParameterValue(fLastTrameChange, id, &alphaR, sizeof(int32));
			break;
		case P_GREEN:
			tmp = *((float*)value);
			alphaG = (int32)tmp;
			BroadcastNewParameterValue(fLastTrameChange, id, &alphaG, sizeof(int32));
			break;
		case P_BLUE:
			tmp = *((float*)value);
			alphaB = (int32)tmp;
			BroadcastNewParameterValue(fLastTrameChange, id, &alphaB, sizeof(int32));
			break;
		case P_THICKNESS:
			tmp = *((float*)value);
			THICKNESS = (int32)tmp;
			BroadcastNewParameterValue(fLastTrameChange, id, &THICKNESS, sizeof(int32));
			break;
		default:
			break;
	}
	fLastTrameChange = when;
}
...
void TrameFilter::handleStartEvent(
	const media_timed_event* pEvent) {
	PRINT(("TrameFilter::handleStartEvent\n"));

	startFilter();	
}
		
void TrameFilter::handleStopEvent(
	const media_timed_event* pEvent) {
	PRINT(("TrameFilter::handleStopEvent\n"));
	
	stopFilter();
}
		
void TrameFilter::ignoreEvent(const media_timed_event* pEvent)
{
	PRINT(("TrameFilter::ignoreEvent\n"));	
}
...
//void TrameFilter::specializeOutputFormat(media_format& ioFormat)
//{
//	char formatStr[256];
//	string_for_format(ioFormat, formatStr, 255);
//	PRINT(("TrameFilter::specializeOutputFormat()\n"
//		"\tinput format: %s\n", formatStr));
//	
...
void TrameFilter::initParameterValues() {
	m_fMixRatio = 0.5;
...
//void TrameFilter::initParameterWeb() {
//	BParameterWeb* pWeb = new BParameterWeb();
//	BParameterGroup* pTopGroup = pWeb->MakeGroup("TrameFilter Parameters");
//
...
void TrameFilter::initFilter() {
	PRINT(("TrameFilter::initFilter()\n"));
...
void TrameFilter::startFilter() {
	PRINT(("TrameFilter::startFilter()\n"));
}
void TrameFilter::stopFilter() {
	PRINT(("TrameFilter::stopFilter()\n"));
}

// figure processing latency by doing 'dry runs' of filterBuffer()
bigtime_t TrameFilter::calcProcessingLatency() {
	PRINT(("TrameFilter::calcProcessingLatency()\n"));
...
void TrameFilter::filterBuffer(BBuffer* inBuffer)
{
	if (!inBuffer) 
		return;
...
/* Sans BBitmap  */
	...
	//memcpy(outData, inData, inHeader->size_used);
	...
	uint32 BCKG_COLOR=(alphaR<<16)+(alphaG<<8)+alphaB;
	
	do if ((i++/c)%(2*THICKNESS)<THICKNESS) *po = BCKG_COLOR;
