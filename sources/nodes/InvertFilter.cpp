...
const char* const InvertFilter::s_nodeName = "InvertFilter";
...
InvertFilter::InvertFilter(BMediaAddOn* pAddOn) :
	// * init base classes
...
status_t InvertFilter::HandleMessage(
	int32 code,
	const void* pData,
	size_t size) {
...
BMediaAddOn* InvertFilter::AddOn(
	int32* poID) const {
...
void InvertFilter::HandleEvent(const media_timed_event* pEvent, bigtime_t howLate, bool realTimeEvent)
{
	ASSERT(pEvent);
...
void InvertFilter::NodeRegistered() {

	PRINT(("InvertFilter::NodeRegistered()\n"));
...
bigtime_t InvertFilter::OfflineTime()
{
...
status_t InvertFilter::AcceptFormat(const media_destination& destination, media_format* format) 
{
...
void InvertFilter::BufferReceived(BBuffer* pBuffer)
{
	ASSERT(pBuffer);
...
status_t InvertFilter::Connected(
	const media_source& source,
	const media_destination& destination,
	const media_format& format,
	media_input* poInput) {
	
	PRINT(("InvertFilter::Connected()\n""\tto source %ld\n", source.id));
...
void InvertFilter::Disconnected(const media_source& source, const media_destination& destination)
{	
	PRINT(("InvertFilter::Disconnected()\n"));
...
void InvertFilter::DisposeInputCookie(int32 cookie)
{
...
status_t InvertFilter::FormatChanged(
	const media_source& source,
	const media_destination& destination,
	int32 changeTag,
...
status_t InvertFilter::GetLatencyFor(
	const media_destination& destination,
	bigtime_t* poLatency,
	media_node_id* poTimeSource) {
	
	PRINT(("InvertFilter::GetLatencyFor()\n"));
...
status_t InvertFilter::GetNextInput(int32* pioCookie, media_input* poInput)
{
	if(*pioCookie != 0)
		return B_BAD_INDEX;
...
void InvertFilter::ProducerDataStatus(const media_destination& destination, int32 status, bigtime_t tpWhen)
{
	PRINT(("InvertFilter::ProducerDataStatus()\n"));
...
status_t InvertFilter::SeekTagRequested(
	const media_destination& destination,
	...
	uint32* poFlags)
{
	...
	PRINT(("InvertFilter::SeekTagRequested()\n"
		"\tNot implemented.\n"));
	return B_ERROR;
}
...
void InvertFilter::AdditionalBufferRequested(
	const media_source& source,
...
//	additionalbufferrequested = true;
	PRINT(("InvertFilter::AdditionalBufferRequested\n"
		"\tOffline mode not implemented."));
}
		
void InvertFilter::Connect(status_t status, const media_source& source, const media_destination& destination, const media_format& format,	char* pioName)
{
	
	PRINT(("InvertFilter::Connect()\n"));
	status_t err;
...
void InvertFilter::Disconnect(const media_source& source, const media_destination& destination)
{
	PRINT(("InvertFilter::Disconnect()\n"));
...
status_t InvertFilter::DisposeOutputCookie(int32 cookie)
{
	return B_OK;
}
		
void InvertFilter::EnableOutput(
	const media_source& source,
	bool enabled,
	int32* _deprecated_)
{
	PRINT(("InvertFilter::EnableOutput()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tbad source\n"));
		return;
...
status_t InvertFilter::FormatChangeRequested(
	const media_source& source,
	const media_destination& destination,
	...
	int32* _deprecated_)
{
	...
	// deny
	PRINT(("InvertFilter::FormatChangeRequested()\n"
		"\tNot supported.\n"));
		
	return B_MEDIA_BAD_FORMAT;
}
		
status_t InvertFilter::FormatProposal(
	const media_source& source,
	media_format* pioFormat)
{

	PRINT(("InvertFilter::FormatProposal()\n"));
...
status_t InvertFilter::FormatSuggestionRequested(
	media_type type,
	int32 quality,
	media_format* poFormat)
{	
	PRINT(("InvertFilter::FormatSuggestionRequested()\n"));
...
status_t InvertFilter::GetLatency(bigtime_t* poLatency)
{	
	PRINT(("InvertFilter::GetLatency()\n"));
...
status_t InvertFilter::GetNextOutput(int32* pioCookie, media_output* poOutput)
{
	if (*pioCookie != 0)
		return B_BAD_INDEX;
...
void InvertFilter::LatencyChanged(
	const media_source& source,
	const media_destination& destination,
	bigtime_t newLatency,
	uint32 flags)
{
	PRINT(("InvertFilter::LatencyChanged()\n"));
...
void InvertFilter::LateNoticeReceived(
	const media_source& source,
	bigtime_t howLate,
	bigtime_t tpWhen)
{
	PRINT(("InvertFilter::LateNoticeReceived()\n"
		"\thowLate == %Ld\n"
		"\twhen    == %Ld\n", howLate, tpWhen));
...
status_t InvertFilter::PrepareToConnect(
	const media_source& source,
	const media_destination& destination,
	media_format* pioFormat,
	...
	char* poName)
{
	...
	PRINT(("InvertFilter::PrepareToConnect()\n"
		"\tproposed format: %s\n", formatStr));
...
status_t InvertFilter::SetBufferGroup(
	const media_source& source,
	BBufferGroup* pGroup)
{
	PRINT(("InvertFilter::SetBufferGroup()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tBad source.\n"));
		return B_MEDIA_BAD_SOURCE;
...
status_t InvertFilter::SetPlayRate(int32 numerator, int32 denominator)
{
	// not supported
	return B_ERROR;
}
	
status_t InvertFilter::VideoClippingChanged(
	const media_source& source,
	int16 numShorts,
	int16* pClipData,
...
status_t InvertFilter::GetParameterValue(int32 id, bigtime_t* last_change, void* value, size_t* size)
{
//	switch (id)
//	{
//		case P_RANGE:
...
void InvertFilter::SetParameterValue(int32 id, bigtime_t when, const void *value,	size_t size)
{
//	float	tmp;
//
//	if (!value)
...
void InvertFilter::handleStartEvent(
	const media_timed_event* pEvent) {
	PRINT(("InvertFilter::handleStartEvent\n"));

	startFilter();	
}
		
void InvertFilter::handleStopEvent(
	const media_timed_event* pEvent) {
	PRINT(("InvertFilter::handleStopEvent\n"));
	
	stopFilter();
}
		
void InvertFilter::ignoreEvent(const media_timed_event* pEvent)
{
	PRINT(("InvertFilter::ignoreEvent\n"));	
}
...
//void InvertFilter::specializeOutputFormat(media_format& ioFormat)
//{
//	char formatStr[256];
//	string_for_format(ioFormat, formatStr, 255);
//	PRINT(("InvertFilter::specializeOutputFormat()\n"
//		"\tinput format: %s\n", formatStr));
//	
...
void InvertFilter::initParameterValues() {
	m_fMixRatio = 0.5;
...
//void InvertFilter::initParameterWeb() {
//	BParameterWeb* pWeb = new BParameterWeb();
//	BParameterGroup* pTopGroup = pWeb->MakeGroup("InvertFilter Parameters");
//
...
void InvertFilter::initFilter() {
	PRINT(("InvertFilter::initFilter()\n"));
...
void InvertFilter::startFilter() {
	PRINT(("InvertFilter::startFilter()\n"));
}
void InvertFilter::stopFilter() {
	PRINT(("InvertFilter::stopFilter()\n"));
}

// figure processing latency by doing 'dry runs' of filterBuffer()
bigtime_t InvertFilter::calcProcessingLatency() {
	PRINT(("InvertFilter::calcProcessingLatency()\n"));
...
void InvertFilter::filterBuffer(BBuffer* inBuffer)
{
	if (!inBuffer) 
		return;
