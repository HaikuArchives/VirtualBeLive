...
const char* const VenetianStripesTransition::s_nodeName = "VenetianStripesTransition";
...
VenetianStripesTransition::~VenetianStripesTransition() 
{
	// shut down
	if (buffers)
		delete buffers;
...
VenetianStripesTransition::VenetianStripesTransition(BMediaAddOn* pAddOn) :
	// * init base classes
...
{	
	...
	BContinuousParameter *pparam = main->MakeContinuousParameter(P_PARAM,B_MEDIA_RAW_VIDEO,"Stripes Number","","",1.0,20.0,1.0);
	BDiscreteParameter	*pmode	= main->MakeDiscreteParameter(P_MODE,B_MEDIA_RAW_VIDEO,"Mode H/V","");
...
status_t VenetianStripesTransition::HandleMessage(
	int32 code,
	const void* pData,
	size_t size) {
...
BMediaAddOn* VenetianStripesTransition::AddOn(
	int32* poID) const {
...
void VenetianStripesTransition::SetRunMode(run_mode mode)
{
...
void VenetianStripesTransition::HandleEvent(const media_timed_event* pEvent, bigtime_t howLate, bool realTimeEvent)
{
	ASSERT(pEvent);
...
void VenetianStripesTransition::NodeRegistered()
{
	PRINT(("VenetianStripesTransition::NodeRegistered()\n"));
...
bigtime_t VenetianStripesTransition::OfflineTime()
{
...
status_t VenetianStripesTransition::AcceptFormat(const media_destination& destination, media_format* format) 
{
...
void VenetianStripesTransition::BufferReceived(BBuffer* pBuffer)
{
	ASSERT(pBuffer);
	...
	{
		PRINT(("VenetianStripesTransition::BufferReceived():\n"
			"\tBad destination.\n"));
		pBuffer->Recycle();
		return;
	}
...
status_t VenetianStripesTransition::Connected(
	const media_source& source,
	const media_destination& destination,
	const media_format& format,
	media_input* poInput) {
	
	PRINT(("VenetianStripesTransition::Connected()\n""\tto source %ld\n", source.id));
...
void VenetianStripesTransition::Disconnected(const media_source& source, const media_destination& destination)
{	
	PRINT(("VenetianStripesTransition::Disconnected()\n"));
...
void VenetianStripesTransition::DisposeInputCookie(int32 cookie)
{
...
status_t VenetianStripesTransition::FormatChanged(
	const media_source& source,
	const media_destination& destination,
	int32 changeTag,
...
status_t VenetianStripesTransition::GetLatencyFor(
	const media_destination& destination,
	bigtime_t* poLatency,
	media_node_id* poTimeSource) {
	
	PRINT(("VenetianStripesTransition::GetLatencyFor()\n"));
...
status_t VenetianStripesTransition::GetNextInput(int32* pioCookie, media_input* poInput)
{
	if (*pioCookie > 1)
		return B_BAD_INDEX;
	if (*pioCookie == 0)
...
void VenetianStripesTransition::ProducerDataStatus(const media_destination& destination, int32 status, bigtime_t tpWhen)
{
	PRINT(("VenetianStripesTransition::ProducerDataStatus()\n"));
...
status_t VenetianStripesTransition::SeekTagRequested(
	const media_destination& destination,
	...
	uint32* poFlags)
{
	...
	PRINT(("VenetianStripesTransition::SeekTagRequested()\n"
		"\tNot implemented.\n"));
	return B_ERROR;
}
...
void VenetianStripesTransition::AdditionalBufferRequested(
	const media_source& source,
...
//	additionalbufferrequested = true;
	PRINT(("VenetianStripesTransition::AdditionalBufferRequested\n"));
}
		
void VenetianStripesTransition::Connect(status_t status, const media_source& source, const media_destination& destination, const media_format& format,	char* pioName)
{
	
	PRINT(("VenetianStripesTransition::Connect()\n"));
	status_t err;
...
void VenetianStripesTransition::Disconnect(const media_source& source, const media_destination& destination)
{
	PRINT(("VenetianStripesTransition::Disconnect()\n"));
...
status_t VenetianStripesTransition::DisposeOutputCookie(int32 cookie)
{
	return B_OK;
}
		
void VenetianStripesTransition::EnableOutput(
	const media_source& source,
	bool enabled,
	int32* _deprecated_)
{
	PRINT(("VenetianStripesTransition::EnableOutput()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tbad source\n"));
		return;
...
status_t VenetianStripesTransition::FormatChangeRequested(
	const media_source& source,
	const media_destination& destination,
	...
	int32* _deprecated_)
{
	...
	// deny
	PRINT(("VenetianStripesTransition::FormatChangeRequested()\n"
		"\tNot supported.\n"));
		
	return B_MEDIA_BAD_FORMAT;
}
		
status_t VenetianStripesTransition::FormatProposal(
	const media_source& source,
	media_format* pioFormat)
{

	PRINT(("VenetianStripesTransition::FormatProposal()\n"));
...
status_t VenetianStripesTransition::FormatSuggestionRequested(
	media_type type,
	int32 quality,
	media_format* poFormat)
{	
	PRINT(("VenetianStripesTransition::FormatSuggestionRequested()\n"));
...
status_t VenetianStripesTransition::GetLatency(bigtime_t* poLatency)
{	
	PRINT(("VenetianStripesTransition::GetLatency()\n"));
...
status_t VenetianStripesTransition::GetNextOutput(int32* pioCookie, media_output* poOutput)
{
	if (*pioCookie != 0)
		return B_BAD_INDEX;
...
void VenetianStripesTransition::LatencyChanged(
	const media_source& source,
	const media_destination& destination,
	bigtime_t newLatency,
	uint32 flags)
{
	PRINT(("VenetianStripesTransition::LatencyChanged()\n"));
...
void VenetianStripesTransition::LateNoticeReceived(
	const media_source& source,
	bigtime_t howLate,
	bigtime_t tpWhen)
{
	PRINT(("VenetianStripesTransition::LateNoticeReceived()\n"
		"\thowLate == %Ld\n"
		"\twhen    == %Ld\n", howLate, tpWhen));
...
status_t VenetianStripesTransition::PrepareToConnect(
	const media_source& source,
	const media_destination& destination,
	media_format* pioFormat,
	...
	char* poName)
{
	...
	PRINT(("VenetianStripesTransition::PrepareToConnect()\n"
		"\tproposed format: %s\n", formatStr));
...
status_t VenetianStripesTransition::SetBufferGroup(const media_source& source, BBufferGroup* pGroup)
{
	PRINT(("VenetianStripesTransition::SetBufferGroup()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tBad source.\n"));
		return B_MEDIA_BAD_SOURCE;
...
status_t VenetianStripesTransition::SetPlayRate(int32 numerator, int32 denominator)
{
	// not supported
	return B_ERROR;
}
	
status_t VenetianStripesTransition::VideoClippingChanged(
	const media_source& source,
	int16 numShorts,
	int16* pClipData,
...
status_t VenetianStripesTransition::GetParameterValue(int32 id, bigtime_t* last_change, void* value, size_t* size)
{
	switch (id)
	{
		case P_STATE:
...
void VenetianStripesTransition::SetParameterValue(int32 id, bigtime_t when, const void *value,	size_t size)
{
	float	tmp;
	...
	switch (id)
	{
		...
		case P_PARAM:
			tmp = *((float*)value);
			TParam = (uint32)tmp;
			BroadcastNewParameterValue(fLastStateChange, id, &TParam, sizeof(uint32));
			break;
...
void VenetianStripesTransition::Start(bigtime_t performance_time)
{
	BMediaEventLooper::Start(performance_time);
}

void VenetianStripesTransition::Stop(bigtime_t performance_time, bool immediate)
{
	BMediaEventLooper::Stop(performance_time, immediate);
}

void VenetianStripesTransition::Seek(bigtime_t media_time, bigtime_t performance_time)
{
	BMediaEventLooper::Seek(media_time, performance_time);
}

void VenetianStripesTransition::TimeWarp(bigtime_t at_real_time, bigtime_t to_performance_time)
{
...
void VenetianStripesTransition::handleStartEvent(
	const media_timed_event* pEvent) {
	PRINT(("VenetianStripesTransition::handleStartEvent\n"));

	startFilter();	
}
		
void VenetianStripesTransition::handleStopEvent(
	const media_timed_event* pEvent) {
	PRINT(("VenetianStripesTransition::handleStopEvent\n"));
	
	stopFilter();
}
		
void VenetianStripesTransition::ignoreEvent(const media_timed_event* pEvent)
{
	PRINT(("VenetianStripesTransition::ignoreEvent\n"));	
}
...
//void VenetianStripesTransition::specializeOutputFormat(media_format& ioFormat)
//{
//	char formatStr[256];
//	string_for_format(ioFormat, formatStr, 255);
//	PRINT(("VenetianStripesTransition::specializeOutputFormat()\n"
//		"\tinput format: %s\n", formatStr));
//	
...
void VenetianStripesTransition::initFilter() {
	PRINT(("VenetianStripesTransition::initFilter()\n"));
...
void VenetianStripesTransition::startFilter() {
	PRINT(("VenetianStripesTransition::startFilter()\n"));
}
void VenetianStripesTransition::stopFilter() {
	PRINT(("VenetianStripesTransition::stopFilter()\n"));
}

// figure processing latency by doing 'dry runs' of filterBuffer()
bigtime_t VenetianStripesTransition::calcProcessingLatency() {
	PRINT(("VenetianStripesTransition::calcProcessingLatency()\n"));
...
void VenetianStripesTransition::MakeTransition(BBuffer* inBuffer, BBuffer *inBuffer2)
{
	status_t	err;
	...
	/* WORK IT OUT  */
		...
		switch(TMode){
			case 0:
				...
				for (l=0;l<L;l++)
					for (c=0;c<C;c++){
						if ((l%delta)<(TState*delta/100)) {*po++=*p2++;p1++;}
						else {*po++=*p1++;p2++;}}
				break;
			case 1:
				delta=C/TParam;
				for (l=0;l<L;l++)
					for (c=0;c<C;c++){
						if ((c%delta)<(TState*delta/100)) {*po++=*p2++;p1++;}
						else {*po++=*p1++;p2++;}}
				break;
		}
...
// END -- VenetianStripesTransition.cpp --
