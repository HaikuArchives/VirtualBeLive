...
const char* const SwapTransition::s_nodeName = "SwapTransition";
...
SwapTransition::SwapTransition(BMediaAddOn* pAddOn) :
	// * init base classes
...
status_t SwapTransition::HandleMessage(
	int32 code,
	const void* pData,
	size_t size) {
...
BMediaAddOn* SwapTransition::AddOn(
	int32* poID) const {
...
void SwapTransition::SetRunMode(run_mode mode)
{
...
void SwapTransition::HandleEvent(const media_timed_event* pEvent, bigtime_t howLate, bool realTimeEvent)
{
	ASSERT(pEvent);
...
void SwapTransition::NodeRegistered()
{
	PRINT(("SwapTransition::NodeRegistered()\n"));
...
bigtime_t SwapTransition::OfflineTime()
{
...
status_t SwapTransition::AcceptFormat(const media_destination& destination, media_format* format) 
{
...
void SwapTransition::BufferReceived(BBuffer* pBuffer)
{
	ASSERT(pBuffer);
...
status_t SwapTransition::Connected(
	const media_source& source,
	const media_destination& destination,
	const media_format& format,
	media_input* poInput) {
	
	PRINT(("SwapTransition::Connected()\n""\tto source %ld\n", source.id));
...
void SwapTransition::Disconnected(const media_source& source, const media_destination& destination)
{	
	PRINT(("SwapTransition::Disconnected()\n"));
...
void SwapTransition::DisposeInputCookie(int32 cookie)
{
...
status_t SwapTransition::FormatChanged(
	const media_source& source,
	const media_destination& destination,
	int32 changeTag,
...
status_t SwapTransition::GetLatencyFor(
	const media_destination& destination,
	bigtime_t* poLatency,
	media_node_id* poTimeSource) {
	
	PRINT(("SwapTransition::GetLatencyFor()\n"));
...
status_t SwapTransition::GetNextInput(int32* pioCookie, media_input* poInput)
{
	if (*pioCookie > 1)
		return B_BAD_INDEX;
	if (*pioCookie == 0)
...
void SwapTransition::ProducerDataStatus(const media_destination& destination, int32 status, bigtime_t tpWhen)
{
	PRINT(("SwapTransition::ProducerDataStatus()\n"));
...
status_t SwapTransition::SeekTagRequested(
	const media_destination& destination,
	...
	uint32* poFlags)
{
	...
	PRINT(("SwapTransition::SeekTagRequested()\n"
		"\tNot implemented.\n"));
	return B_ERROR;
}
...
void SwapTransition::AdditionalBufferRequested(
	const media_source& source,
...
//	additionalbufferrequested = true;
	PRINT(("SwapTransition::AdditionalBufferRequested\n"));
}
		
void SwapTransition::Connect(status_t status, const media_source& source, const media_destination& destination, const media_format& format,	char* pioName)
{
	
	PRINT(("SwapTransition::Connect()\n"));
	status_t err;
...
void SwapTransition::Disconnect(const media_source& source, const media_destination& destination)
{
	PRINT(("SwapTransition::Disconnect()\n"));
...
status_t SwapTransition::DisposeOutputCookie(int32 cookie)
{
	return B_OK;
}
		
void SwapTransition::EnableOutput(
	const media_source& source,
	bool enabled,
	int32* _deprecated_)
{
	PRINT(("SwapTransition::EnableOutput()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tbad source\n"));
		return;
...
status_t SwapTransition::FormatChangeRequested(
	const media_source& source,
	const media_destination& destination,
	...
	int32* _deprecated_)
{
	...
	// deny
	PRINT(("SwapTransition::FormatChangeRequested()\n"
		"\tNot supported.\n"));
		
	return B_MEDIA_BAD_FORMAT;
}
		
status_t SwapTransition::FormatProposal(
	const media_source& source,
	media_format* pioFormat)
{

	PRINT(("SwapTransition::FormatProposal()\n"));
...
status_t SwapTransition::FormatSuggestionRequested(
	media_type type,
	int32 quality,
	media_format* poFormat)
{	
	PRINT(("SwapTransition::FormatSuggestionRequested()\n"));
...
status_t SwapTransition::GetLatency(bigtime_t* poLatency)
{	
	PRINT(("SwapTransition::GetLatency()\n"));
...
status_t SwapTransition::GetNextOutput(int32* pioCookie, media_output* poOutput)
{
	if (*pioCookie != 0)
		return B_BAD_INDEX;
...
void SwapTransition::LatencyChanged(
	const media_source& source,
	const media_destination& destination,
	bigtime_t newLatency,
	uint32 flags)
{
	PRINT(("SwapTransition::LatencyChanged()\n"));
...
void SwapTransition::LateNoticeReceived(
	const media_source& source,
	bigtime_t howLate,
	bigtime_t tpWhen)
{
	PRINT(("SwapTransition::LateNoticeReceived()\n"
		"\thowLate == %Ld\n"
		"\twhen    == %Ld\n", howLate, tpWhen));
...
status_t SwapTransition::PrepareToConnect(
	const media_source& source,
	const media_destination& destination,
	media_format* pioFormat,
	...
	char* poName)
{
	...
	PRINT(("SwapTransition::PrepareToConnect()\n"
		"\tproposed format: %s\n", formatStr));
...
status_t SwapTransition::SetBufferGroup(const media_source& source, BBufferGroup* pGroup)
{
	PRINT(("SwapTransition::SetBufferGroup()\n"));
	if (source != m_output.source)
	{
		PRINT(("\tBad source.\n"));
		return B_MEDIA_BAD_SOURCE;
...
status_t SwapTransition::SetPlayRate(int32 numerator, int32 denominator)
{
	// not supported
	return B_ERROR;
}
	
status_t SwapTransition::VideoClippingChanged(
	const media_source& source,
	int16 numShorts,
	int16* pClipData,
...
status_t SwapTransition::GetParameterValue(int32 id, bigtime_t* last_change, void* value, size_t* size)
{
	switch (id)
	{
		case P_STATE:
...
void SwapTransition::SetParameterValue(int32 id, bigtime_t when, const void *value,	size_t size)
{
	float	tmp;
...
void SwapTransition::Start(bigtime_t performance_time)
{
	BMediaEventLooper::Start(performance_time);
}

void SwapTransition::Stop(bigtime_t performance_time, bool immediate)
{
	BMediaEventLooper::Stop(performance_time, immediate);
}

void SwapTransition::Seek(bigtime_t media_time, bigtime_t performance_time)
{
	BMediaEventLooper::Seek(media_time, performance_time);
}

void SwapTransition::TimeWarp(bigtime_t at_real_time, bigtime_t to_performance_time)
{
...
void SwapTransition::handleStartEvent(
	const media_timed_event* pEvent) {
	PRINT(("SwapTransition::handleStartEvent\n"));

	startFilter();	
}
		
void SwapTransition::handleStopEvent(
	const media_timed_event* pEvent) {
	PRINT(("SwapTransition::handleStopEvent\n"));
	
	stopFilter();
}
		
void SwapTransition::ignoreEvent(const media_timed_event* pEvent)
{
	PRINT(("SwapTransition::ignoreEvent\n"));	
}
...
//void SwapTransition::specializeOutputFormat(media_format& ioFormat)
//{
//	char formatStr[256];
//	string_for_format(ioFormat, formatStr, 255);
//	PRINT(("SwapTransition::specializeOutputFormat()\n"
//		"\tinput format: %s\n", formatStr));
//	
...
void SwapTransition::initFilter() {
	PRINT(("SwapTransition::initFilter()\n"));
...
void SwapTransition::startFilter() {
	PRINT(("SwapTransition::startFilter()\n"));
}
void SwapTransition::stopFilter() {
	PRINT(("SwapTransition::stopFilter()\n"));
}

// figure processing latency by doing 'dry runs' of filterBuffer()
bigtime_t SwapTransition::calcProcessingLatency() {
	PRINT(("SwapTransition::calcProcessingLatency()\n"));
...
void SwapTransition::MakeTransition(BBuffer* inBuffer, BBuffer *inBuffer2)
{
	status_t	err;
