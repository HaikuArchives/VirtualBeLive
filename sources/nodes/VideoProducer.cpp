...
#define ALERT(a) printf("VideoProducer::"); printf(a); printf("\n")
	...
	BControllable()
{
	...
	fConnected = false;
	fEnabled = false;
	mGoodOfflineConsumer = false;  // Assume that the consumer is not offline aware 

	mDataStatus = B_PRODUCER_STOPPED;
...
{
	if ((BBufferProducer::HandleMessage(message, data, size) < 0)
...
VideoProducer::SetTimeSource(BTimeSource *time_source)
{
	...
	{
		ALERT("SetTimeSource : releasing sem");
		release_sem(fFrameSync);
		ALERT("SetTimeSource : acquire_sem");
		acquire_sem(fFrameSync);
		ALERT("SetTimeSource : sem acquired");
	}
	else release_sem(fFrameSync);
}
...
void VideoProducer::SetOfflineTime(bigtime_t time)
{
		...
		char *io_name)
{
	...
	if (RunMode() == B_OFFLINE)
	{
		ALERT("Connect : releasing sem");
		release_sem(fFrameSync);
		ALERT("Connect : acquire_sem");
		acquire_sem(fFrameSync);
		ALERT("Connect : sem acquired");
	}
	else release_sem(fFrameSync);
}
		...
		const media_seek_tag *prev_tag)
{
	...
	mGoodOfflineConsumer = true;
	ALERT("AdditionalBufferRequested");
	if (RunMode() == B_OFFLINE)
	{
		ALERT("AdditionalBufferRequested : releasing sem");
		release_sem(fFrameSync);
		ALERT("AdditionalBufferRequested : acquire_sem");
		acquire_sem(fFrameSync);
		ALERT("AdditionalBufferRequested : sem acquired");
	}
	else release_sem(fFrameSync);
}
...
VideoProducer::HandleStart(bigtime_t performance_time)
{
	...
	fFrameSync = create_sem(1, "frame synchronization");
	if (fFrameSync < B_OK)
		return;
	ALERT("HandleStart : acquiring sem for first time");
	acquire_sem(fFrameSync);
	fTimeToQuit = false;
	...
	sleep(1);
	if (RunMode() == B_OFFLINE)
	{ // il fo envoyer le premier buffer en mode offline...
		ALERT("HandleStart : releasing sem");
		release_sem(fFrameSync);
		ALERT("HandleStart : acquire_sem");
		acquire_sem(fFrameSync);
		ALERT("HandleStart : sem acquired");
	}
	else release_sem(fFrameSync);
...
VideoProducer::HandleStop(void)
{
	...
	if (RunMode() == B_OFFLINE)
	{
		...
		{
			fTimeToQuit = true;
			ALERT("HandleStop : releasing sem");
			release_sem(fFrameSync);
...
VideoProducer::HandleTimeWarp(bigtime_t performance_time)
{
	...
	if (RunMode() == B_OFFLINE)
	{
		ALERT("HandleTimeWarp : releasing sem");
		release_sem(fFrameSync);
		acquire_sem(fFrameSync);
	}
	else release_sem(fFrameSync);
...
VideoProducer::HandleSeek(bigtime_t performance_time)
{
	...
	if (RunMode() == B_OFFLINE)
	{
/*		ALERT("HandleSeek : releasing sem");
		release_sem(fFrameSync);
		acquire_sem(fFrameSync);
*/	}
	else release_sem(fFrameSync);
...
VideoProducer::FrameGenerator()
{
	...
	while (!fTimeToQuit) 
	{
		printf("FrameGenerator() Entered\n");
		status_t err;
		if (B_OFFLINE == RunMode())
		{
			if (firstTime)
			{
				ALERT("First loop in offlinemode");
				wait_until = system_time() + 1000 * 100;
				//err = acquire_sem_etc(fFrameSync, 1, B_ABSOLUTE_TIMEOUT, wait_until);
				err = acquire_sem(fFrameSync);
				ALERT("first sem acquire passed");
				firstTime = false;
/*				if (err == B_TIMED_OUT)
				{
					ALERT("Timed out");
					mGoodOfflineConsumer = false;
				}
				else*/ 
				if (err != B_NO_ERROR)
				{
					ALERT("Erreur lors du premier acquire_sem");
				}
				else
				{
					ALERT("Good Acquire");
			...
			{
				if (mGoodOfflineConsumer)
				{
					err = acquire_sem(fFrameSync);
					ALERT("FrameGenerator: sem acquired");
				}
				else
				{
					wait_until = system_time() + 1000 * 1000;
					err = acquire_sem_etc(fFrameSync, 1, B_ABSOLUTE_TIMEOUT, wait_until);
				}
			...
			{
				ALERT("Erreur lors du second acquire_sem");
				break;
			}
			if (err == B_BAD_SEM_ID)
				break;
			...
			if (!buffer)
				continue;
			ALERT("Buffer Request Successfull");
...
//			h->time_source = TimeSource()->ID();
			...
			ALERT("FrameGenerator : releasing sem");
			release_sem(fFrameSync);
		}
		else
		{
			err = acquire_sem_etc(fFrameSync, 1, B_ABSOLUTE_TIMEOUT,
					wait_until);
			...
			{
				ALERT("Buffer Sent!(not in offline mode)");
			}
		}
	}
